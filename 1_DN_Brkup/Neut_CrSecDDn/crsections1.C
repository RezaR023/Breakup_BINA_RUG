...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcrsecsfiWholefinal_factor2_mbarn_Neut.root","recreate");
  fitfunctions();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(b3_ID==1 && LT>.1 && dowscale<100 && abs(b3_WLtdc1-b3_WLtdc2)<80 && b3_T1>0 && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 && b3_Pol==31 && b3_MWPChits==1 /*&& b3_ballhits==1*/)
      {//<<--<<--!
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
	    ...
	    if ((abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && b3_chNeut1==1 && b3_chNeut2==2 && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		||(abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && b3_chNeut1==2 && b3_chNeut2==1 && phi_selection(tta2,tta1,b3_MWPCphi1,b3_MWPCphi2)) )
	      { 
		contrevent++;
		...
		//if (abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		if (1==1 && abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && b3_chNeut1==1 && b3_chNeut2==2 && cutgIneff->IsInside(b3_EFW2,b3_EFW1) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		  { 
		    ...
		    TOF2pp=R2/(C0*TMath::Sqrt(1-TMath::Power((mn/(Ptarg0+mn)),2)));
		    ...
		    if ( 1==1 && MMP>938.2 && MMP<938.7 && cutg21->IsInside(b3_EFW1,deltTOF2) /*&& cutgpm1->IsInside(b3_EFW1,deltTOF2)*/)
		      {  
		...
		if (1==1 && abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && b3_chNeut1==2 && b3_chNeut2==1 && cutgIneff->IsInside(b3_EFW1,b3_EFW2) && phi_selection(tta2,tta1,b3_MWPCphi1,b3_MWPCphi2))
		  {
		    ...
		    *****************/
		    ...
		    TOF1pp=R1/(C0*TMath::Sqrt(1-TMath::Power((mn/(Ptarg0n+mn)),2)));
		    ...
		    if ( 1==1 && MMPn>938.2 && MMPn<938.7 && cutg21->IsInside(b3_EFW2,deltTOF3) /*&& cutgpm1->IsInside(b3_EFW2,deltTOF3)*/)
		      {   
			he0[q]->Fill(b3_EFW2,b3_EFW1);
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ 
	  ...
	  output[q][j] =(double) sbin1D[q][j]->GetEntries();//Integral(sig1,sig2);
	  ...
	  double count11=(double)sbinraw1D[q][j]->GetEntries();//Integral(sig1E,sig2E);
	  //sbinraw1D[q][j]->Add(f22,-1);
	  double count12=(double)sbinraw1D[q][j]->GetEntries();//Integral(sig1E,sig2E);
