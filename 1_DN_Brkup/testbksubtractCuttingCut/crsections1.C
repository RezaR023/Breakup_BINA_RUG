...
funcc2->SetParameters(25.69,-0.7636,0.01181,-8.67e-5,2.307e-7);
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeHistNeutron_Snr12_Nbin18_newcut1_testBkSubtCutingCutn_inclRawCorr.root","recreate");
  fitfunctions();
			...
			for(int j=0 ; j<Snr ; j++)
			  {
			      ...
			      if(b3_Pol==31 /*&& value1>-6 && value1<8*/) 
				{
				  ...
				  Phi31pmn[q][j]->Fill(b3_MWPCphi1,dowscale/LT);
				  Phiraw31pmn[q][j]->Fill(b3_MWPCphi1);
				}
			      if(b3_Pol==27 /*&& value1>-6 && value1<8*/) 
				{
				  Phi27pmn[q][j]->Fill(b3_MWPCphi1,dowscale/LT);
				  Phiraw27pmn[q][j]->Fill(b3_MWPCphi1);
				}
			      if(b3_Pol==13 /*&& value1>-6 && value1<8*/) 
				{
				  Phi13pmn[q][j]->Fill(b3_MWPCphi1,dowscale/LT);
				  Phiraw13pmn[q][j]->Fill(b3_MWPCphi1);
				}
			      }
			  }
		      }
			...
			for(int j=0 ; j<Snr ; j++)
			  {
				...
				ETpmbkg[q][j]->Fill(deltTOF3);	
			      if(b3_Pol==31 /*&& value1>-6 && value1<8*/) 
				{
				  Phi31pmn[q][j]->Fill(b3_MWPCphi2,dowscale/LT);
				  Phiraw31pmn[q][j]->Fill(b3_MWPCphi2);
				}
			      if(b3_Pol==27 /*&& value1>-6 && value1<8*/) 
				{
				  Phi27pmn[q][j]->Fill(b3_MWPCphi2,dowscale/LT);
				  Phiraw27pmn[q][j]->Fill(b3_MWPCphi2);
				}
			      if(b3_Pol==13 /*&& value1>-6 && value1<8*/) 
				{
				  Phi13pmn[q][j]->Fill(b3_MWPCphi2,dowscale/LT);
				  Phiraw13pmn[q][j]->Fill(b3_MWPCphi2);
				}
			      }
			  }
		      }
	  ...
	  for(int k=0;k<Phiraw31pm[q][j]->GetNbinsX();k++)
	    {
	      ...
	      double val0=Phi31pm[q][j]->GetBinContent(k);
	      double valn=.5*Phi31pmn[q][j]->GetBinContent(k);
	      double valnn=val0-(int)valn;

	      double valnr=.5*Phiraw31pmn[q][j]->GetBinContent(k);
	      double valnnr=val-(int)valnr;
	      double E2valnnr=val+(int)valnr;
	      double Evalnnr=sqrt(E2valnnr);
	      if (valnnr>0)   
		{
		  Phiraw31pm[q][j]->SetBinContent(k,valnnr);
		  Phiraw31pm[q][j]->SetBinError(k,Evalnnr);
		}
	      else if (valnnr<=0)   
		{
		  Phiraw31pm[q][j]->SetBinContent(k,0.);
		  Phiraw31pm[q][j]->SetBinError(k,5*val);
		}
	      //if(val>0 && valnnr>0)
		//	{
		  //		  Phiraw31pm[q][j]->SetBinContent(k,valnnr);
	      ...
	      if(val>0 && valnn>0)
		{
		  ...
		  Phi31pm[q][j]->SetBinContent(k,valnn);
		}
	    }
	  for(int k=0;k<Phiraw27pm[q][j]->GetNbinsX();k++)
	    {
	      ...
	      double val0=Phi27pm[q][j]->GetBinContent(k);
	      double valn=.5*Phi27pmn[q][j]->GetBinContent(k);
	      double valnn=val0-(int)valn;

	      double valnr=.5*Phiraw27pmn[q][j]->GetBinContent(k);
	      double valnnr=val-(int)valnr;
	      double E2valnnr=val+(int)valnr;
	      double Evalnnr=sqrt(E2valnnr);
	      if (valnnr>0)   
		{
		  Phiraw27pm[q][j]->SetBinContent(k,valnnr);
		  Phiraw27pm[q][j]->SetBinError(k,Evalnnr);
		}
	      else if (valnnr<=0)   
		{
		  Phiraw27pm[q][j]->SetBinContent(k,0.);
		  Phiraw27pm[q][j]->SetBinError(k,5*val);
		}
	      ...
	      if(val>0 && valnn>0)
		{
		  ...
		  Phi27pm[q][j]->SetBinContent(k,valnn);
		}
	    }
	  for(int k=0;k<Phiraw13pm[q][j]->GetNbinsX();k++)
	    {
	      ...
	      double val0=Phi13pm[q][j]->GetBinContent(k);
	      double valn=.5*Phi13pmn[q][j]->GetBinContent(k);
	      double valnn=val0-(int)valn;

	      double valnr=.5*Phiraw13pmn[q][j]->GetBinContent(k);
	      double valnnr=val-(int)valnr;
	      double E2valnnr=val+(int)valnr;
	      double Evalnnr=sqrt(E2valnnr);
	      if (valnnr>0)   
		{
		  Phiraw13pm[q][j]->SetBinContent(k,valnnr);
		  Phiraw13pm[q][j]->SetBinError(k,Evalnnr);
		}
	      else if (valnnr<=0)   
		{
		  Phiraw13pm[q][j]->SetBinContent(k,0.);
		  Phiraw13pm[q][j]->SetBinError(k,5*val);
		}
	      ...
	      if(val>0 && valnn>0)
		{
		  ...
		  Phi13pm[q][j]->SetBinContent(k,valnn);
		}
	    }
	    /**
...
int BookingHist(){
  ...
  /*  for (int i=0;i<Nkinema;i++){
   ...
   for (int i=0;i<Nkinema;i++){
    for(int j=0 ; j<Snr ; j++)
      {
	/*sprintf(car7pmn[i][j],"Phipmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],31,j);
	sprintf(car8pmn[i][j],"Phirawpmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],31,j);
	sprintf(car9pmn[i][j],"Phipmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],27,j);
	sprintf(car10pmn[i][j],"Phirawpmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],27,j);
	sprintf(car11pmn[i][j],"Phipmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],13,j);
	sprintf(car12pmn[i][j],"Phirawpmn%d_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],13,j);*/
	Phi31pmn[i][j]= new TH1D (car7pm[i][j],"pol;phi",Nbin,0,360);
	Phiraw31pmn[i][j]= new TH1D (car8pm[i][j],"pol;phi",Nbin,0,360);
	Phi27pmn[i][j]= new TH1D (car9pm[i][j],"pol;phi",Nbin,0,360);
	Phiraw27pmn[i][j]= new TH1D (car10pm[i][j],"pol;phi",Nbin,0,360);
	Phi13pmn[i][j]= new TH1D (car11pm[i][j],"pol;phi",Nbin,0,360);
	Phiraw13pmn[i][j]= new TH1D (car12pm[i][j],"pol;phi",Nbin,0,360);
	Phi31pmn[i][j]->Sumw2();
	//Phiraw31pm[i][j]->Sumw2();
	Phi27pmn[i][j]->Sumw2();
	//Phiraw27pm[i][j]->Sumw2();
