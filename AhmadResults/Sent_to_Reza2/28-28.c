{
//=========Macro generated from canvas: canvas_15/Cross section and ...
//=========  (Sat Sep 12 07:22:48 2009) by ROOT version5.06/00
   TCanvas *canvas_15 = new TCanvas("canvas_15", "Cross section and ...",4,21,1202,820);
   gStyle->SetOptStat(0);
   canvas_15->Range(0,0,1,1);
   canvas_15->SetFillColor(10);
   canvas_15->SetBorderSize(2);
   canvas_15->SetFrameFillColor(0);
   TLatex *   tex = new TLatex(0.48,0.042,"S [MeV]");
   tex->SetTextSize(0.0329484);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.05,0.72,"#frac{d^{5}#sigma}{d#Omega_{1}d#Omega_{2}dS} [#frac{#mub}{sr^{2} MeV}]");
   tex->SetTextSize(0.029484);
   tex->SetTextAngle(90);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.05,0.6,"iT_{11}");
   tex->SetTextSize(0.0329484);
   tex->SetTextAngle(90);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.05,0.39,"T_{20}");
   tex->SetTextSize(0.0329484);
   tex->SetTextAngle(90);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.05,0.185,"T_{22}");
   tex->SetTextSize(0.0329484);
   tex->SetTextAngle(90);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.2,0.96,"#phi_{12} = 180^{#circ}");
   tex->SetTextSize(0.0329484);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.47,0.96,"#phi_{12} = 160^{#circ}");
   tex->SetTextSize(0.0329484);
   tex->SetLineWidth(2);
   tex->Draw();
      tex = new TLatex(0.75,0.96,"#phi_{12} = 140^{#circ}");
   tex->SetTextSize(0.0329484);
   tex->SetLineWidth(2);
   tex->Draw();
  
// ------------>Primitives in pad: pad1
   TPad *pad1 = new TPad("pad1", "pad1",0.1,0.075,1,0.945);
   pad1->Draw();
   pad1->cd();
   pad1->Range(0,0,1,1);
   pad1->SetFillColor(10);
   pad1->SetBorderSize(2);
   pad1->SetLeftMargin(0.0333333);
   pad1->SetRightMargin(0.03125);
   pad1->SetTopMargin(0.0250522);
   pad1->SetBottomMargin(0.0250522);
   pad1->SetFrameFillColor(0);
  
// ------------>Primitives in pad: pad1_1
   pad1_1 = new TPad("pad1_1", "pad1_1",0,0.737474,0.345139,0.974948);
   pad1_1->Draw();
   pad1_1->cd();
   pad1_1->Range(84.878,-3,280,0.30103);
   pad1_1->SetFillColor(10);
   pad1_1->SetBorderMode(0);
   pad1_1->SetBorderSize(2);
   pad1_1->SetLogy();
   pad1_1->SetTickx();
   pad1_1->SetTicky();
   pad1_1->SetLeftMargin(0.18);
   pad1_1->SetRightMargin(0);
   pad1_1->SetTopMargin(0);
   pad1_1->SetBottomMargin(0);
   pad1_1->SetFrameFillColor(0);
   
   //TH1 *hisCro = new TH2D("hisCro","",15,120,280,2000,1,2000);
   TH1 *hisCro = new TH2D("hisCro","",110,130,240,2000,1,2000);
   hisCro->SetStats(0);
   hisCro->GetXaxis()->CenterTitle(true);
   hisCro->GetXaxis()->SetNdivisions(207);
   hisCro->GetXaxis()->SetLabelOffset(0.03);
   hisCro->GetXaxis()->SetLabelSize(0.08);
   hisCro->GetXaxis()->SetTitleOffset(0.9);
   hisCro->GetYaxis()->CenterTitle(true);
   hisCro->GetYaxis()->SetLabelOffset(0.03);
   hisCro->GetYaxis()->SetLabelSize(0.08);
   hisCro->GetYaxis()->SetTitleOffset(0.9);
   hisCro->Draw("");
   
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_Cro_45");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,0.0156904*1000);
   gre->SetPointError(0,0,0.000889725);
   gre->SetPoint(1,150+5,0.0189746*1000);
   gre->SetPointError(1,0,0.000982215);
   gre->SetPoint(2,160+5,0.0214048*1000);
   gre->SetPointError(2,0,0.00104215);
   gre->SetPoint(3,170+5,0.0339128*1000);
   gre->SetPointError(3,0,0.00131879);
   gre->SetPoint(4,180+5,0.0654945*1000);
   gre->SetPointError(4,0,0.00181249);
   gre->SetPoint(5,190+5,0.172238*1000);
   gre->SetPointError(5,0,0.00295431);
   gre->SetPoint(6,200+5,0.404935*1000);
   gre->SetPointError(6,0,0.00452735);
   gre->SetPoint(7,210+5,0.660126*1000);
   gre->SetPointError(7,0,0.00577121);
   gre->SetPoint(8,220+5,0.525918*1000);
   gre->SetPointError(8,0,0.00516624);
   gre->SetPoint(9,230+5,0.0938641*1000);
   gre->SetPointError(9,0,0.00217737);
   gre->Draw("sp");
 /*     TGraphErrors *gren = new TGraphErrors(10);
   gren->SetName("gr_crsection_28_28_180");
   gren->SetTitle("");
   gren->SetFillColor(1);
   gren->SetLineColor(2);
   gren->SetMarkerColor(2);
   gren->SetMarkerStyle(26);
   gren->SetPoint(0,140.357,18.41905);
   gren->SetPointError(0,0,1.150624);
   gren->SetPoint(1,150.3269,16.2773);
   gren->SetPointError(1,0,1.081091);
   gren->SetPoint(2,160.4185,18.41905);
   gren->SetPointError(2,0,1.176578);
   gren->SetPoint(3,170.3885,22.70255);
   gren->SetPointError(3,0,1.227698);
   gren->SetPoint(4,180.3584,33.41131);
   gren->SetPointError(4,0,1.502523);
   gren->SetPoint(5,190.45,71.96284);
   gren->SetPointError(5,0,2.184273);
   gren->SetPoint(6,200.42,164.0582);
   gren->SetPointError(6,0,3.331432);
   gren->SetPoint(7,210.3899,440.3441);
   gren->SetPointError(7,0,5.461482);
   gren->SetPoint(8,220.4815,887.9703);
   gren->SetPointError(8,0,7.714063);
   gren->SetPoint(9,230.4514,693.0709);
   gren->SetPointError(9,0,6.859919);
   gren->Draw("samep");

   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_crsection_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,140.512,18.9555);
   gre->SetPointError(0,0,1.04883);
   gre->SetPoint(1,150.756,17.79768);
   gre->SetPointError(1,0,1.094036);
   gre->SetPoint(2,161,24.2374);
   gre->SetPointError(2,0,1.179545);
   gre->SetPoint(3,171.244,33.66544);
   gre->SetPointError(3,0,1.296575);
   gre->SetPoint(4,181.488,64.26477);
   gre->SetPointError(4,0,1.763962);
   gre->SetPoint(5,191.732,159.4429);
   gre->SetPointError(5,0,2.714468);
   gre->SetPoint(6,201.976,405.0561);
   gre->SetPointError(6,0,4.298389);
   gre->SetPoint(7,212.22,764.8171);
   gre->SetPointError(7,0,5.868829);
   gre->SetPoint(8,222.464,629.5865);
   gre->SetPointError(8,0,5.405539);
   gre->SetPoint(9,232.708,202.761);
   gre->SetPointError(9,0,3.086774);
   gre->Draw("sp");*/
  /* TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_crsection_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(21);
   gre->SetPoint(0,140.512,14.02583);
   gre->SetPointError(0,0,0.9290244);
   gre->SetPoint(1,150.756,15.22145);
   gre->SetPointError(1,0,0.9533672);
   gre->SetPoint(2,161,19.35825);
   gre->SetPointError(2,0,1.102266);
   gre->SetPoint(3,171.244,21.93962);
   gre->SetPointError(3,0,1.096191);
   gre->SetPoint(4,181.488,37.15553);
   gre->SetPointError(4,0,1.353726);
   gre->SetPoint(5,191.732,74.85525);
   gre->SetPointError(5,0,1.877776);
   gre->SetPoint(6,201.976,184.9062);
   gre->SetPointError(6,0,2.924356);
   gre->SetPoint(7,212.22,464.2281);
   gre->SetPointError(7,0,4.591965);
   gre->SetPoint(8,222.464,817.8011);
   gre->SetPointError(8,0,6.099468);
   gre->SetPoint(9,232.708,579.5597);
   gre->SetPointError(9,0,5.223073);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_crsection_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
//   gre->SetPoint(0,135,8.61423);
//   gre->SetPointError(0,0,0.3913164);
   gre->SetPoint(1,145,13.59715);
   gre->SetPointError(1,0,0.473581);
   gre->SetPoint(2,155,15.75566);
   gre->SetPointError(2,0,0.516442);
//   gre->SetPoint(3,165,739.2963);
//   gre->SetPointError(3,0,19.75363);
   gre->SetPoint(4,175,29.8473);
   gre->SetPointError(4,0,0.6259834);
   gre->SetPoint(5,185,50.32621);
   gre->SetPointError(5,0,0.7824038);
   gre->SetPoint(6,195,109.3754);
   gre->SetPointError(6,0,1.13812);
   gre->SetPoint(7,205,263.2784);
   gre->SetPointError(7,0,1.757747);
   gre->SetPoint(8,215,614.5711);
   gre->SetPointError(8,0,2.663785);
   gre->SetPoint(9,225,963.8792);
   gre->SetPointError(9,0,3.359416);
   gre->SetPoint(10,235,324.4982);
   gre->SetPointError(10,0,1.999859);
 //  gre->SetPoint(11,245,467.5375);
 //  gre->SetPointError(11,0,3.044776);
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_crsection_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,17.5095);
   gre->SetPointError(1,0,0.5251563);
   gre->SetPoint(2,155,21.1617);
   gre->SetPointError(2,0,0.5619714);
   gre->SetPoint(3,165,27.04845);
   gre->SetPointError(3,0,0.5953975);
   gre->SetPoint(4,175,45.96581);
   gre->SetPointError(4,0,0.7517526);
   gre->SetPoint(5,185,96.31711);
   gre->SetPointError(5,0,1.073604);
   gre->SetPoint(6,195,229.8243);
   gre->SetPointError(6,0,1.648046);
   gre->SetPoint(7,205,508.8666);
   gre->SetPointError(7,0,2.434348);
   gre->SetPoint(8,215,788.2054);
   gre->SetPointError(8,0,3.02214);
   gre->SetPoint(9,225,565.6409);
   gre->SetPointError(9,0,2.602137);
   gre->SetPoint(10,235,109.0412);
   gre->SetPointError(10,0,1.253196);
   gre->SetPoint(11,245,8905.603);
   gre->SetPointError(11,0,1.787675);
   ////gre->Draw("sp");
   TF1 *phspSC_28_28_180 = new TF1("phspSC_28_28_180","pol2",13,280);
   phspSC_28_28_180->SetFillColor(19);
   phspSC_28_28_180->SetFillStyle(0);
   phspSC_28_28_180->SetLineWidth(2);
   phspSC_28_28_180->SetParameter(0,-0.0309941);
   phspSC_28_28_180->SetParError(0,0);
   phspSC_28_28_180->SetParLimits(0,0,0);
   phspSC_28_28_180->SetParameter(1,0.00102447);
   phspSC_28_28_180->SetParError(1,0);
   phspSC_28_28_180->SetParLimits(1,0,0);
   phspSC_28_28_180->SetParameter(2,-2.09751e-06);
   phspSC_28_28_180->SetParError(2,0);
   phspSC_28_28_180->SetParLimits(2,0,0);
   phspSC_28_28_180->Draw("same");
   pad1_1->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_4
   pad1_4 = new TPad("pad1_4", "pad1_4",0,0.5,0.345139,0.737474);
   pad1_4->Draw();
   pad1_4->cd();
   pad1_4->Range(84.878,-0.57,280,0.57);
   pad1_4->SetFillColor(10);
   pad1_4->SetBorderMode(0);
   pad1_4->SetBorderSize(2);
   pad1_4->SetTickx();
   pad1_4->SetTicky();
   pad1_4->SetLeftMargin(0.18);
   pad1_4->SetRightMargin(0);
   pad1_4->SetTopMargin(0);
   pad1_4->SetBottomMargin(0);
   pad1_4->SetFrameFillColor(0);
   
   TH1 *hisT11 = new TH2D("hisT11","",110,130,240,20,-0.57,0.57);
   hisT11->SetStats(0);
   hisT11->GetXaxis()->CenterTitle(true);
   hisT11->GetXaxis()->SetNdivisions(207);
   hisT11->GetXaxis()->SetLabelOffset(0.03);
   hisT11->GetXaxis()->SetLabelSize(0.08);
   hisT11->GetXaxis()->SetTitleOffset(0.9);
   hisT11->GetYaxis()->CenterTitle(true);
   hisT11->GetYaxis()->SetLabelOffset(0.03);
   hisT11->GetYaxis()->SetLabelSize(0.08);
   hisT11->GetYaxis()->SetTitleOffset(0.9);
   hisT11->Draw("");
   
   gre = new TGraphErrors(10);
   gre->SetName("gr_T11_45");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.18809);
   gre->SetPointError(0,0,0.0443746);
   gre->SetPoint(1,150+5,-0.142434);
   gre->SetPointError(1,0,0.0473156);
   gre->SetPoint(2,160+5,-0.172617);
   gre->SetPointError(2,0,0.0400933);
   gre->SetPoint(3,170+5,-0.24813);
   gre->SetPointError(3,0,0.0299576);
   gre->SetPoint(4,180+5,-0.259285);
   gre->SetPointError(4,0,0.0199568);
   gre->SetPoint(5,190+5,-0.2816);
   gre->SetPointError(5,0,0.0128565);
   gre->SetPoint(6,200+5,-0.274578);
   gre->SetPointError(6,0,0.00886336);
   gre->SetPoint(7,210+5,-0.26429);
   gre->SetPointError(7,0,0.00716169);
   gre->SetPoint(8,220+5,-0.266046);
   gre->SetPointError(8,0,0.00845514);
   gre->SetPoint(9,230+5,-0.266072);
   gre->SetPointError(9,0,0.0199238);
   gre->Draw("sp");
     /* TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,140.3975,0.002069767);
   gre->SetPointError(0,0,0.0897445);
   gre->SetPoint(1,150.4125,-0.02952382);
   gre->SetPointError(1,0,0.07870671);
   gre->SetPoint(2,160.4275,-0.1189935);
   gre->SetPointError(2,0,0.07834372);
   gre->SetPoint(3,170.4425,-0.2597791);
   gre->SetPointError(3,0,0.06899574);
   gre->SetPoint(4,180.4575,-0.1954735);
   gre->SetPointError(4,0,0.05053162);
   gre->SetPoint(5,190.4725,-0.2906497);
   gre->SetPointError(5,0,0.03522953);
   gre->SetPoint(6,200.4875,-0.3006479);
   gre->SetPointError(6,0,0.02205591);
   gre->SetPoint(7,210.5025,-0.303864);
   gre->SetPointError(7,0,0.01550827);
   gre->SetPoint(8,220.5175,-0.284161);
   gre->SetPointError(8,0,0.01472574);
   gre->SetPoint(9,230.5325,-0.3021984);
   gre->SetPointError(9,0,0.01908568);
   gre->Draw("sp");

   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,140.3975,0.001052018);
   gre->SetPointError(0,0,0.07925361);
   gre->SetPoint(1,150.4125,-0.02305719);
   gre->SetPointError(1,0,0.07165302);
   gre->SetPoint(2,160.4275,-0.1187443);
   gre->SetPointError(2,0,0.06422793);
   gre->SetPoint(3,170.4425,-0.267646);
   gre->SetPointError(3,0,0.06344169);
   gre->SetPoint(4,180.4575,-0.1931549);
   gre->SetPointError(4,0,0.0480109);
   gre->SetPoint(5,190.4725,-0.2911514);
   gre->SetPointError(5,0,0.03460558);
   gre->SetPoint(6,200.4875,-0.3022524);
   gre->SetPointError(6,0,0.02130963);
   gre->SetPoint(7,210.5025,-0.3005859);
   gre->SetPointError(7,0,0.01601439);
   gre->SetPoint(8,220.5175,-0.2843521);
   gre->SetPointError(8,0,0.01462028);
   gre->SetPoint(9,230.5325,-0.3018441);
   gre->SetPointError(9,0,0.01883063);*/
//Fix Par Method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.03301984);
   gre->SetPointError(1,0,0.04633741);
   gre->SetPoint(2,155,-0.07420754);
   gre->SetPointError(2,0,0.03842098);
   gre->SetPoint(3,165,-0.1291059);
   gre->SetPointError(3,0,0.03946111);
   gre->SetPoint(4,175,-0.2192019);
   gre->SetPointError(4,0,0.0331953);
   gre->SetPoint(5,185,-0.240563);
   gre->SetPointError(5,0,0.02678758);
   gre->SetPoint(6,195,-0.2787659);
   gre->SetPointError(6,0,0.01863512);
   gre->SetPoint(7,205,-0.298928);
   gre->SetPointError(7,0,0.01201204);
   gre->SetPoint(8,215,-0.2816491);
   gre->SetPointError(8,0,0.007944679);
   gre->SetPoint(9,225,-0.269653);
   gre->SetPointError(9,0,0.006161797);
   gre->SetPoint(10,235,-0.2646176);
   gre->SetPointError(10,0,0.01001462);
   gre->Draw("sp");
//Ahmad calibration Fix par 
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,-0.02663207);
   gre->SetPointError(1,0,0.03777533);
   gre->SetPoint(2,155,-0.1036908);
   gre->SetPointError(2,0,0.036681);
   gre->SetPoint(3,165,-0.1441139);
   gre->SetPointError(3,0,0.03485365);
   gre->SetPoint(4,175,-0.2232808);
   gre->SetPointError(4,0,0.02613353);
   gre->SetPoint(5,185,-0.2695243);
   gre->SetPointError(5,0,0.01921359);
   gre->SetPoint(6,195,-0.300701);
   gre->SetPointError(6,0,0.01253457);
   gre->SetPoint(7,205,-0.2765354);
   gre->SetPointError(7,0,0.008669268);
   gre->SetPoint(8,215,-0.2880478);
   gre->SetPointError(8,0,0.006994246);
   gre->SetPoint(9,225,-0.25215);
   gre->SetPointError(9,0,0.008075874);
   gre->SetPoint(10,235,-0.2718437);
   gre->SetPointError(10,0,0.01599487);
  // gre->Draw("sp");
//New selection method for odd and even parity
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,-0.03117766);
   gre->SetPointError(1,0,0.04825137);
   gre->SetPoint(2,155,-0.06177312);
   gre->SetPointError(2,0,0.03997952);
   gre->SetPoint(3,165,-0.1257378);
   gre->SetPointError(3,0,0.04066719);
   gre->SetPoint(4,175,-0.2171306);
   gre->SetPointError(4,0,0.03397894);
   gre->SetPoint(5,185,-0.2399062);
   gre->SetPointError(5,0,0.02721623);
   gre->SetPoint(6,195,-0.277363);
   gre->SetPointError(6,0,0.01895275);
   gre->SetPoint(7,205,-0.3000939);
   gre->SetPointError(7,0,0.01210279);
   gre->SetPoint(8,215,-0.2821388);
   gre->SetPointError(8,0,0.007971073);
   gre->SetPoint(9,225,-0.2702691);
   gre->SetPointError(9,0,0.006210984);
   gre->SetPoint(10,235,-0.2651921);
   gre->SetPointError(10,0,0.01007554);
//   gre->Draw("sp");
//Prev method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(22);
   gre->SetPoint(1,145,-0.03279886);
   gre->SetPointError(1,0,0.04606181);
   gre->SetPoint(2,155,-0.0739712);
   gre->SetPointError(2,0,0.04165576);
   gre->SetPoint(3,165,-0.1288502);
   gre->SetPointError(3,0,0.04046245);
   gre->SetPoint(4,175,-0.2178678);
   gre->SetPointError(4,0,0.03520524);
   gre->SetPoint(5,185,-0.2405207);
   gre->SetPointError(5,0,0.02753415);
   gre->SetPoint(6,195,-0.2790953);
   gre->SetPointError(6,0,0.01847273);
   gre->SetPoint(7,205,-0.300191);
   gre->SetPointError(7,0,0.0116929);
   gre->SetPoint(8,215,-0.2830648);
   gre->SetPointError(8,0,0.00777743);
   gre->SetPoint(9,225,-0.2709731);
   gre->SetPointError(9,0,0.006144623);
   gre->SetPoint(10,235,-0.2674082);
   gre->SetPointError(10,0,0.01011879);
   gre->SetPoint(11,245,-0.2654334);
   gre->SetPointError(11,0,0.007566612);
// gre->Draw("sp");
//Set Par method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(22);
   gre->SetPoint(1,145,-0.03281814);
   gre->SetPointError(1,0,0.04606194);
   gre->SetPoint(2,155,-0.0739754);
   gre->SetPointError(2,0,0.04165621);
   gre->SetPoint(3,165,-0.1288573);
   gre->SetPointError(3,0,0.04046278);
   gre->SetPoint(4,175,-0.2178764);
   gre->SetPointError(4,0,0.03520512);
   gre->SetPoint(5,185,-0.2405219);
   gre->SetPointError(5,0,0.02753422);
   gre->SetPoint(6,195,-0.2790827);
   gre->SetPointError(6,0,0.01847273);
   gre->SetPoint(7,205,-0.3001909);
   gre->SetPointError(7,0,0.0116929);
   gre->SetPoint(8,215,-0.283065);
   gre->SetPointError(8,0,0.00777743);
   gre->SetPoint(9,225,-0.270973);
   gre->SetPointError(9,0,0.006144624);
   gre->SetPoint(10,235,-0.2674075);
   gre->SetPointError(10,0,0.01011879);
   gre->SetPoint(11,245,-0.2654305);
   gre->SetPointError(11,0,0.007566619);
// gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_4->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_7
   pad1_7 = new TPad("pad1_7", "pad1_7",0,0.262526,0.345139,0.5);
   pad1_7->Draw();
   pad1_7->cd();
   pad1_7->Range(84.878,-0.57,280,0.57);
   pad1_7->SetFillColor(10);
   pad1_7->SetBorderMode(0);
   pad1_7->SetBorderSize(2);
   pad1_7->SetTickx();
   pad1_7->SetTicky();
   pad1_7->SetLeftMargin(0.18);
   pad1_7->SetRightMargin(0);
   pad1_7->SetTopMargin(0);
   pad1_7->SetBottomMargin(0);
   pad1_7->SetFrameFillColor(0);
   
   TH1 *hisT20 = new TH2D("hisT20","",110,130,240,20,-0.57,0.57);
   hisT20->SetStats(0);
   hisT20->GetXaxis()->CenterTitle(true);
   hisT20->GetXaxis()->SetNdivisions(207);
   hisT20->GetXaxis()->SetLabelOffset(0.03);
   hisT20->GetXaxis()->SetLabelSize(0.08);
   hisT20->GetXaxis()->SetTitleOffset(0.9);
   hisT20->GetYaxis()->CenterTitle(true);
   hisT20->GetYaxis()->SetLabelOffset(0.03);
   hisT20->GetYaxis()->SetLabelSize(0.08);
   hisT20->GetYaxis()->SetTitleOffset(0.9);
   hisT20->Draw("");
   
   gre = new TGraphErrors(10);
   gre->SetName("gr_T20_45");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,0.170552);
   gre->SetPointError(0,0,0.0824911);
   gre->SetPoint(1,150+5,-0.149844);
   gre->SetPointError(1,0,0.0647411);
   gre->SetPoint(2,160+5,-0.20972);
   gre->SetPointError(2,0,0.0588438);
   gre->SetPoint(3,170+5,-0.355352);
   gre->SetPointError(3,0,0.0428347);
   gre->SetPoint(4,180+5,-0.37114);
   gre->SetPointError(4,0,0.029611);
   gre->SetPoint(5,190+5,-0.454541);
   gre->SetPointError(5,0,0.0180972);
   gre->SetPoint(6,200+5,-0.417592);
   gre->SetPointError(6,0,0.0123169);
   gre->SetPoint(7,210+5,-0.430747);
   gre->SetPointError(7,0,0.00974391);
   gre->SetPoint(8,220+5,-0.418917);
   gre->SetPointError(8,0,0.0114019);
   gre->SetPoint(9,230+5,-0.451344);
   gre->SetPointError(9,0,0.0260867);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,140.3975,-0.1455084);
   gre->SetPointError(0,0,0.1119104);
   gre->SetPoint(1,150.4125,-0.1628162);
   gre->SetPointError(1,0,0.1075063);
   gre->SetPoint(2,160.4275,-0.2899044);
   gre->SetPointError(2,0,0.09848505);
   gre->SetPoint(3,170.4425,-0.2377803);
   gre->SetPointError(3,0,0.08977344);
   gre->SetPoint(4,180.4575,-0.4048221);
   gre->SetPointError(4,0,0.06456133);
   gre->SetPoint(5,190.4725,-0.3695246);
   gre->SetPointError(5,0,0.04371222);
   gre->SetPoint(6,200.4875,-0.4379516);
   gre->SetPointError(6,0,0.02780866);
   gre->SetPoint(7,210.5025,-0.3824087);
   gre->SetPointError(7,0,0.02028832);
   gre->SetPoint(8,220.5175,-0.4117719);
   gre->SetPointError(8,0,0.01794634);
   gre->SetPoint(9,230.5325,-0.4582611);
   gre->SetPointError(9,0,0.02338011);*/
//Fix Par Method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,0.1725666);
   gre->SetPointError(1,0,0.07091211);
   gre->SetPoint(2,155,-0.06897217);
   gre->SetPointError(2,0,0.05878563);
   gre->SetPoint(3,165,-0.05926775);
   gre->SetPointError(3,0,0.05597308);
   gre->SetPoint(4,175,-0.2183608);
   gre->SetPointError(4,0,0.04661042);
   gre->SetPoint(5,185,-0.2846879);
   gre->SetPointError(5,0,0.03542476);
   gre->SetPoint(6,195,-0.3670798);
   gre->SetPointError(6,0,0.02343496);
   gre->SetPoint(7,205,-0.3913618);
   gre->SetPointError(7,0,0.01499886);
   gre->SetPoint(8,215,-0.3740958);
   gre->SetPointError(8,0,0.009932368);
   gre->SetPoint(9,225,-0.3948252);
   gre->SetPointError(9,0,0.007777808);
   gre->SetPoint(10,235,-0.4082993);
   gre->SetPointError(10,0,0.01283313);
   gre->Draw("sp");
//ahmad alibration
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,0.09161728);
   gre->SetPointError(1,0,0.05848729);
   gre->SetPoint(2,155,-0.09951257);
   gre->SetPointError(2,0,0.05208085);
   gre->SetPoint(3,165,-0.03223387);
   gre->SetPointError(3,0,0.05076968);
   gre->SetPoint(4,175,-0.2770845);
   gre->SetPointError(4,0,0.03622551);
   gre->SetPoint(5,185,-0.3778762);
   gre->SetPointError(5,0,0.02459489);
   gre->SetPoint(6,195,-0.3705939);
   gre->SetPointError(6,0,0.01607678);
   gre->SetPoint(7,205,-0.3799408);
   gre->SetPointError(7,0,0.01088416);
   gre->SetPoint(8,215,-0.3936535);
   gre->SetPointError(8,0,0.008757127);
   gre->SetPoint(9,225,-0.4011044);
   gre->SetPointError(9,0,0.01010993);
   gre->SetPoint(10,235,-0.4178043);
   gre->SetPointError(10,0,0.02074257);
   //gre->Draw("sp");
//New selection events for odd and even Parity
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,0.2258092);
   gre->SetPointError(1,0,0.07546289);
   gre->SetPoint(2,155,-0.04987763);
   gre->SetPointError(2,0,0.06083484);
   gre->SetPoint(3,165,-0.03817544);
   gre->SetPointError(3,0,0.05848878);
   gre->SetPoint(4,175,-0.2002879);
   gre->SetPointError(4,0,0.04790882);
   gre->SetPoint(5,185,-0.2738203);
   gre->SetPointError(5,0,0.03622878);
   gre->SetPoint(6,195,-0.3604485);
   gre->SetPointError(6,0,0.02380203);
   gre->SetPoint(7,205,-0.3898478);
   gre->SetPointError(7,0,0.01509297);
   gre->SetPoint(8,215,-0.3729988);
   gre->SetPointError(8,0,0.009980671);
   gre->SetPoint(9,225,-0.3956609);
   gre->SetPointError(9,0,0.007829049);
   gre->SetPoint(10,235,-0.406394);
   gre->SetPointError(10,0,0.01293638);
//   gre->Draw("sp");
//Prev Method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,10.73339*.01);
   gre->SetPointError(1,0,0.4386563);
   gre->SetPoint(2,155,9.962616*.01);
   gre->SetPointError(2,0,0.3873356);
   gre->SetPoint(3,165,10.01719*.01);
   gre->SetPointError(3,0,0.3685124);
   gre->SetPoint(4,175,9.495761*.01);
   gre->SetPointError(4,0,0.3217705);
   gre->SetPoint(5,185,9.275283*.01);
   gre->SetPointError(5,0,0.2498763);
   gre->SetPoint(6,195,8.97716*.01);
   gre->SetPointError(6,0,0.1697354);
   gre->SetPoint(7,205,8.892462*.01);
   gre->SetPointError(7,0,0.109563);
   gre->SetPoint(8,215,8.952714*.01);
   gre->SetPointError(8,0,0.07211436);
   gre->SetPoint(9,225,8.869261*.01);
   gre->SetPointError(9,0,0.05682612);
   gre->SetPoint(10,235,8.815576*.01);
   gre->SetPointError(10,0,0.09415584);
 //  gre->Draw("sp");
// Set Par method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,0.06802704);
   gre->SetPointError(1,0,0.06727185);
   gre->SetPoint(2,155,-0.05081261);
   gre->SetPointError(2,0,0.05938029);
   gre->SetPoint(3,165,-0.04463664);
   gre->SetPointError(3,0,0.05642683);
   gre->SetPoint(4,175,-0.1273762);
   gre->SetPointError(4,0,0.04918763);
   gre->SetPoint(5,185,-0.1622953);
   gre->SetPointError(5,0,0.0381703);
   gre->SetPoint(6,195,-0.2077619);
   gre->SetPointError(6,0,0.02592937);
   gre->SetPoint(7,205,-0.2211072);
   gre->SetPointError(7,0,0.01673307);
   gre->SetPoint(8,215,-0.2116769);
   gre->SetPointError(8,0,0.01101522);
   gre->SetPoint(9,225,-0.2236587);
   gre->SetPointError(9,0,0.008684042);
   gre->SetPoint(10,235,-0.2316267);
   gre->SetPointError(10,0,0.01439077);
   gre->SetPoint(11,245,-0.2335486);
   gre->SetPointError(11,0,0.01079713);
// gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_7->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_10
   pad1_10 = new TPad("pad1_10", "pad1_10",0,0,0.345139,0.262526);
   pad1_10->Draw();
   pad1_10->cd();
   pad1_10->Range(84.878,-0.855,280,0.57);
   pad1_10->SetFillColor(10);
   pad1_10->SetBorderMode(0);
   pad1_10->SetBorderSize(2);
   pad1_10->SetTickx();
   pad1_10->SetTicky();
   pad1_10->SetLeftMargin(0.18);
   pad1_10->SetRightMargin(0);
   pad1_10->SetTopMargin(0);
   pad1_10->SetBottomMargin(0.2);
   pad1_10->SetFrameFillColor(0);
   
   TH1 *hisT22 = new TH2D("hisT22","",100,130,240,20,-0.57,0.57);
   hisT22->SetStats(0);
   hisT22->GetXaxis()->CenterTitle(true);
   hisT22->GetXaxis()->SetNdivisions(207);
   hisT22->GetXaxis()->SetLabelOffset(0.03);
   hisT22->GetXaxis()->SetLabelSize(0.08);
   hisT22->GetXaxis()->SetTitleOffset(0.9);
   hisT22->GetYaxis()->CenterTitle(true);
   hisT22->GetYaxis()->SetLabelOffset(0.03);
   hisT22->GetYaxis()->SetLabelSize(0.08);
   hisT22->GetYaxis()->SetTitleOffset(0.9);
   hisT22->Draw("");
   
   gre = new TGraphErrors(10);
   gre->SetName("gr_T22_45");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.181013);
   gre->SetPointError(0,0,0.0461465);
   gre->SetPoint(1,149+5.907,-0.112639);
   gre->SetPointError(1,0,0.035814);
   gre->SetPoint(2,160+5,-0.15346);
   gre->SetPointError(2,0,0.0328711);
   gre->SetPoint(3,170+5,-0.129827);
   gre->SetPointError(3,0,0.023381);
   gre->SetPoint(4,180+5,-0.133111);
   gre->SetPointError(4,0,0.0162516);
   gre->SetPoint(5,190+5,-0.11674);
   gre->SetPointError(5,0,0.0100952);
   gre->SetPoint(6,200+5,-0.0885978);
   gre->SetPointError(6,0,0.00697089);
   gre->SetPoint(7,210+5,-0.099751);
   gre->SetPointError(7,0,0.0054963);
   gre->SetPoint(8,220+5,-0.104068);
   gre->SetPointError(8,0,0.00654706);
   gre->SetPoint(9,230+5,-0.10801);
   gre->SetPointError(9,0,0.0155552);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,140.3975,-0.184751);
   gre->SetPointError(0,0,0.06295427);
   gre->SetPoint(1,150.4125,-0.1464571);
   gre->SetPointError(1,0,0.06211031);
   gre->SetPoint(2,160.4275,-0.07795579);
   gre->SetPointError(2,0,0.05728159);
   gre->SetPoint(3,170.4425,-0.1332934);
   gre->SetPointError(3,0,0.05178754);
   gre->SetPoint(4,180.4575,-0.05966606);
   gre->SetPointError(4,0,0.04267642);
   gre->SetPoint(5,190.4725,-0.1269743);
   gre->SetPointError(5,0,0.02714256);
   gre->SetPoint(6,200.4875,-0.09670743);
   gre->SetPointError(6,0,0.0179318);
   gre->SetPoint(7,210.5025,-0.1060881);
   gre->SetPointError(7,0,0.01296192);
   gre->SetPoint(8,220.5175,-0.102341);
   gre->SetPointError(8,0,0.01175231);
   gre->SetPoint(9,230.5325,-0.1317184);
   gre->SetPointError(9,0,0.01558815);   
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,140.3975,-0.179273);
   gre->SetPointError(0,0,0.06108745);
   gre->SetPoint(1,150.4125,-0.1412212);
   gre->SetPointError(1,0,0.05989277);
   gre->SetPoint(2,160.4275,-0.07220119);
   gre->SetPointError(2,0,0.053053);
   gre->SetPoint(3,170.4425,-0.1256539);
   gre->SetPointError(3,0,0.04881988);
   gre->SetPoint(4,180.4575,-0.05318282);
   gre->SetPointError(4,0,0.03803138);
   gre->SetPoint(5,190.4725,-0.1147184);
   gre->SetPointError(5,0,0.02452061);
   gre->SetPoint(6,200.4875,-0.08526573);
   gre->SetPointError(6,0,0.01581013);
   gre->SetPoint(7,210.5025,-0.09539085);
   gre->SetPointError(7,0,0.01165418);
   gre->SetPoint(8,220.5175,-0.09110413);
   gre->SetPointError(8,0,0.01046193);
   gre->SetPoint(9,230.5325,-0.1154569);
   gre->SetPointError(9,0,0.01366369);*/
//Fix Par Method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.1216398);
   gre->SetPointError(1,0,0.03897154);
   gre->SetPoint(2,155,-0.1444004);
   gre->SetPointError(2,0,0.03253976);
   gre->SetPoint(3,165,-0.1176605);
   gre->SetPointError(3,0,0.03107266);
   gre->SetPoint(4,175,-0.1030179);
   gre->SetPointError(4,0,0.02643871);
   gre->SetPoint(5,185,-0.08833115);
   gre->SetPointError(5,0,0.02016562);
   gre->SetPoint(6,195,-0.0968116);
   gre->SetPointError(6,0,0.01325819);
   gre->SetPoint(7,205,-0.09257626);
   gre->SetPointError(7,0,0.008464802);
   gre->SetPoint(8,215,-0.09591692);
   gre->SetPointError(8,0,0.005647664);
   gre->SetPoint(9,225,-0.1073531);
   gre->SetPointError(9,0,0.004434602);
   gre->SetPoint(10,235,-0.1129005);
   gre->SetPointError(10,0,0.007353121);
   gre->Draw("sp");
//Ahmad calibration
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,-0.1284734);
   gre->SetPointError(1,0,0.03213583);
   gre->SetPoint(2,155,-0.1232693);
   gre->SetPointError(2,0,0.02865329);
   gre->SetPoint(3,165,-0.09120624);
   gre->SetPointError(3,0,0.02820468);
   gre->SetPoint(4,175,-0.1154691);
   gre->SetPointError(4,0,0.02007658);
   gre->SetPoint(5,185,-0.08491883);
   gre->SetPointError(5,0,0.01366331);
   gre->SetPoint(6,195,-0.08885866);
   gre->SetPointError(6,0,0.00890909);
   gre->SetPoint(7,205,-0.1033137);
   gre->SetPointError(7,0,0.006111878);
   gre->SetPoint(8,215,-0.09652429);
   gre->SetPointError(8,0,0.005014749);
   gre->SetPoint(9,225,-0.1166154);
   gre->SetPointError(9,0,0.005857095);
   gre->SetPoint(10,235,-0.1210836);
   gre->SetPointError(10,0,0.01236371);
//   gre->Draw("sp");
//New selection Method for odd and even parity
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,-0.1525076);
   gre->SetPointError(1,0,0.04266355);
   gre->SetPoint(2,155,-0.1453744);
   gre->SetPointError(2,0,0.03340453);
   gre->SetPoint(3,165,-0.1166113);
   gre->SetPointError(3,0,0.03263182);
   gre->SetPoint(4,175,-0.1133691);
   gre->SetPointError(4,0,0.02736892);
   gre->SetPoint(5,185,-0.08704004);
   gre->SetPointError(5,0,0.02061937);
   gre->SetPoint(6,195,-0.09423683);
   gre->SetPointError(6,0,0.01353011);
   gre->SetPoint(7,205,-0.09321964);
   gre->SetPointError(7,0,0.008537963);
   gre->SetPoint(8,215,-0.095752);
   gre->SetPointError(8,0,0.005690968);
   gre->SetPoint(9,225,-0.1075031);
   gre->SetPointError(9,0,0.004468289);
   gre->SetPoint(10,235,-0.1129468);
   gre->SetPointError(10,0,0.007410486);
//   gre->Draw("sp");
//Prev method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(22);
   gre->SetPoint(1,145,-0.752364*.5);
   gre->SetPointError(1,0,0.2410746);
   gre->SetPoint(2,155,-0.9514437*.5);
   gre->SetPointError(2,0,0.2144029);
   gre->SetPoint(3,165,-0.7746438*.5);
   gre->SetPointError(3,0,0.2045745);
   gre->SetPoint(4,175,-0.7111759*.5);
   gre->SetPointError(4,0,0.1825171);
   gre->SetPoint(5,185,-0.6230685*.5);
   gre->SetPointError(5,0,0.1422426);
   gre->SetPoint(6,195,-0.7011867*.5);
   gre->SetPointError(6,0,0.09602686);
   gre->SetPoint(7,205,-0.6762473*.5);
   gre->SetPointError(7,0,0.06183331);
   gre->SetPoint(8,215,-0.6964085*.5);
   gre->SetPointError(8,0,0.04100509);
   gre->SetPoint(9,225,-0.7843389*.5);
   gre->SetPointError(9,0,0.03240003);
   gre->SetPoint(10,235,-0.8283433*.5);
   gre->SetPointError(10,0,0.05394938);
//   gre->Draw("sp");
// Set Par Method
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.1153948);
   gre->SetPointError(1,0,0.03697095);
   gre->SetPoint(2,155,-0.1458613);
   gre->SetPointError(2,0,0.03286893);
   gre->SetPoint(3,165,-0.1186147);
   gre->SetPointError(3,0,0.03132456);
   gre->SetPoint(4,175,-0.108714);
   gre->SetPointError(4,0,0.02790058);
   gre->SetPoint(5,185,-0.09517398);
   gre->SetPointError(5,0,0.02172852);
   gre->SetPoint(6,195,-0.1071162);
   gre->SetPointError(6,0,0.01466939);
   gre->SetPoint(7,205,-0.103274);
   gre->SetPointError(7,0,0.009443526);
   gre->SetPoint(8,215,-0.106374);
   gre->SetPointError(8,0,0.006263385);
   gre->SetPoint(9,225,-0.1198613);
   gre->SetPointError(9,0,0.004951302);
   gre->SetPoint(10,235,-0.126604);
   gre->SetPointError(10,0,0.008245619);
   gre->SetPoint(11,245,-0.1291326);
   gre->SetPointError(11,0,0.006191333);
//gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_10->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_2
   pad1_2 = new TPad("pad1_2", "pad1_2",0.345139,0.737474,0.656944,0.974948);
   pad1_2->Draw();
   pad1_2->cd();
   pad1_2->Range(120,-3,280,0.30103);
   pad1_2->SetFillColor(10);
   pad1_2->SetBorderMode(0);
   pad1_2->SetBorderSize(2);
   pad1_2->SetLogy();
   pad1_2->SetTickx();
   pad1_2->SetTicky();
   pad1_2->SetLeftMargin(0);
   pad1_2->SetRightMargin(0);
   pad1_2->SetTopMargin(0);
   pad1_2->SetBottomMargin(0);
   pad1_2->SetFrameFillColor(0);
   
   TH1 *hisCro = new TH2D("hisCro","",80,140,220,2000,1,2000);
   hisCro->SetStats(0);
   hisCro->GetXaxis()->CenterTitle(true);
   hisCro->GetXaxis()->SetNdivisions(207);
   hisCro->GetXaxis()->SetLabelOffset(0.03);
   hisCro->GetXaxis()->SetLabelSize(0.08);
   hisCro->GetXaxis()->SetTitleOffset(0.9);
   hisCro->GetYaxis()->CenterTitle(true);
   hisCro->GetYaxis()->SetLabelOffset(0.03);
   hisCro->GetYaxis()->SetLabelSize(0.08);
   hisCro->GetYaxis()->SetTitleOffset(0.9);
   hisCro->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_Cro_46");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,0.0313041*1000);
   gre->SetPointError(0,0,0.00126104);
   gre->SetPoint(1,150+5,0.0298258*1000);
   gre->SetPointError(1,0,0.0012278);
   gre->SetPoint(2,160+5,0.032609*1000);
   gre->SetPointError(2,0,0.00128594);
   gre->SetPoint(3,170+5,0.0385212*1000);
   gre->SetPointError(3,0,0.00139375);
   gre->SetPoint(4,180+5,0.0544154*1000);
   gre->SetPointError(4,0,0.00166986);
   gre->SetPoint(5,190+5,0.0843545*1000);
   gre->SetPointError(5,0,0.00206425);
   gre->SetPoint(6,200+5,0.114125*1000);
   gre->SetPointError(6,0,0.00239341);
   gre->SetPoint(7,210+5,0.12192*1000);
   gre->SetPointError(7,0,0.00248276);
   gre->Draw("sp");
    /*  TGraphErrors *gren = new TGraphErrors(10);
   gren->SetName("gr_crsection_28_28_160");
   gren->SetTitle("");
   gren->SetFillColor(1);
   gren->SetLineColor(2);
   gren->SetMarkerColor(2);
   gren->SetMarkerStyle(26);
   gren->SetPoint(0,137.1455,31.74568);
   gren->SetPointError(0,0,1.563052);
   gren->SetPoint(1,146.8783,30.92286);
   gren->SetPointError(1,0,1.517383);
   gren->SetPoint(2,156.6112,31.74568);
   gren->SetPointError(2,0,1.568014);
   gren->SetPoint(3,166.4585,31.33427);
   gren->SetPointError(3,0,1.532205);
   gren->SetPoint(4,176.1913,35.44833);
   gren->SetPointError(4,0,1.601019);
   gren->SetPoint(5,185.9242,46.96771);
   gren->SetPointError(5,0,1.820431);
   gren->SetPoint(6,195.7715,71.24069);
   gren->SetPointError(6,0,2.250064);
   gren->SetPoint(7,205.5043,119.3752);
   gren->SetPointError(7,0,2.856983);
   gren->SetPoint(8,215.2372,159.6931);
   gren->SetPointError(8,0,3.361368);
   gren->SetPoint(9,225.0845,183.1432);
   gren->SetPointError(9,0,3.677645);
   gren->Draw("samep");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_crsection_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,137.2615,37.2032);
   gre->SetPointError(0,0,1.504303);
   gre->SetPoint(1,147.2645,37.84549);
   gre->SetPointError(1,0,1.607451);
   gre->SetPoint(2,157.2675,37.66306);
   gre->SetPointError(2,0,1.562421);
   gre->SetPoint(3,167.2705,38.93622);
   gre->SetPointError(3,0,1.56282);
   gre->SetPoint(4,177.2735,50.90381);
   gre->SetPointError(4,0,1.639981);
   gre->SetPoint(5,187.2765,77.27094);
   gre->SetPointError(5,0,1.936438);
   gre->SetPoint(6,197.2795,114.9604);
   gre->SetPointError(6,0,2.316794);
   gre->SetPoint(7,207.2825,143.6446);
   gre->SetPointError(7,0,2.609338);
   gre->SetPoint(8,217.2855,132.0026);
   gre->SetPointError(8,0,2.666756);
   gre->SetPoint(9,227.2885,86.60907);
   gre->SetPointError(9,0,2.142995);
   gre->Draw("samep");*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_crsection_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,28.49457);
   gre->SetPointError(1,0,0.6375355);
   gre->SetPoint(2,155,28.89627);
   gre->SetPointError(2,0,0.6557559);
   gre->SetPoint(3,165,33.23926);
   gre->SetPointError(3,0,0.6783583);
   gre->SetPoint(4,175,36.92616);
   gre->SetPointError(4,0,0.6969206);
   gre->SetPoint(5,185,48.21449);
   gre->SetPointError(5,0,0.7723159);
   gre->SetPoint(6,195,72.84419);
   gre->SetPointError(6,0,0.9311657);
   gre->SetPoint(7,205,115.3895);
   gre->SetPointError(7,0,1.155157);
   gre->SetPoint(8,215,155.915);
   gre->SetPointError(8,0,1.364466);
   gre->Draw("sp");
   TF1 *phspSC_28_28_160 = new TF1("phspSC_28_28_160","pol2",13,280);
   phspSC_28_28_160->SetFillColor(19);
   phspSC_28_28_160->SetFillStyle(0);
   phspSC_28_28_160->SetLineWidth(2);
   phspSC_28_28_160->SetParameter(0,-0.162555);
   phspSC_28_28_160->SetParError(0,0);
   phspSC_28_28_160->SetParLimits(0,0,0);
   phspSC_28_28_160->SetParameter(1,0.00266244);
   phspSC_28_28_160->SetParError(1,0);
   phspSC_28_28_160->SetParLimits(1,0,0);
   phspSC_28_28_160->SetParameter(2,-6.77334e-06);
   phspSC_28_28_160->SetParError(2,0);
   phspSC_28_28_160->SetParLimits(2,0,0);
   phspSC_28_28_160->Draw("same");
   pad1_2->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_5
   pad1_5 = new TPad("pad1_5", "pad1_5",0.345139,0.5,0.656944,0.737474);
   pad1_5->Draw();
   pad1_5->cd();
   pad1_5->Range(120,-0.57,280,0.57);
   pad1_5->SetFillColor(10);
   pad1_5->SetBorderMode(0);
   pad1_5->SetBorderSize(2);
   pad1_5->SetTickx();
   pad1_5->SetTicky();
   pad1_5->SetLeftMargin(0);
   pad1_5->SetRightMargin(0);
   pad1_5->SetTopMargin(0);
   pad1_5->SetBottomMargin(0);
   pad1_5->SetFrameFillColor(0);
   
   TH1 *hisT11 = new TH2D("hisT11","",80,140,220,20,-0.57,0.57);
   hisT11->SetStats(0);
   hisT11->GetXaxis()->CenterTitle(true);
   hisT11->GetXaxis()->SetNdivisions(207);
   hisT11->GetXaxis()->SetLabelOffset(0.03);
   hisT11->GetXaxis()->SetLabelSize(0.08);
   hisT11->GetXaxis()->SetTitleOffset(0.9);
   hisT11->GetYaxis()->CenterTitle(true);
   hisT11->GetYaxis()->SetLabelOffset(0.03);
   hisT11->GetYaxis()->SetLabelSize(0.08);
   hisT11->GetYaxis()->SetTitleOffset(0.9);
   hisT11->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T11_46");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.141976);
   gre->SetPointError(0,0,0.0346755);
   gre->SetPoint(1,150+5,-0.142389);
   gre->SetPointError(1,0,0.0358892);
   gre->SetPoint(2,160+5,-0.126618);
   gre->SetPointError(2,0,0.0356608);
   gre->SetPoint(3,170+5,-0.176852);
   gre->SetPointError(3,0,0.0312603);
   gre->SetPoint(4,180+5,-0.250142);
   gre->SetPointError(4,0,0.0257195);
   gre->SetPoint(5,190+5,-0.306597);
   gre->SetPointError(5,0,0.0202925);
   gre->SetPoint(6,200+5,-0.252705);
   gre->SetPointError(6,0,0.018112);
   gre->SetPoint(7,210+5,-0.249604);
   gre->SetPointError(7,0,0.0182322);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,0.04387294);
   gre->SetPointError(0,0,0.0778872);
   gre->SetPoint(1,146.9165,-0.1442398);
   gre->SetPointError(1,0,0.07270046);
   gre->SetPoint(2,156.6875,-0.3756962);
   gre->SetPointError(2,0,0.07013687);
   gre->SetPoint(3,166.4585,-0.2229216);
   gre->SetPointError(3,0,0.07688369);
   gre->SetPoint(4,176.2295,-0.3456829);
   gre->SetPointError(4,0,0.07107367);
   gre->SetPoint(5,186.0005,-0.3901686);
   gre->SetPointError(5,0,0.0596983);
   gre->SetPoint(6,195.7715,-0.3659831);
   gre->SetPointError(6,0,0.05012525);
   gre->SetPoint(7,205.5425,-0.3511094);
   gre->SetPointError(7,0,0.04120231);
   gre->SetPoint(8,215.3135,-0.2952039);
   gre->SetPointError(8,0,0.0371868);
   gre->SetPoint(9,225.0845,-0.2762755);
   gre->SetPointError(9,0,0.03165627);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,0.04068113);
   gre->SetPointError(0,0,0.07774116);
   gre->SetPoint(1,146.9165,-0.1370495);
   gre->SetPointError(1,0,0.07470692);
   gre->SetPoint(2,156.6875,-0.2960593);
   gre->SetPointError(2,0,0.08575164);
   gre->SetPoint(3,166.4585,-0.1963424);
   gre->SetPointError(3,0,0.08300835);
   gre->SetPoint(4,176.2295,-0.3119707);
   gre->SetPointError(4,0,0.07537584);
   gre->SetPoint(5,186.0005,-0.3623271);
   gre->SetPointError(5,0,0.0596735);
   gre->SetPoint(6,195.7715,-0.3392031);
   gre->SetPointError(6,0,0.0502051);
   gre->SetPoint(7,205.5425,-0.3262075);
   gre->SetPointError(7,0,0.04101692);
   gre->SetPoint(8,215.3135,-0.280333);
   gre->SetPointError(8,0,0.03492266);
   gre->SetPoint(9,225.0845,-0.256822);
   gre->SetPointError(9,0,0.03109846);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,137.1455,-0.02733285);
   gre->SetPointError(0,0,0.06103921);
   gre->SetPoint(1,146.9165,-0.05582049);
   gre->SetPointError(1,0,0.06317421);
   gre->SetPoint(2,156.6875,-0.1817089);
   gre->SetPointError(2,0,0.06614808);
   gre->SetPoint(3,166.4585,-0.245513);
   gre->SetPointError(3,0,0.06356328);
   gre->SetPoint(4,176.2295,-0.3333308);
   gre->SetPointError(4,0,0.05747378);
   gre->SetPoint(5,186.0005,-0.2878431);
   gre->SetPointError(5,0,0.04728439);
   gre->SetPoint(6,195.7715,-0.3591329);
   gre->SetPointError(6,0,0.03896442);
   gre->SetPoint(7,205.5425,-0.2883718);
   gre->SetPointError(7,0,0.03675066);
   gre->SetPoint(8,215.3135,-0.2402535);
   gre->SetPointError(8,0,0.03491891);
   gre->SetPoint(9,225.0845,-0.2184276);
   gre->SetPointError(9,0,0.03710567);
      gre->Draw("sp");

   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,137.1455,-0.02728628);
   gre->SetPointError(0,0,0.05884079);
   gre->SetPoint(1,146.9165,-0.05854159);
   gre->SetPointError(1,0,0.05915107);
   gre->SetPoint(2,156.6875,-0.1995843);
   gre->SetPointError(2,0,0.05849517);
   gre->SetPoint(3,166.4585,-0.2528824);
   gre->SetPointError(3,0,0.05907009);
   gre->SetPoint(4,176.2295,-0.3422055);
   gre->SetPointError(4,0,0.05395699);
   gre->SetPoint(5,186.0005,-0.2898317);
   gre->SetPointError(5,0,0.04630089);
   gre->SetPoint(6,195.7715,-0.3659933);
   gre->SetPointError(6,0,0.03695009);
   gre->SetPoint(7,205.5425,-0.290966);
   gre->SetPointError(7,0,0.03581016);
   gre->SetPoint(8,215.3135,-0.2413768);
   gre->SetPointError(8,0,0.03439446);
   gre->SetPoint(9,225.0845,-0.2285553);
   gre->SetPointError(9,0,0.03376248);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.08915433);
   gre->SetPointError(1,0,0.0331265);
   gre->SetPoint(2,155,-0.07713429);
   gre->SetPointError(2,0,0.03324851);
   gre->SetPoint(3,165,-0.1386199);
   gre->SetPointError(3,0,0.03081172);
   gre->SetPoint(4,175,-0.2138387);
   gre->SetPointError(4,0,0.03050668);
   gre->SetPoint(5,185,-0.2453911);
   gre->SetPointError(5,0,0.02680141);
   gre->SetPoint(6,195,-0.2656255);
   gre->SetPointError(6,0,0.02308295);
   gre->SetPoint(7,205,-0.294434);
   gre->SetPointError(7,0,0.01763784);
   gre->SetPoint(8,215,-0.2714861);
   gre->SetPointError(8,0,0.01555876);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,-0.02663207);
   gre->SetPointError(1,0,0.03777533);
   gre->SetPoint(2,155,-0.1036908);
   gre->SetPointError(2,0,0.036681);
   gre->SetPoint(3,165,-0.1441139);
   gre->SetPointError(3,0,0.03485365);
   gre->SetPoint(4,175,-0.2232808);
   gre->SetPointError(4,0,0.02613353);
   gre->SetPoint(5,185,-0.2695243);
   gre->SetPointError(5,0,0.01921359);
   gre->SetPoint(6,195,-0.300701);
   gre->SetPointError(6,0,0.01253457);
   gre->SetPoint(7,205,-0.2765354);
   gre->SetPointError(7,0,0.008669268);
   gre->SetPoint(8,215,-0.2880478);
   gre->SetPointError(8,0,0.006994246);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,-0.09116171);
   gre->SetPointError(1,0,0.03426064);
   gre->SetPoint(2,155,-0.0804512);
   gre->SetPointError(2,0,0.03477819);
   gre->SetPoint(3,165,-0.1344378);
   gre->SetPointError(3,0,0.0316197);
   gre->SetPoint(4,175,-0.2109013);
   gre->SetPointError(4,0,0.03154876);
   gre->SetPoint(5,185,-0.2471971);
   gre->SetPointError(5,0,0.02751402);
   gre->SetPoint(6,195,-0.2624812);
   gre->SetPointError(6,0,0.02414593);
   gre->SetPoint(7,205,-0.286607);
   gre->SetPointError(7,0,0.01826934);
   gre->SetPoint(8,215,-0.2694324);
   gre->SetPointError(8,0,0.01578512);
      gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_5->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_8
   pad1_8 = new TPad("pad1_8", "pad1_8",0.345139,0.262526,0.656944,0.5);
   pad1_8->Draw();
   pad1_8->cd();
   pad1_8->Range(120,-0.57,280,0.57);
   pad1_8->SetFillColor(10);
   pad1_8->SetBorderMode(0);
   pad1_8->SetBorderSize(2);
   pad1_8->SetTickx();
   pad1_8->SetTicky();
   pad1_8->SetLeftMargin(0);
   pad1_8->SetRightMargin(0);
   pad1_8->SetTopMargin(0);
   pad1_8->SetBottomMargin(0);
   pad1_8->SetFrameFillColor(0);
   
   TH1 *hisT20 = new TH2D("hisT20","",80,140,220,20,-0.57,0.57);
   hisT20->SetStats(0);
   hisT20->GetXaxis()->CenterTitle(true);
   hisT20->GetXaxis()->SetNdivisions(207);
   hisT20->GetXaxis()->SetLabelOffset(0.03);
   hisT20->GetXaxis()->SetLabelSize(0.08);
   hisT20->GetXaxis()->SetTitleOffset(0.9);
   hisT20->GetYaxis()->CenterTitle(true);
   hisT20->GetYaxis()->SetLabelOffset(0.03);
   hisT20->GetYaxis()->SetLabelSize(0.08);
   hisT20->GetYaxis()->SetTitleOffset(0.9);
   hisT20->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T20_46");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.010033);
   gre->SetPointError(0,0,0.0532086);
   gre->SetPoint(1,150+5,-0.0583754);
   gre->SetPointError(1,0,0.0524522);
   gre->SetPoint(2,160+5,-0.144478);
   gre->SetPointError(2,0,0.0498304);
   gre->SetPoint(3,170+5,-0.320375);
   gre->SetPointError(3,0,0.0413684);
   gre->SetPoint(4,180+5,-0.366993);
   gre->SetPointError(4,0,0.0343923);
   gre->SetPoint(5,190+5,-0.389218);
   gre->SetPointError(5,0,0.0277551);
   gre->SetPoint(6,200+5,-0.421735);
   gre->SetPointError(6,0,0.0236038);
   gre->SetPoint(7,210+5,-0.43456);
   gre->SetPointError(7,0,0.0230349);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,-0.1312503);
   gre->SetPointError(0,0,0.1038189);
   gre->SetPoint(1,146.9165,-0.1598946);
   gre->SetPointError(1,0,0.1018618);
   gre->SetPoint(2,156.6875,-0.09126302);
   gre->SetPointError(2,0,0.1041181);
   gre->SetPoint(3,166.4585,-0.0467309);
   gre->SetPointError(3,0,0.106767);
   gre->SetPoint(4,176.2295,-0.2687392);
   gre->SetPointError(4,0,0.09255672);
   gre->SetPoint(5,186.0005,-0.4401292);
   gre->SetPointError(5,0,0.07625633);
   gre->SetPoint(6,195.7715,-0.4906001);
   gre->SetPointError(6,0,0.06099951);
   gre->SetPoint(7,205.5425,-0.4208088);
   gre->SetPointError(7,0,0.05089053);
   gre->SetPoint(8,215.3135,-0.4281801);
   gre->SetPointError(8,0,0.04230119);
   gre->SetPoint(9,225.0845,-0.4899304);
   gre->SetPointError(9,0,0.03562176);
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,-0.08024947);
   gre->SetPointError(0,0,0.1062547);
   gre->SetPoint(1,146.9165,-0.09527972);
   gre->SetPointError(1,0,0.105033);
   gre->SetPoint(2,156.6875,-0.05951627);
   gre->SetPointError(2,0,0.1054116);
   gre->SetPoint(3,166.4585,-0.03684002);
   gre->SetPointError(3,0,0.1068377);
   gre->SetPoint(4,176.2295,-0.1557135);
   gre->SetPointError(4,0,0.0976353);
   gre->SetPoint(5,186.0005,-0.2478777);
   gre->SetPointError(5,0,0.08586207);
   gre->SetPoint(6,195.7715,-0.2752131);
   gre->SetPointError(6,0,0.07006817);
   gre->SetPoint(7,205.5425,-0.2352619);
   gre->SetPointError(7,0,0.05711588);
   gre->SetPoint(8,215.3135,-0.2405295);
   gre->SetPointError(8,0,0.04752291);
   gre->SetPoint(9,225.0845,-0.2743849);
   gre->SetPointError(9,0,0.04094533);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,137.1455,-0.03055173);
   gre->SetPointError(0,0,0.09007096);
   gre->SetPoint(1,146.9165,-0.04926658);
   gre->SetPointError(1,0,0.0861452);
   gre->SetPoint(2,156.6875,-0.05106922);
   gre->SetPointError(2,0,0.08763228);
   gre->SetPoint(3,166.4585,-0.09491496);
   gre->SetPointError(3,0,0.08826597);
   gre->SetPoint(4,176.2295,-0.1822749);
   gre->SetPointError(4,0,0.0816845);
   gre->SetPoint(5,186.0005,-0.2368338);
   gre->SetPointError(5,0,0.06788011);
   gre->SetPoint(6,195.7715,-0.2329114);
   gre->SetPointError(6,0,0.0566101);
   gre->SetPoint(7,205.5425,-0.2323872);
   gre->SetPointError(7,0,0.0494764);
   gre->SetPoint(8,215.3135,-0.2752786);
   gre->SetPointError(8,0,0.04639527);
   gre->SetPoint(9,225.0845,-0.2766725);
   gre->SetPointError(9,0,0.04880978);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,137.1455,-0.0339066);
   gre->SetPointError(0,0,0.08990608);
   gre->SetPoint(1,146.9165,-0.07193062);
   gre->SetPointError(1,0,0.08506846);
   gre->SetPoint(2,156.6875,-0.07500845);
   gre->SetPointError(2,0,0.08647488);
   gre->SetPoint(3,166.4585,-0.1606071);
   gre->SetPointError(3,0,0.08498809);
   gre->SetPoint(4,176.2295,-0.3237099);
   gre->SetPointError(4,0,0.07481396);
   gre->SetPoint(5,186.0005,-0.4223341);
   gre->SetPointError(5,0,0.06014058);
   gre->SetPoint(6,195.7715,-0.4148464);
   gre->SetPointError(6,0,0.0502951);
   gre->SetPoint(7,205.5425,-0.4138335);
   gre->SetPointError(7,0,0.04397382);
   gre->SetPoint(8,215.3135,-0.4870452);
   gre->SetPointError(8,0,0.04021057);
   gre->SetPoint(9,225.0845,-0.4912857);
   gre->SetPointError(9,0,0.04220996);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,0.06960258);
   gre->SetPointError(1,0,0.04902024);
   gre->SetPoint(2,155,0.1709498);
   gre->SetPointError(2,0,0.04936401);
   gre->SetPoint(3,165,-0.007459273);
   gre->SetPointError(3,0,0.04490668);
   gre->SetPoint(4,175,-0.1238063);
   gre->SetPointError(4,0,0.04217161);
   gre->SetPoint(5,185,-0.2697634);
   gre->SetPointError(5,0,0.03605201);
   gre->SetPoint(6,195,-0.3363573);
   gre->SetPointError(6,0,0.02941164);
   gre->SetPoint(7,205,-0.3673063);
   gre->SetPointError(7,0,0.02316243);
   gre->SetPoint(8,215,-0.3893414);
   gre->SetPointError(8,0,0.01939841);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,0.09161728);
   gre->SetPointError(1,0,0.05848729);
   gre->SetPoint(2,155,-0.09951257);
   gre->SetPointError(2,0,0.05208085);
   gre->SetPoint(3,165,-0.03223387);
   gre->SetPointError(3,0,0.05076968);
   gre->SetPoint(4,175,-0.2770845);
   gre->SetPointError(4,0,0.03622551);
   gre->SetPoint(5,185,-0.3778762);
   gre->SetPointError(5,0,0.02459489);
   gre->SetPoint(6,195,-0.3705939);
   gre->SetPointError(6,0,0.01607678);
   gre->SetPoint(7,205,-0.3799408);
   gre->SetPointError(7,0,0.01088416);
   gre->SetPoint(8,215,-0.3936535);
   gre->SetPointError(8,0,0.008757127);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,0.08225119);
   gre->SetPointError(1,0,0.0503982);
   gre->SetPoint(2,155,0.1963226);
   gre->SetPointError(2,0,0.05131972);
   gre->SetPoint(3,165,-0.01916134);
   gre->SetPointError(3,0,0.04552241);
   gre->SetPoint(4,175,-0.107019);
   gre->SetPointError(4,0,0.04346661);
   gre->SetPoint(5,185,-0.2590022);
   gre->SetPointError(5,0,0.037022);
   gre->SetPoint(6,195,-0.3243303);
   gre->SetPointError(6,0,0.0301229);
   gre->SetPoint(7,205,-0.3627185);
   gre->SetPointError(7,0,0.02360173);
   gre->SetPoint(8,215,-0.3892878);
   gre->SetPointError(8,0,0.01974271);
      gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_8->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_11
   pad1_11 = new TPad("pad1_11", "pad1_11",0.345139,0,0.656944,0.262526);
   pad1_11->Draw();
   pad1_11->cd();
   pad1_11->Range(120,-0.855,280,0.57);
   pad1_11->SetFillColor(10);
   pad1_11->SetBorderMode(0);
   pad1_11->SetBorderSize(2);
   pad1_11->SetTickx();
   pad1_11->SetTicky();
   pad1_11->SetLeftMargin(0);
   pad1_11->SetRightMargin(0);
   pad1_11->SetTopMargin(0);
   pad1_11->SetBottomMargin(0.2);
   pad1_11->SetFrameFillColor(0);
   
   TH1 *hisT22 = new TH2D("hisT22","",80,140,220,20,-0.57,0.57);
   hisT22->SetStats(0);
   hisT22->GetXaxis()->CenterTitle(true);
   hisT22->GetXaxis()->SetNdivisions(207);
   hisT22->GetXaxis()->SetLabelOffset(0.03);
   hisT22->GetXaxis()->SetLabelSize(0.08);
   hisT22->GetXaxis()->SetTitleOffset(0.9);
   hisT22->GetYaxis()->CenterTitle(true);
   hisT22->GetYaxis()->SetLabelOffset(0.03);
   hisT22->GetYaxis()->SetLabelSize(0.08);
   hisT22->GetYaxis()->SetTitleOffset(0.9);
   hisT22->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T22_46");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.112528);
   gre->SetPointError(0,0,0.0311084);
   gre->SetPoint(1,150+5,-0.112556);
   gre->SetPointError(1,0,0.0298442);
   gre->SetPoint(2,160+5,-0.110201);
   gre->SetPointError(2,0,0.0291385);
   gre->SetPoint(3,170+5,-0.0965096);
   gre->SetPointError(3,0,0.0238847);
   gre->SetPoint(4,180+5,-0.0678602);
   gre->SetPointError(4,0,0.0204308);
   gre->SetPoint(5,190+5,-0.0430129);
   gre->SetPointError(5,0,0.0159915);
   gre->SetPoint(6,200+5,-0.062424);
   gre->SetPointError(6,0,0.0135097);
   gre->SetPoint(7,210+5,-0.0672613);
   gre->SetPointError(7,0,0.0132278);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,-0.06464397);
   gre->SetPointError(0,0,0.05903717);
   gre->SetPoint(1,146.9165,-0.07700744);
   gre->SetPointError(1,0,0.06024062);
   gre->SetPoint(2,156.6875,-0.05322927);
   gre->SetPointError(2,0,0.05773626);
   gre->SetPoint(3,166.4585,-0.06049976);
   gre->SetPointError(3,0,0.06492387);
   gre->SetPoint(4,176.2295,-0.1396756);
   gre->SetPointError(4,0,0.05197281);
   gre->SetPoint(5,186.0005,-0.09825568);
   gre->SetPointError(5,0,0.04650462);
   gre->SetPoint(6,195.7715,-0.06082649);
   gre->SetPointError(6,0,0.03646098);
   gre->SetPoint(7,205.5425,-0.05144222);
   gre->SetPointError(7,0,0.03087056);
   gre->SetPoint(8,215.3135,-0.08191884);
   gre->SetPointError(8,0,0.02508405);
   gre->SetPoint(9,225.0845,-0.04042917);
   gre->SetPointError(9,0,0.02057666);
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(0,137.1455,-0.06622238);
   gre->SetPointError(0,0,0.06042234);
   gre->SetPoint(1,146.9165,-0.07956061);
   gre->SetPointError(1,0,0.06211609);
   gre->SetPoint(2,156.6875,-0.05394878);
   gre->SetPointError(2,0,0.05845352);
   gre->SetPoint(3,166.4585,-0.06056678);
   gre->SetPointError(3,0,0.06496686);
   gre->SetPoint(4,176.2295,-0.1488253);
   gre->SetPointError(4,0,0.05482456);
   gre->SetPoint(5,186.0005,-0.1108933);
   gre->SetPointError(5,0,0.05236264);
   gre->SetPoint(6,195.7715,-0.0699554);
   gre->SetPointError(6,0,0.04188155);
   gre->SetPoint(7,205.5425,-0.05773252);
   gre->SetPointError(7,0,0.0346469);
   gre->SetPoint(8,215.3135,-0.09210544);
   gre->SetPointError(8,0,0.02818046);
   gre->SetPoint(9,225.0845,-0.04649912);
   gre->SetPointError(9,0,0.02365178);
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,137.1455,-0.06095921);
   gre->SetPointError(0,0,0.04981882);
   gre->SetPoint(1,146.9165,0.01432757);
   gre->SetPointError(1,0,0.04945424);
   gre->SetPoint(2,156.6875,-0.02882723);
   gre->SetPointError(2,0,0.04869048);
   gre->SetPoint(3,166.4585,-0.02539545);
   gre->SetPointError(3,0,0.05080268);
   gre->SetPoint(4,176.2295,-0.05175212);
   gre->SetPointError(4,0,0.04617393);
   gre->SetPoint(5,186.0005,-0.05553961);
   gre->SetPointError(5,0,0.04217551);
   gre->SetPoint(6,195.7715,-0.06121388);
   gre->SetPointError(6,0,0.03395636);
   gre->SetPoint(7,205.5425,-0.058448);
   gre->SetPointError(7,0,0.02956192);
   gre->SetPoint(8,215.3135,-0.09034425);
   gre->SetPointError(8,0,0.02698647);
   gre->SetPoint(9,225.0845,-0.05827772);
   gre->SetPointError(9,0,0.02850241);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,137.1455,-0.06084534);
   gre->SetPointError(0,0,0.04972762);
   gre->SetPoint(1,146.9165,0.0141612);
   gre->SetPointError(1,0,0.0488361);
   gre->SetPoint(2,156.6875,-0.02844929);
   gre->SetPointError(2,0,0.0480474);
   gre->SetPoint(3,166.4585,-0.0244534);
   gre->SetPointError(3,0,0.04891606);
   gre->SetPoint(4,176.2295,-0.04742343);
   gre->SetPointError(4,0,0.04229021);
   gre->SetPoint(5,186.0005,-0.04920906);
   gre->SetPointError(5,0,0.03736675);
   gre->SetPoint(6,195.7715,-0.05438627);
   gre->SetPointError(6,0,0.03016844);
   gre->SetPoint(7,205.5425,-0.05195248);
   gre->SetPointError(7,0,0.02627415);
   gre->SetPoint(8,215.3135,-0.07830897);
   gre->SetPointError(8,0,0.02338905);
   gre->SetPoint(9,225.0845,-0.05039978);
   gre->SetPointError(9,0,0.02464846);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.07490084);
   gre->SetPointError(1,0,0.02820686);
   gre->SetPoint(2,155,-0.05319484);
   gre->SetPointError(2,0,0.02783395);
   gre->SetPoint(3,165,-0.0529165);
   gre->SetPointError(3,0,0.02519676);
   gre->SetPoint(4,175,-0.05345461);
   gre->SetPointError(4,0,0.0249403);
   gre->SetPoint(5,185,-0.06498079);
   gre->SetPointError(5,0,0.02106776);
   gre->SetPoint(6,195,-0.03100457);
   gre->SetPointError(6,0,0.01780546);
   gre->SetPoint(7,205,-0.08749629);
   gre->SetPointError(7,0,0.0133378);
   gre->SetPoint(8,215,-0.06188182);
   gre->SetPointError(8,0,0.01122546);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_180");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(4);
   gre->SetMarkerColor(4);
   gre->SetMarkerStyle(24);
   gre->SetPoint(1,145,-0.1284734);
   gre->SetPointError(1,0,0.03213583);
   gre->SetPoint(2,155,-0.1232693);
   gre->SetPointError(2,0,0.02865329);
   gre->SetPoint(3,165,-0.09120624);
   gre->SetPointError(3,0,0.02820468);
   gre->SetPoint(4,175,-0.1154691);
   gre->SetPointError(4,0,0.02007658);
   gre->SetPoint(5,185,-0.08491883);
   gre->SetPointError(5,0,0.01366331);
   gre->SetPoint(6,195,-0.08885866);
   gre->SetPointError(6,0,0.00890909);
   gre->SetPoint(7,205,-0.1033137);
   gre->SetPointError(7,0,0.006111878);
   gre->SetPoint(8,215,-0.09652429);
   gre->SetPointError(8,0,0.005014749);
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_160");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(3);
   gre->SetMarkerColor(3);
   gre->SetMarkerStyle(28);
   gre->SetPoint(1,145,-0.0729152);
   gre->SetPointError(1,0,0.02901494);
   gre->SetPoint(2,155,-0.05432687);
   gre->SetPointError(2,0,0.02886309);
   gre->SetPoint(3,165,-0.0654644);
   gre->SetPointError(3,0,0.02529287);
   gre->SetPoint(4,175,-0.05849973);
   gre->SetPointError(4,0,0.02552504);
   gre->SetPoint(5,185,-0.06946853);
   gre->SetPointError(5,0,0.0216437);
   gre->SetPoint(6,195,-0.02726882);
   gre->SetPointError(6,0,0.01833269);
   gre->SetPoint(7,205,-0.08438312);
   gre->SetPointError(7,0,0.01373157);
   gre->SetPoint(8,215,-0.06197271);
   gre->SetPointError(8,0,0.01153766);
      gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_11->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_3
   pad1_3 = new TPad("pad1_3", "pad1_3",0.656944,0.737474,0.96875,0.974948);
   pad1_3->Draw();
   pad1_3->cd();
   pad1_3->Range(120,-3,288.421,0.30103);
   pad1_3->SetFillColor(10);
   pad1_3->SetBorderMode(0);
   pad1_3->SetBorderSize(2);
   pad1_3->SetLogy();
   pad1_3->SetTickx();
   pad1_3->SetTicky();
   pad1_3->SetLeftMargin(0);
   pad1_3->SetRightMargin(0.05);
   pad1_3->SetTopMargin(0);
   pad1_3->SetBottomMargin(0);
   pad1_3->SetFrameFillColor(0);
   
   TH1 *hisCro = new TH2D("hisCro","",80,140,220,2000,1,2000);
   hisCro->SetStats(0);
   hisCro->GetXaxis()->CenterTitle(true);
   hisCro->GetXaxis()->SetNdivisions(207);
   hisCro->GetXaxis()->SetLabelOffset(0.03);
   hisCro->GetXaxis()->SetLabelSize(0.08);
   hisCro->GetXaxis()->SetTitleOffset(0.9);
   hisCro->GetYaxis()->CenterTitle(true);
   hisCro->GetYaxis()->SetLabelOffset(0.03);
   hisCro->GetYaxis()->SetLabelSize(0.08);
   hisCro->GetYaxis()->SetTitleOffset(0.9);
   hisCro->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_Cro_47");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,0.0256324*1000);
   gre->SetPointError(0,0,0.00112537);
   gre->SetPoint(1,150+5,0.0249901*1000);
   gre->SetPointError(1,0,0.00111428);
   gre->SetPoint(2,160+5,0.0226475*1000);
   gre->SetPointError(2,0,0.00105814);
   gre->SetPoint(3,170+5,0.0212811*1000);
   gre->SetPointError(3,0,0.00103821);
   gre->SetPoint(4,180+5,0.022339*1000);
   gre->SetPointError(4,0,0.00105444);
   gre->SetPoint(5,190+5,0.0228887*1000);
   gre->SetPointError(5,0,0.00106851);
   gre->SetPoint(6,200+5,0.0249768*1000);
   gre->SetPointError(6,0,0.00110812);
   gre->SetPoint(7,210+5,0.03261*1000);
   gre->SetPointError(7,0,0.00127333);
   gre->Draw("sp");
   /*TGraphErrors *gren = new TGraphErrors(10);
   gren->SetName("gr_crsection_28_28_140");
   gren->SetTitle("");
   gren->SetFillColor(1);
   gren->SetLineColor(2);
   gren->SetMarkerColor(2);
   gren->SetMarkerStyle(26);
   gren->SetPoint(0,136.2941,25.11801);
   gren->SetPointError(0,0,1.449628);
   gren->SetPoint(1,145.3929,26.37971);
   gren->SetPointError(1,0,1.535777);
   gren->SetPoint(2,154.4916,27.90703);
   gren->SetPointError(2,0,1.559732);
   gren->SetPoint(3,163.4833,27.57501);
   gren->SetPointError(3,0,1.568161);
   gren->SetPoint(4,172.5821,25.11801);
   gren->SetPointError(4,0,1.481231);
   gren->SetPoint(5,181.6809,21.93054);
   gren->SetPointError(5,0,1.343825);
   gren->SetPoint(6,190.7796,26.3133);
   gren->SetPointError(6,0,1.477963);
   gren->SetPoint(7,199.7713,27.37579);
   gren->SetPointError(7,0,1.442701);
   gren->SetPoint(8,208.8701,29.9656);
   gren->SetPointError(8,0,1.578035);
   gren->SetPoint(9,217.9688,43.91074);
   gren->SetPointError(9,0,2.135677);   
   gren->Draw("samep");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_crsection_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,136.485,31.45311);
   gre->SetPointError(0,0,1.429731);
//   gre->SetPoint(1,145.795,-1290.418);
 //  gre->SetPointError(1,0,-65.89886);
   gre->SetPoint(2,155.105,30.03345);
   gre->SetPointError(2,0,1.599087);
   gre->SetPoint(3,164.415,29.54578);
   gre->SetPointError(3,0,1.569214);
   gre->SetPoint(4,173.725,29.85582);
   gre->SetPointError(4,0,1.528482);
   gre->SetPoint(5,183.035,26.30088);
   gre->SetPointError(5,0,1.373901);
   gre->SetPoint(6,192.345,30.71858);
   gre->SetPointError(6,0,1.312893);
   gre->SetPoint(7,201.655,23.79011);
   gre->SetPointError(7,0,1.275989);
   gre->SetPoint(8,210.965,24.29601);
   gre->SetPointError(8,0,1.498719);
   gre->SetPoint(9,220.275,29.20599);
   gre->SetPointError(9,0,1.433883);
   gre->Draw("sp");*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_crsection_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,25.42819);
   gre->SetPointError(1,0,3.038373);
   gre->SetPoint(2,155,25.42844);
   gre->SetPointError(2,0,0.6544242);
   gre->SetPoint(3,165,23.98219);
   gre->SetPointError(3,0,0.6375922);
   gre->SetPoint(4,175,23.00014);
   gre->SetPointError(4,0,0.6147124);
   gre->SetPoint(5,185,22.61975);
   gre->SetPointError(5,0,0.5988189);
   gre->SetPoint(6,195,27.52257);
   gre->SetPointError(6,0,0.6047161);
//   gre->SetPoint(7,205,223.586);
//   gre->SetPointError(7,0,5.559403);
   gre->SetPoint(8,215,29.77393);
   gre->SetPointError(8,0,0.7396229);
  // gre->SetPoint(9,225,41.26139);
  // gre->SetPointError(9,0,0.7190267);
   gre->Draw("sp");
   TF1 *phspSC_28_28_140 = new TF1("phspSC_28_28_140","pol2",13,280);
   phspSC_28_28_140->SetFillColor(19);
   phspSC_28_28_140->SetFillStyle(0);
   phspSC_28_28_140->SetLineWidth(2);
   phspSC_28_28_140->SetParameter(0,-0.165351);
   phspSC_28_28_140->SetParError(0,0);
   phspSC_28_28_140->SetParLimits(0,0,0);
   phspSC_28_28_140->SetParameter(1,0.00268979);
   phspSC_28_28_140->SetParError(1,0);
   phspSC_28_28_140->SetParLimits(1,0,0);
   phspSC_28_28_140->SetParameter(2,-7.06519e-06);
   phspSC_28_28_140->SetParError(2,0);
   phspSC_28_28_140->SetParLimits(2,0,0);
   phspSC_28_28_140->Draw("same");
   pad1_3->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_6
   pad1_6 = new TPad("pad1_6", "pad1_6",0.656944,0.5,0.96875,0.737474);
   pad1_6->Draw();
   pad1_6->cd();
   pad1_6->Range(120,-0.57,288.421,0.57);
   pad1_6->SetFillColor(10);
   pad1_6->SetBorderMode(0);
   pad1_6->SetBorderSize(2);
   pad1_6->SetTickx();
   pad1_6->SetTicky();
   pad1_6->SetLeftMargin(0);
   pad1_6->SetRightMargin(0.05);
   pad1_6->SetTopMargin(0);
   pad1_6->SetBottomMargin(0);
   pad1_6->SetFrameFillColor(0);
   
   TH1 *hisT11 = new TH2D("hisT11","",80,140,220,20,-0.57,0.57);
   hisT11->SetStats(0);
   hisT11->GetXaxis()->CenterTitle(true);
   hisT11->GetXaxis()->SetNdivisions(207);
   hisT11->GetXaxis()->SetLabelOffset(0.03);
   hisT11->GetXaxis()->SetLabelSize(0.08);
   hisT11->GetXaxis()->SetTitleOffset(0.9);
   hisT11->GetYaxis()->CenterTitle(true);
   hisT11->GetYaxis()->SetLabelOffset(0.03);
   hisT11->GetYaxis()->SetLabelSize(0.08);
   hisT11->GetYaxis()->SetTitleOffset(0.9);
   hisT11->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T11_47");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.147805);
   gre->SetPointError(0,0,0.0377358);
   gre->SetPoint(1,150+5,-0.173257);
   gre->SetPointError(1,0,0.0404309);
   gre->SetPoint(2,160+5,-0.16989);
   gre->SetPointError(2,0,0.0388751);
   gre->SetPoint(3,170+5,-0.131969);
   gre->SetPointError(3,0,0.0407905);
   gre->SetPoint(4,180+5,-0.135031);
   gre->SetPointError(4,0,0.0447718);
   gre->SetPoint(5,190+5,-0.168726);
   gre->SetPointError(5,0,0.0413152);
   gre->SetPoint(6,200+5,-0.140363);
   gre->SetPointError(6,0,0.0407254);
   gre->SetPoint(7,210+5,-0.183545);
   gre->SetPointError(7,0,0.0386632);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,136.3655,-0.1618737);
   gre->SetPointError(0,0,0.06820598);
   gre->SetPoint(1,145.4365,-0.1933687);
   gre->SetPointError(1,0,0.06412827);
   gre->SetPoint(2,154.5075,0.05427211);
   gre->SetPointError(2,0,0.06389271);
   gre->SetPoint(3,163.5785,-0.1485784);
   gre->SetPointError(3,0,0.07570329);
   gre->SetPoint(4,172.6495,-0.1712523);
   gre->SetPointError(4,0,0.07261428);
   gre->SetPoint(5,181.7205,-0.2363874);
   gre->SetPointError(5,0,0.07260121);
   gre->SetPoint(6,190.7915,-0.1180157);
   gre->SetPointError(6,0,0.07833056);
   gre->SetPoint(7,199.8625,-0.1273984);
   gre->SetPointError(7,0,0.08505982);
   gre->SetPoint(8,208.9335,-0.220977);
   gre->SetPointError(8,0,0.07801601);
   gre->SetPoint(9,218.0045,-0.06383592);
   gre->SetPointError(9,0,0.06319762);   
      gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_iT11_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,136.3655,-0.1691016);
   gre->SetPointError(0,0,0.06331858);
   gre->SetPoint(1,145.4365,-0.2019357);
   gre->SetPointError(1,0,0.05782705);
   gre->SetPoint(2,154.5075,0.05293762);
   gre->SetPointError(2,0,0.05653225);
   gre->SetPoint(3,163.5785,-0.1755965);
   gre->SetPointError(3,0,0.06122014);
   gre->SetPoint(4,172.6495,-0.178615);
   gre->SetPointError(4,0,0.0655131);
   gre->SetPoint(5,181.7205,-0.2443503);
   gre->SetPointError(5,0,0.06883552);
   gre->SetPoint(6,190.7915,-0.130076);
   gre->SetPointError(6,0,0.06787026);
   gre->SetPoint(7,199.8625,-0.1314611);
   gre->SetPointError(7,0,0.0801268);
   gre->SetPoint(8,208.9335,-0.2332475);
   gre->SetPointError(8,0,0.06727623);
   gre->SetPoint(9,218.0045,-0.06303528);
   gre->SetPointError(9,0,0.0578481);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_iT11_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.1211596);
   gre->SetPointError(1,0,0.03404485);
   gre->SetPoint(2,155,-0.0979608);
   gre->SetPointError(2,0,0.03093037);
   gre->SetPoint(3,165,-0.07123552);
   gre->SetPointError(3,0,0.03177124);
   gre->SetPoint(4,175,-0.05595794);
   gre->SetPointError(4,0,0.03383073);
   gre->SetPoint(5,185,-0.1494934);
   gre->SetPointError(5,0,0.03520663);
   gre->SetPoint(6,195,-0.1255653);
   gre->SetPointError(6,0,0.03580132);
   gre->SetPoint(7,205,-0.1712487);
   gre->SetPointError(7,0,0.03329344);
   gre->SetPoint(8,215,-0.1354664);
   gre->SetPointError(8,0,0.02797513);
      gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_6->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_9
   pad1_9 = new TPad("pad1_9", "pad1_9",0.656944,0.262526,0.96875,0.5);
   pad1_9->Draw();
   pad1_9->cd();
   pad1_9->Range(120,-0.57,288.421,0.57);
   pad1_9->SetFillColor(10);
   pad1_9->SetBorderMode(0);
   pad1_9->SetBorderSize(2);
   pad1_9->SetTickx();
   pad1_9->SetTicky();
   pad1_9->SetLeftMargin(0);
   pad1_9->SetRightMargin(0.05);
   pad1_9->SetTopMargin(0);
   pad1_9->SetBottomMargin(0);
   pad1_9->SetFrameFillColor(0);
   
   TH1 *hisT20 = new TH2D("hisT20","",80,140,220,20,-0.57,0.57);
   hisT20->SetStats(0);
   hisT20->GetXaxis()->CenterTitle(true);
   hisT20->GetXaxis()->SetNdivisions(207);
   hisT20->GetXaxis()->SetLabelOffset(0.03);
   hisT20->GetXaxis()->SetLabelSize(0.08);
   hisT20->GetXaxis()->SetTitleOffset(0.9);
   hisT20->GetYaxis()->CenterTitle(true);
   hisT20->GetYaxis()->SetLabelOffset(0.03);
   hisT20->GetYaxis()->SetLabelSize(0.08);
   hisT20->GetYaxis()->SetTitleOffset(0.9);
   hisT20->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T20_47");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,0.0605567);
   gre->SetPointError(0,0,0.0590635);
   gre->SetPoint(1,150+5,0.00817709);
   gre->SetPointError(1,0,0.060136);
   gre->SetPoint(2,160+5,-0.131463);
   gre->SetPointError(2,0,0.0574728);
   gre->SetPoint(3,170+5,-0.068253);
   gre->SetPointError(3,0,0.0630354);
   gre->SetPoint(4,180+5,-0.214373);
   gre->SetPointError(4,0,0.0592313);
   gre->SetPoint(5,190+5,-0.396532);
   gre->SetPointError(5,0,0.0523288);
   gre->SetPoint(6,200+5,-0.317397);
   gre->SetPointError(6,0,0.0529972);
   gre->SetPoint(7,210+5,-0.272215);
   gre->SetPointError(7,0,0.0498408);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,136.3655,-0.0655205);
   gre->SetPointError(0,0,0.09743698);
   gre->SetPoint(1,145.4365,-0.08152334);
   gre->SetPointError(1,0,0.09140149);
   gre->SetPoint(2,154.5075,-0.06813578);
   gre->SetPointError(2,0,0.09326746);
   gre->SetPoint(3,163.5785,-0.1599732);
   gre->SetPointError(3,0,0.09454672);
   gre->SetPoint(4,172.6495,-0.1607049);
   gre->SetPointError(4,0,0.101063);
   gre->SetPoint(5,181.7205,-0.12003);
   gre->SetPointError(5,0,0.1096395);
   gre->SetPoint(6,190.7915,-0.2231384);
   gre->SetPointError(6,0,0.1034074);
   gre->SetPoint(7,199.8625,-0.1686701);
   gre->SetPointError(7,0,0.1054682);
   gre->SetPoint(8,208.9335,-0.3102512);
   gre->SetPointError(8,0,0.09907677);
   gre->SetPoint(9,218.0045,-0.2403444);
   gre->SetPointError(9,0,0.08709202);   
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T20_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,136.3655,-0.1033324);
   gre->SetPointError(0,0,0.09538871);
   gre->SetPoint(1,145.4365,-0.131734);
   gre->SetPointError(1,0,0.08882664);
   gre->SetPoint(2,154.5075,-0.1091294);
   gre->SetPointError(2,0,0.09114109);
   gre->SetPoint(3,163.5785,-0.2828177);
   gre->SetPointError(3,0,0.08773026);
   gre->SetPoint(4,172.6495,-0.2819645);
   gre->SetPointError(4,0,0.0938677);
   gre->SetPoint(5,181.7205,-0.2088392);
   gre->SetPointError(5,0,0.104056);
   gre->SetPoint(6,190.7915,-0.3958883);
   gre->SetPointError(6,0,0.0925198);
   gre->SetPoint(7,199.8625,-0.2958306);
   gre->SetPointError(7,0,0.09755654);
   gre->SetPoint(8,208.9335,-0.5502777);
   gre->SetPointError(8,0,0.08376887);
   gre->SetPoint(9,218.0045,-0.4261572);
   gre->SetPointError(9,0,0.07712418);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T20_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,0.0828188);
   gre->SetPointError(1,0,0.05155439);
   gre->SetPoint(2,155,0.02529497);
   gre->SetPointError(2,0,0.04764532);
   gre->SetPoint(3,165,0.03409132);
   gre->SetPointError(3,0,0.04881125);
   gre->SetPoint(4,175,0.0278299);
   gre->SetPointError(4,0,0.05090699);
   gre->SetPoint(5,185,-0.1044757);
   gre->SetPointError(5,0,0.05054896);
   gre->SetPoint(6,195,-0.1484299);
   gre->SetPointError(6,0,0.04982524);
   gre->SetPoint(7,205,-0.3215472);
   gre->SetPointError(7,0,0.04300286);
   gre->SetPoint(8,215,-0.210867);
   gre->SetPointError(8,0,0.03564083);
      gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_9->Modified();
   pad1->cd();
  
// ------------>Primitives in pad: pad1_12
   pad1_12 = new TPad("pad1_12", "pad1_12",0.656944,0,0.96875,0.262526);
   pad1_12->Draw();
   pad1_12->cd();
   pad1_12->Range(120,-0.855,288.421,0.57);
   pad1_12->SetFillColor(10);
   pad1_12->SetBorderMode(0);
   pad1_12->SetBorderSize(2);
   pad1_12->SetTickx();
   pad1_12->SetTicky();
   pad1_12->SetLeftMargin(0);
   pad1_12->SetRightMargin(0.05);
   pad1_12->SetTopMargin(0);
   pad1_12->SetBottomMargin(0.2);
   pad1_12->SetFrameFillColor(0);
   
   TH1 *hisT22 = new TH2D("hisT22","",80,140,220,20,-0.57,0.57);
   hisT22->SetStats(0);
   hisT22->GetXaxis()->CenterTitle(true);
   hisT22->GetXaxis()->SetNdivisions(207);
   hisT22->GetXaxis()->SetLabelOffset(0.03);
   hisT22->GetXaxis()->SetLabelSize(0.08);
   hisT22->GetXaxis()->SetTitleOffset(0.9);
   hisT22->GetYaxis()->CenterTitle(true);
   hisT22->GetYaxis()->SetLabelOffset(0.03);
   hisT22->GetYaxis()->SetLabelSize(0.08);
   hisT22->GetYaxis()->SetTitleOffset(0.9);
   hisT22->Draw("");
   
   gre = new TGraphErrors(8);
   gre->SetName("gr_T22_47");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineStyle(2);
   gre->SetMarkerStyle(20);
   gre->SetMarkerSize(0.9);
   gre->SetPoint(0,140+5,-0.0279354);
   gre->SetPointError(0,0,0.0341343);
   gre->SetPoint(1,150+5,0.066652);
   gre->SetPointError(1,0,0.0348742);
   gre->SetPoint(2,160+5,-0.0182454);
   gre->SetPointError(2,0,0.0328016);
   gre->SetPoint(3,170+5,-0.0419145);
   gre->SetPointError(3,0,0.0352337);
   gre->SetPoint(4,180+5,0.0935646);
   gre->SetPointError(4,0,0.0343094);
   gre->SetPoint(5,190+5,0.0570998);
   gre->SetPointError(5,0,0.0305675);
   gre->SetPoint(6,200+5,0.0423424);
   gre->SetPointError(6,0,0.030868);
   gre->SetPoint(7,210+5,0.0141065);
   gre->SetPointError(7,0,0.0305022);
   gre->Draw("sp");
   /*TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(5);
   gre->SetMarkerColor(5);
   gre->SetMarkerStyle(20);
   gre->SetPoint(0,136.3655,-0.03545324);
   gre->SetPointError(0,0,0.05859504);
   gre->SetPoint(1,145.4365,-0.09542624);
   gre->SetPointError(1,0,0.05256657);
   gre->SetPoint(2,154.5075,-0.003037036);
   gre->SetPointError(2,0,0.05593567);
   gre->SetPoint(3,163.5785,0.05854312);
   gre->SetPointError(3,0,0.05295478);
   gre->SetPoint(4,172.6495,0.1044504);
   gre->SetPointError(4,0,0.05935642);
   gre->SetPoint(5,181.7205,0.0319732);
   gre->SetPointError(5,0,0.06343961);
   gre->SetPoint(6,190.7915,0.09400741);
   gre->SetPointError(6,0,0.05915557);
   gre->SetPoint(7,199.8625,0.109373);
   gre->SetPointError(7,0,0.05684901);
   gre->SetPoint(8,208.9335,0.01698445);
   gre->SetPointError(8,0,0.05705383);
   gre->SetPoint(9,218.0045,-0.08598343);
   gre->SetPointError(9,0,0.04925696);   
   gre->Draw("sp");
   TGraphErrors *gre = new TGraphErrors(10);
   gre->SetName("gr_T22_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(6);
   gre->SetMarkerColor(6);
   gre->SetMarkerStyle(23);
   gre->SetPoint(0,136.3655,-0.03470708);
   gre->SetPointError(0,0,0.05736328);
   gre->SetPoint(1,145.4365,-0.09273828);
   gre->SetPointError(1,0,0.05108573);
   gre->SetPoint(2,154.5075,-0.002964242);
   gre->SetPointError(2,0,0.05466041);
   gre->SetPoint(3,163.5785,0.05432301);
   gre->SetPointError(3,0,0.04913694);
   gre->SetPoint(4,172.6495,0.09701393);
   gre->SetPointError(4,0,0.05513045);
   gre->SetPoint(5,181.7205,0.03035298);
   gre->SetPointError(5,0,0.06020887);
   gre->SetPoint(6,190.7915,0.08410955);
   gre->SetPointError(6,0,0.05292719);
   gre->SetPoint(7,199.8625,0.1011704);
   gre->SetPointError(7,0,0.05258449);
   gre->SetPoint(8,208.9335,0.01437962);
   gre->SetPointError(8,0,0.04823871);
   gre->SetPoint(9,218.0045,-0.07614735);
   gre->SetPointError(9,0,0.04361942);*/
   TGraphErrors *gre = new TGraphErrors(15);
   gre->SetName("gr_T22_28_28_140");
   gre->SetTitle("");
   gre->SetFillColor(1);
   gre->SetLineColor(2);
   gre->SetMarkerColor(2);
   gre->SetMarkerStyle(26);
   gre->SetPoint(1,145,-0.04851397);
   gre->SetPointError(1,0,0.03042898);
   gre->SetPoint(2,155,0.01190616);
   gre->SetPointError(2,0,0.02776582);
   gre->SetPoint(3,165,0.005918564);
   gre->SetPointError(3,0,0.02902642);
   gre->SetPoint(4,175,0.02078005);
   gre->SetPointError(4,0,0.02981337);
   gre->SetPoint(5,185,0.06575205);
   gre->SetPointError(5,0,0.0289712);
   gre->SetPoint(6,195,0.05847273);
   gre->SetPointError(6,0,0.02879342);
   gre->SetPoint(7,205,0.05252368);
   gre->SetPointError(7,0,0.02550452);
   gre->SetPoint(8,215,-0.004430727);
   gre->SetPointError(8,0,0.02032637);
   gre->Draw("sp");
   TF1 *line = new TF1("line","0*x",120,280);
   line->SetFillColor(19);
   line->SetFillStyle(0);
   line->SetLineColor(14);
   line->SetLineWidth(1);
   line->Draw("SAME");
   pad1_12->Modified();
   pad1->cd();
   pad1->Modified();
   canvas_15->cd();
   canvas_15->Modified();
   canvas_15->cd();
   canvas_15->SetSelected(canvas_15);
}
