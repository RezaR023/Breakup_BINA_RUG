...
TRandom *xran = new TRandom();
   ...
   ////cut for selection withut hardonic interaction
   TCutG *cutgt = new TCutG("cutgt",14);
   cutgt->SetVarX("");
   cutgt->SetVarY("");
   cutgt->SetTitle("Graph");
   cutgt->SetFillColor(1);
   cutgt->SetPoint(0,0.124617,-6.59537);
   cutgt->SetPoint(1,7.4237,-2.31323);
   cutgt->SetPoint(2,15.0788,-0.172162);
   cutgt->SetPoint(3,27.3627,0.);
   cutgt->SetPoint(4,54.6007,0.);
   cutgt->SetPoint(5,70.6231,0.0);
   cutgt->SetPoint(6,85.3159,0.037417);
   cutgt->SetPoint(7,90.1907,.45522);
   cutgt->SetPoint(8,90.0809,-1.12375);
   cutgt->SetPoint(9,87.9181,-2.22692);
   cutgt->SetPoint(10,18.6394,-3.38376);
   cutgt->SetPoint(11,10.9842,-4.6922);
   cutgt->SetPoint(12,0.658697,-12.067);
   cutgt->SetPoint(13,0.124617,-6.59537);
   /////more restrict cut based on Phi12==140 for diff configs
   cutgg = new TCutG("cutgg",14);
   //  cutgg->SetVarX("");
   //cutgg->SetVarY("");
   cutgg->SetTitle("Graph");
   cutgg->SetFillColor(1);
   cutgg->SetPoint(0,1.01446,-8.00555);
   cutgg->SetPoint(1,5.56203,-5.23193);
   cutgg->SetPoint(2,13.2579,-2.99647);
   cutgg->SetPoint(3,22.8778,-1.96153);
   cutgg->SetPoint(4,66.4296,-1.92013);
   cutgg->SetPoint(5,69.9277,-1.83734);
   cutgg->SetPoint(6,69.9277,-0.140044);
   cutgg->SetPoint(7,61.5322,0.0255452);
   cutgg->SetPoint(8,21.8284,-0.222839);
   cutgg->SetPoint(9,9.75979,-1.54756);
   cutgg->SetPoint(10,4.86241,-3.36904);
   cutgg->SetPoint(11,1.36427,-5.06634);
   cutgg->SetPoint(12,0.489738,-7.79857);
   cutgg->SetPoint(13,1.01446,-8.00555);
...
//double charge=1640492.0;
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaeUsingCutNbin9UsingTOF_MWTOF1_Snr5.root","recreate");
  fitfunctions();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
		...
		if (MMN>939.2 /*&& MMN<939.8*/) ET[q]->Fill(b3_EFW1,deltTOF2);
		if (MMN>939.2 /*&& MMN<939.8*/ /*&& cutgg->IsInside(b3_EFW1,deltTOF2)*/ && Dtarg0<70){
		...
		/////////////////////////////////////if (deltTOF2<1.3 && cutg->IsInside(b3_EFW1,deltTOF2)){   
		  ...
		  Double_t eep = 5e-1*Dtarg/1000;//GetEnergyResolutionNeutron(pProton);
		  ...
		  for(int j=0 ; j<Snr ; j++)
		    {
		      if (cut[q][j]->IsInside(Ptargn,Dtarg) && cutgg->IsInside(b3_EFW1,deltTOF2))
			{
			  ...
			  if(b3_Pol==31 && value1>-500/*-10/*6*/ && value1<500/*10/*8*/) 
			    {
			  ...
			  if(b3_Pol==27 && value1>-500/*-10/*6*/ && value1<500/*10/*8*/) 
			    {
			  ...
			  if(b3_Pol==13 && value1>-500/*-10/*6*/ && value1<500/*10/*8*/)
			    {//<<--pol13
...
int BookingHist(){
  ...
  for (int i=0;i<Nkinema;i++){
    ...
    ET[i] = new TH2D (name22[i],"",300,0,150,200,-50,50);
	   ...
	   for(int k=0 ; k<Nbin ; k++)
	     {
	       ...
	       ETpm31[i][j][k] = new TH1D (namETpm31[i][j][k],"",2*60,-10,20);
	       // ETrawpm31[i][j][k] = new TH2D (namETrawpm31[i][j][k],"",60,0,120,45,-15,30);
		ETrawpm31[i][j][k] = new TH1D (namETrawpm31[i][j][k],"",2*60,-10,20);
	       ...
	       ETpm27[i][j][k] = new TH1D (namETpm27[i][j][k],"",2*60,-10,20);
	       //ETrawpm27[i][j][k] = new TH2D (namETrawpm27[i][j][k],"",60,0,120,45,-15,30);//120,0,120,70,-20,50);
	       ETrawpm27[i][j][k] = new TH1D (namETrawpm27[i][j][k],"",2*60,-10,20);
	       ...
	       ETpm13[i][j][k] = new TH1D (namETpm13[i][j][k],"",2*60,-10,20);
	       //ETrawpm13[i][j][k] = new TH2D (namETrawpm13[i][j][k],"",60,0,120,45,-15,30);//120,0,120,70,-20,50);
	       ETrawpm13[i][j][k] = new TH1D (namETrawpm13[i][j][k],"",2*60,-10,20);
	     }
	 }
     }
