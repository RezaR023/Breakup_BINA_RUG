...
TFile *f1=new TFile("../vbrkupAnaPowfiWholeUsingCutNbin20_AhCal_dtta1n.root");
...
//double charge=1640492.0;
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeFixPar_AhCal_dtta1n_offset.root","recreate");
  fitfunctions();
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ qj=0;
	  ...
	  if ((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) 
		...
		if (1==1 && hEn[q][j]->GetMean()<.45/* && MinEnval[q]<0.15 */&& MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q])) )
		{
		  outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< PoCm_p[q][j]->GetMean()<< '\t' << PoCm_d[q][j]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<endl;
		}
	      jq++;
	    }
	}
