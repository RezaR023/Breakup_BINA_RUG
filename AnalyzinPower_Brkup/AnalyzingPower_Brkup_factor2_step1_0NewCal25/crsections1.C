...
//double charge=1640492.0;
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeUsingCutNbin20_dtta1_Dpm6.root","recreate");
  fitfunctions();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
		...
		if (1==1 && abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
		  ...
		  double P_Neut=1000*pNeutron0.P();
		  ...
		  //////////////////////////////////////////
		    ...
		    //		    TDC_ratio_phi[q]->Fill(b3_MWPCphi1,deltTOF2);
		    if (cutg->IsInside(b3_EFW1,deltTOF2)) MissingMassN00->Fill(P_Neut,MMN);
		    ///////cut when your first particle is deuteorn///
		    //+++    if ( 1==1 && cutgpm1->IsInside(b3_EFW1,deltTOF2))
		    if (deltTOF2<1.3 && cutg->IsInside(b3_EFW1,deltTOF2)/* && MMN>932 && MMN<947*/){ 
			  ...
			  if (cut[q][j]->IsInside(Ptarg,Dtarg))
			    {
			      ...
			      if(b3_Pol==31 && value1>-6/*-10/*6*/ && value1<6/*10/*8*/) 
				{
			      ...
			      if(b3_Pol==27 && value1>-6/*-10/*6*/ && value1<6/*10/*8*/) 
				{
			      ...
			      if(b3_Pol==13 && value1>-6/*-10/*6*/ && value1<6/*10/*8*/)
				{//<<--pol13
		...
		if (1==1 && abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
		  ...
		  //////////////////////////////////////////
		    ...
		    if (cutg->IsInside(b3_EFW2,deltTOF3)) MissingMassN00->Fill(P_Neut,MMN);
		    ///////cut when your second particle is deuteron//////
		    ////++++	    if ( 1==1 && cutgpm1->IsInside(b3_EFW2,deltTOF3))
		    if (deltTOF3<1.3 && cutg->IsInside(b3_EFW2,deltTOF3)/* && MMN>932 && MMN<947*/){ 
		      /**************
...
int Define_GCuts(){
  ...
  for (int k=0 ; k<Nkinema ; k++){ds=0.;
	...
	if (Scu[k][i]>tmps21 && Scu[k][i]<(s2+ds/10)){
      ...
      sprintf(cha[0],"cut_%d_%d_%d_%d",(int)tta_1[k],(int)tta_2[k],(int)phi12[k],count);
