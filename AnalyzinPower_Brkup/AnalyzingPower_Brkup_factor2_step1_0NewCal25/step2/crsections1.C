...
TFile *f1=new TFile("../vbrkupAnaPowfiWholeUsingCutNbin20_dtta1_Dpm6.root");
...
//double charge=1640492.0;
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeFixPar_dtta1n_dot45En_offset_Dpm6.root","recreate");
  fitfunctions();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
{
 ...
 int1.close();
 charge31=((II31+.28)/II31)*charge31;
 charge27=((II27+.28)/II27)*charge27;
 charge13=((II13+.28)/II13)*charge13;
 cout << "charge31= " << charge31 << '\t' << charge27 << '\t' << charge13 << endl;
   ...
   TF1 *function1pi=new TF1("function1pi","[0]+[1]*sqrt(3)*cos((x+180)*0.01745)",0,360);
   ...
   TF1 *function2pi=new TF1("function2pi","[0]+[1]*0.0- [2]*(1./sqrt(8))-[3]*sqrt(3)*.5*cos(2.*(x+180)*0.01745)",0,360);
   ...
   TF1 *functionsumi=new TF1("functionsumi","1+[1]*sqrt(3)*cos((x+180)*0.01745)- [2]*(1./sqrt(8))-[3]*sqrt(3)*.5*cos(2.*(x+180)*0.01745)",0,360);
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ qj=0;
	  ...
	  //////////////////////////////////////////////////
	  /*
	  functionsum->SetParameters(1,0.1);
	  ratiosum27[q][j]->Fit(functionsum,"Q");
	  double p0p=functionsum->GetParameter(0);
	  double p1p=functionsum->GetParameter(1);
	  double Ep1p=functionsum->GetParError(1);
	  */
	  /////////////////////////////////////////////////
	  ...
	  ////////////////////////////////////////////////
	  /*
	  functionsumi->SetParameters(1,0.0,0.04,0.01);
	  ratiosum13[q][j]->Fit(functionsumi,"Q");
	  double p20p=functionsumi->GetParameter(0);
	  double p2p=functionsumi->GetParameter(2);
	  double p3p=functionsumi->GetParameter(3);
	  double Ep2p=functionsumi->GetParError(2);
	  double Ep3p=functionsumi->GetParError(3);
	  */
	  ///////////////////////////////////////////////
	  ...
	  if ((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) 
		...
		{
		  outch1 << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< PoCm_p[q][j]->GetMean()<< '\t' << PoCm_d[q][j]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j <<endl;
		}
		if (1==1 && hEn[q][j]->GetMean()<1/* && MinEnval[q]<0.15 */&& MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q])) )
		{
		  outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<endl;
		}
	      jq++;
	    }
	}
  ...
  for(int q=0 ; q<Nkinema; q++)
    {  
      for(int j=0 ; j<Snr ; j++)
	{
	  if (((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) && (MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+((j)*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q]))) )//|| (MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q]))) || (MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q]))))
	    {
	      if (hEn[q][j-1]->GetMean()<EnMin && (Smin1[q]+(dss[q]/2)+((j-1)*dss[q]))>Smin2[q] && hEn[q][j-1]->GetMean()==MINv(hEn[q][j-1]->GetMean(),hEn[q][j]->GetMean()) && hEn[q][j-1]->GetMean()==MINv(hEn[q][j-1]->GetMean(),hEn[q][j+1]->GetMean()))
		outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+((j-1)*dss[q]) <<'\t'<< PoCm_p[q][j-1]->GetMean()<< '\t' << PoCm_d[q][j-1]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j-1]->GetMean() <<'\t'<< iT11[q][j-1]<<'\t' << EiT11[q][j-1] <<'\t' << T20[q][j-1] << '\t' << ET20[q][j-1] <<'\t' << T22[q][j-1] <<'\t' << ET22[q][j-1] <<'\t' << hEn[q][j-1]->GetMean()<< '\t'  << j-1 << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<'\t' << 1 <<endl;
	      else if (hEn[q][j+1]->GetMean()<EnMin && (Smin1[q]+(dss[q]/2)+((j+1)*dss[q]))<Smax2[q] && hEn[q][j+1]->GetMean()==MINv(hEn[q][j+1]->GetMean(),hEn[q][j]->GetMean()) && hEn[q][j+1]->GetMean()==MINv(hEn[q][j-1]->GetMean(),hEn[q][j+1]->GetMean()))
		outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+((j+1)*dss[q]) <<'\t'<< PoCm_p[q][j+1]->GetMean()<< '\t' << PoCm_d[q][j+1]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j+1]->GetMean() <<'\t'<< iT11[q][j+1]<<'\t' << EiT11[q][j+1] <<'\t' << T20[q][j+1] << '\t' << ET20[q][j+1] <<'\t' << T22[q][j+1] <<'\t' << ET22[q][j+1] <<'\t' << hEn[q][j+1]->GetMean()<< '\t'  << j+1 << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<'\t' << 2 <<endl;
	      else if (hEn[q][j]->GetMean()<EnMin && hEn[q][j]->GetMean()==MINv(hEn[q][j+1]->GetMean(),hEn[q][j]->GetMean()) && hEn[q][j]->GetMean()==MINv(hEn[q][j-1]->GetMean(),hEn[q][j]->GetMean()))
		{
		  outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< PoCm_p[q][j]->GetMean()<< '\t' << PoCm_d[q][j]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<'\t' << 3 <<endl;
		}
	    }
	}
    }*/
