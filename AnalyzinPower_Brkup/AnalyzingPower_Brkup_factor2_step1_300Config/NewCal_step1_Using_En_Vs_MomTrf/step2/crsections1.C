...
double MinEnSval[Nkinema]={0.};
double MinEnval[Nkinema];
...
//TFile *f1=new TFile("../vbrkupAnaPowfiWholeUsingCutNbin20_newCal25_100n_missMassn.root");
TFile *f1=new TFile("../vbrkupAnaPowNbin20_newCal25_missMassConstr_F1.root");
...
//double charge=1640492.0;
 ...
 double iT11n[20][4]={0.},EiT11n[20][4]={0.},T20n[20][4]={0.},T20nn[20][4]={0.},ET20n[20][4]={0.},T22n[20][4]={0.},ET22n[20][4]={0.};
  double Ayy[Nkinema][Snr], EAyy[Nkinema][Snr];
  double Ay[Nkinema][Snr],EAy[Nkinema][Snr];
  double Azz[Nkinema][Snr], EAzz[Nkinema][Snr];
  TFile *f = new TFile("vbrkupAnaPowfiWholeFixPar3_newCal25_100_QFSdata_F1_final.root","recreate");
  fitfunctions();
  TPostScript *ps = new TPostScript(psfile0,111);
  cc=new TCanvas("cc","");
  cc->Divide(4,3);
  ofstream outch,outch_0;
  outch.open(QFS_res);
if(outch)
    {
...
if(outch0)
  ...
  if (fChain == 0) return;
  Long64_t nentries =1.;// fChain->GetEntriesFast();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
{
 ...
 //charge31=2102641.442860676;//2215992.348253;//2082035.135253;//ofset .28 pA,.5 pa, .24 pA
 //charge27=2048011.648117252;//2158772.5350156;//2027879.2543156;//ofset .28 pA,.5 pa, .24 pA
 //charge13=1400735.420351998;//1513471.2630413;//1380235.0391413;//ofset .28 pA,.5 pa, .24 pA
 charge31=2169619.969903132;	charge27=2113458.127600262;	charge13=1467353.651229712;//ofset .41 pA
   ...
   TF1 *function2p0=new TF1("function2p0","([0]+[1]*0.0- [2]*(1./sqrt(8))-[3]*sqrt(3)*.5*cos(2.*(x+180)*0.01745))",0,360);
   //   TF1 *funcT20=new TF1("funcT20","2*M_PI*[0]([1]*1+sqrt(2)*0.25*[2])",0,360);
...
for (int as=0;as<4;as++)
  {
    for (int lj=0;lj<20;lj++)
      {
	for(int k=0;k<Phiraw31pm0[lj][as]->GetNbinsX();k++)
	  {
	    double val=Phiraw31pm0[lj][as]->GetBinContent(k);
	    double error=Phiraw31pm0[lj][as]->GetBinError(k);
	    if(val>0)
	      {
		double newerror=(error/val)*Phi31pm0[lj][as]->GetBinContent(k);
		Phi31pm0[lj][as]->SetBinError(k,newerror);
	      }
	  } 
	for(int k=0;k<Phiraw27pm0[lj][as]->GetNbinsX();k++)
	  {
	    double val=Phiraw27pm0[lj][as]->GetBinContent(k);
	    double error=Phiraw27pm0[lj][as]->GetBinError(k);
	    if(val>0)
	      {
		double newerror=(error/val)*Phi27pm0[lj][as]->GetBinContent(k);
		Phi27pm0[lj][as]->SetBinError(k,newerror);
	      }
	  } 
      	for(int k=0;k<Phiraw13pm0[lj][as]->GetNbinsX();k++)
	  {
	    double val=Phiraw13pm0[lj][as]->GetBinContent(k);
	    double error=Phiraw13pm0[lj][as]->GetBinError(k);
	    if(val>0)
	      {
		double newerror=(error/val)*Phi13pm0[lj][as]->GetBinContent(k);
		Phi13pm0[lj][as]->SetBinError(k,newerror);
	      }
	  }
	Phi31pm0[lj][as]->Scale(1./charge31);
	Phi27pm0[lj][as]->Scale(1./charge27);
	Phi13pm0[lj][as]->Scale(1./(charge13));
	Integ13[lj][as]=(double) Phi13pm0[lj][as]->Integral(0,Nbin);
	Integ31[lj][as]=(double) Phi31pm0[lj][as]->Integral(0,Nbin);
	ratiosum270[lj][as]->Divide(Phi27pm0[lj][as],Phi31pm0[lj][as],1,1);
	function1p->SetParameters(1,0.1);
	function1p->FixParameter(0,1);
	ratiosum270[lj][as]->Fit(function1p,"Q");
	double p0pn=function1p->GetParameter(0);
	double p1pn=function1p->GetParameter(1);
	double Ep1pn=function1p->GetParError(1);

	ratiosum130[lj][as]->Divide(Phi13pm0[lj][as],Phi31pm0[lj][as],1,1);
	///preRatio[lj][as]=ratiosum130[lj][as]->Integral(0,Nbin);
	function2p->SetParameters(1,0.0,0.04,0.01);
	function2p->FixParameter(0,1);
	function2p->FixParameter(1,0);
	ratiosum130[lj][as]->Fit(function2p,"Q");
	double p20pn=function2p->GetParameter(0);
	double p2pn=function2p->GetParameter(2);
	double p3pn=function2p->GetParameter(3);
	double Ep2pn=function2p->GetParError(2);
	double Ep3pn=function2p->GetParError(3);
	/*
	ratiosum130[lj][as]->Fit(functionsum,"Q");
	functionsum->FixParameter(0,1);
	double p2pn=functionsum->GetParameter(2);
	double p3pn=functionsum->GetParameter(3);
	double Ep2pn=functionsum->GetParError(2);
	double Ep3pn=functionsum->GetParError(3);
*/
	  iT11n[lj][as]=p1pn/pz27;
	  EiT11n[lj][as]=Ep1pn/(-pz27);
	  T20n[lj][as]= p2pn/pzz13;
	  ET20n[lj][as]=Ep2pn/(-pzz13);
	  T22n[lj][as]= p3pn/pzz13;
	  ET22n[lj][as]=Ep3pn/(-pzz13);

	  ratioT20[lj][as]=Integ13[lj][as]/(2*M_PI*Integ31[lj][as]);
	  T20nn[lj][as]= sqrt(8)*(1-ratioT20[lj][as])/pzz13;
	  outch_0 << 0.772+0.004*lj <<'\t'<< iT11n[lj][as] <<'\t' << EiT11n[lj][as] <<'\t' << T20n[lj][as] << '\t' << ET20n[lj][as] <<'\t' << T22n[lj][as] <<'\t' << ET22n[lj][as] << '\t' << T20nn[lj][as] << '\t' << 0.15+as*0.15 <<endl;
      }
  }
/*
  for(int q=0 ; q<Nkinema; q++)
    ...
    cout << " HELOOO3" << "\n";
	  ...
	  //Ks27m[qj]->Fill(function1p->GetChisquare()/function1p->GetNDF());
	  qj++;
	  ...
	  //Ks27m[qj]->Fill(function1m->GetChisquare()/function1m->GetNDF());
	  qj++;
	  ...
	  //Ks27m[qj]->Fill(function2p->GetChisquare()/function2p->GetNDF());
	  qj++;
	  ...
	  //Ks27m[qj]->Fill(function2m->GetChisquare()/function2m->GetNDF());
	  qj++;
	  ...
	  if ((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) 
	    {
	      ...
	      if (Mtrf_p[q][j]->GetMean()>0 && (iT11[q][j]>0 ||  T20[q][j]>0 || T22[q][j]>0))
		{
		  outch0 << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< PoCm_p[q][j]->GetMean()<< '\t' << PoCm_d[q][j]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j <<endl;
		}
	      if (hEn[q][j]->GetMean()<.45 && MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q])))
		{
		  outch << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< PoCm_p[q][j]->GetMean()<< '\t' << PoCm_d[q][j]->GetMean() <<'\t'<< 0.001*Mtrf_p[q][j]->GetMean() <<'\t'<< iT11[q][j]<<'\t' << EiT11[q][j] <<'\t' << T20[q][j] << '\t' << ET20[q][j] <<'\t' << T22[q][j] <<'\t' << ET22[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<endl;
		}
	      jq++;
	    }
	}
...
for (int as=0;as<4;as++)
  {
    for (int lj=0;lj<20;lj++)
      {
	ratiosum270[lj][as]->Write();
	ratiosum130[lj][as]->Write();
      }
  }
// for (int i=0;i<Nkinema;i++){
//   for (int j=0;j<Snr;j++){
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
    ...
    do { l++;
      ...
      Smax1[k]=280;
      if (ES3[k][l]<.3 && Scu[k][l]>Smin2[k] /*&& Scu[k][l]<Smax2[k]*/)
	{ 
	  MinEnval[k]=MINv(MinEnval[k],ES3[k][l]);
	  if (MinEnval[k]==ES3[k][l]) MinEnSval[k]=Scu[k][l];
	}
 ...
 hEnMT0= (TH2D*)f1->Get("hEnMT0");
  for (int as=0;as<4;as++)
      ...
      for (int lj=0;lj<20;lj++)
	{
	  ...
	  Phi31pm0[lj][as]= (TH1D*)f1->Get(car7pm0[lj][as]);
	  Phiraw31pm0[lj][as]= (TH1D*)f1->Get(car8pm0[lj][as]);
	  Phi27pm0[lj][as]= (TH1D*)f1->Get(car9pm0[lj][as]);
	  Phiraw27pm0[lj][as]= (TH1D*)f1->Get(car10pm0[lj][as]);
	  Phi13pm0[lj][as]= (TH1D*)f1->Get(car11pm0[lj][as]);
	  Phiraw13pm0[lj][as]= (TH1D*)f1->Get(car12pm0[lj][as]);
	  
	  sprintf(car230[lj][as],"ratiosum270_%d_%d",(int)(772+lj*4),as);
	  ratiosum270[lj][as]= new TH1D (car230[lj][as],"cntStgP+cntStgM/2;MWPCphi ",Nbin,0,360);
	  ratiosum270[lj][as]->Sumw2();
	  sprintf(car250[lj][as],"ratiosum130_%d_%d",(int)(772+lj*4),as);
	  ratiosum130[lj][as]= new TH1D (car250[lj][as],"cntStgP+cntStgM/2;MWPCphi ",Nbin,0,360);
	  ratiosum130[lj][as]->Sumw2();
  ...
  for (int i=0;i<Nkinema;i++){
	...
	//	hEn[i][j]= new TH1D (car18en[i][j],"hEn;En [MeV]",300,0,0.3);
	...
	hEn[i][j]=(TH1D*)f1->Get(car18en[i][j]);
	Mtrf_p[i][j]=(TH1D*)f1->Get(nmt1[i][j]);
	PoCm_d[i][j]=(TH1D*)f1->Get(nmcmd[i][j]);
	PoCm_p[i][j]=(TH1D*)f1->Get(nmcmp[i][j]);
      }
    }
