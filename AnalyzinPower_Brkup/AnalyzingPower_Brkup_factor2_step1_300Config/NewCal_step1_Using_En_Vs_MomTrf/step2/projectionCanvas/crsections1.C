...
void crsections1::Loop()
{
 ...
 double iT11n[20][4]={0.},EiT11n[20][4]={0.},T20n[20][4]={0.},T20nn[20][4]={0.},ET20n[20][4]={0.},T22n[20][4]={0.},ET22n[20][4]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeFixPar3_newCal25_100_QFSdata.root","recreate");
  fitfunctions();
  TPostScript *ps = new TPostScript(psfile0,111);
  TCanvas *ccn=new TCanvas("ccn","");
  /*cc->Size(27,18);
  cc->SetFillColor(10);
...
do { l++;
 ...
 gra=new TGraph();
 gra->SetLineColor(2);
 for (int i=0;i<cntra-3;i++)
   {
     gra->SetPoint(i,EE[i+1],SS[i+1]);
   }
  for (int i=1;i<13;i++)
    {
      sprintf(nnn[i],"test_%i",i);
      test[i] = new TH1D(nnn[i],"",500,0,5);
    }
  hEnMT0= new TH2D ("hEnMT0",";u 1e3*[GeV/c]^2;En [MeV]",800,500,900,500,0,5);
 hEnMT0= (TH2D*)f1->Get("hEnMT0");
 hEnMT0->SetStats(0);
 ps->NewPage();
  for (int i=1;i<13;i++)
    {
      hEnMT0->ProjectionY(nnn[i],560+(i-1)*10,560+i*10,"e");
      double pyM= test[i]->GetMaximumBin();
      double pyMc=test[i]->GetBinContent(pyM);
      cout << pyM << '\t' << pyMc << endl;
      test[i]->Scale(1/pyMc);
      ccn->cd(i);
      //test[i]->SetMarkerStyle(7);
      test[i]->Draw();
      gra->Draw("same");
      //      ccn->Update();
    }
