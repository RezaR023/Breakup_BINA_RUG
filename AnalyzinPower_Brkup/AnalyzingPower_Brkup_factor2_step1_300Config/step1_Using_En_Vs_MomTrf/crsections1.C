...
double  Len_1 = 0.0;
double  Len_2 = 0.0;
double tta1=0.0,tta2=0.0,pphi12=0.0, dtta=1.;
...
TRandom *xran = new TRandom();
   ...
   cutg->SetPoint(11,81.4279,1.259853);
   cutg->SetPoint(12,82.5279,1.369853);
   cutg->SetPoint(13,85.5135,1.469853);
   cutg->SetPoint(14,96.2847,1.569853);
   cutg->SetPoint(15,111.126,1.569853);
...
//double charge=1640492.0;
 ...
 double iT11[Nkinema][Snr]={0.},EiT11[Nkinema][Snr]={0.},ImiT11[Nkinema][Snr]={0.},EImiT11[Nkinema][Snr]={0.},T20[Nkinema][Snr]={0.},ET20[Nkinema][Snr]={0.},T22[Nkinema][Snr]={0.},ET22[Nkinema][Snr]={0.},ImT22[Nkinema][Snr]={0.},EImT22[Nkinema][Snr]={0.};
  ...
  TFile *f = new TFile("vbrkupAnaPowfiWholeUsingCutNbin20_MixCal_100n_missMass.root","recreate");
  fitfunctions();
...
if(outch)
  ...
  ///////////////////////////////////
  double Ekin0=.130;//0.1277;//0.130;//, 160
  double Mtar0=1.8756;
  double Mbm0=1.8756;
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
	    ...
	    if ((abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
		||(abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5)) )
	      {//asume that first particle is d and second is p//
		contrevent++;	    
		...
		//asume that first particle is d and second is p//
		if (1==1 && abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
		  {
		  ...
		  //////////////////////////////////////////
		    ...
		    if (deltTOF2<1.3 && cutg->IsInside(b3_EFW1,deltTOF2) /*&& E_Neut>=0 && E_Neut<0.3*/){ 
		      ...
		      for (int as=0;as<4;as++)
			{
			  if (E_Neut>=as*.150 && E_Neut<(as+1)*.150)
				...
				{
				  if (Momtrf_p>=770+lj*4 && Momtrf_p<770+(lj+1)*4)
				    {
		...
		if (1==1 && abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
		  {
		    /****************/
		  ...
		  //////////////////////////////////////////
		    ...
		    if (deltTOF3<1.3 && cutg->IsInside(b3_EFW2,deltTOF3)/* && E_Neut>=0 && E_Neut<0.3*/){ 
		      /**************
...
int BookingHist(){
  ...
  //	sprintf(car18ens0,"hEnS0_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i]);
	...
	hEnMT0= new TH2D ("hEnMT0",";MT_P [1e3*GeV/c]^2;En [MeV]",800,500,900,500,0,5);
  ...
  for (int i=0;i<Nkinema;i++){
	...
	hEnMT[i]= new TH2D (car18enMT[i],";MT_P [1e3*GeV/c]^2;En [MeV]",800,500,900,500,0,5);
    for(int j=0 ; j<Snr ; j++)
      {
