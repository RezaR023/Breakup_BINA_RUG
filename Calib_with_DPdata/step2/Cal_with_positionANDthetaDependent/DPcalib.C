...
char nam[10][350], nam1[10][350],namgP[10][350],nam2[10][350],nam5[10][21][350],namt[10][350];
...
void DPcalib::Loop()
{
  ...
  if(int1)
	...
	for (int t=0;t<21;t++)
	  {
	    sprintf(nam5[i][t],"gr_EpXp_%d_%f",i,17.5+t);
	    //gainP[i][j] = new TGraphErrors();
	    gEpXp[i][t] = new TGraph();
	    // gainP[i][t]->SetName(nam5[i][t]);
	  }
      }
    int itl=0;
    for (int i=0;i<10;i++)
	...
	for (int t=0;t<21;t++)
	  {itl=0;
	    for (int lt=0;lt<cnl;lt++)
	      {
		if (abs(17.5+t-Stt[lt])<.5)
		  {
		    gEpXp[i][t]->SetPoint(itl,S11[lt],S22[lt]);
		    itl++;
		  }	    
	      }
	  }
      }
    double param[10][21][7]={0.};
    for (int i=0;i<10;i++)
      {
	for (int t=0;t<21;t++)
	  {
	    gEpXp[i][t]->Fit("gaus","Q","",-100,100);
	    //gEpXp[i][t]->GetParameters(param[i][t]);
