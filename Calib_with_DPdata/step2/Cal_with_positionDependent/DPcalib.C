...
TFile *f1=new TFile("../../vvbreakDP_CHANdivEKINDep.root");
...
{
  using namespace std;
  TFile *f = new TFile("vbreakDPn_CHANdivEKINDep.root","recreate");
...
*/
	...
	htes[i]=new TH2D(nam2[i],";#X_{p} [.5*mm];E_{d} [Chan]",160,-80,80,1900,0,190);
	
      }
    int lg2=0;
    for (int i=0;i<10;i++)//lg1=0;Ref=0,0;   sss=150;
      {lg2=0;
	for (int ss=0;ss<160;ss=ss+8)
	  {//lg++;//120//previous <120
	    hx[i]->ProjectionY("test",hx[i]->GetXaxis()->FindBin(-80+ss),hx[i]->GetXaxis()->FindBin(-72+ss));//4+2*ss,8+2*ss);
	    bm=test->GetMaximumBin();
	    BC=test->GetBinContent(bm);
	     if (BC>40)
	      {  //<<--for BC
		ffga->SetParameters(BC,bm,2);
		test->Fit(ffga,"Q","",.1*bm-2,.1*bm+2);
		// bg=ffga->GetParameter(0);
		bm=Ref=ffga->GetParameter(1);
		//bn=ffga->GetParameter(2);
		htes[i]->Fill(-76+ss,Ref);//fx[i]->Eval(-78+ss));
		//cc->cd();
		//test->Draw("cont0");
		//cc->Update();
		//	ps->NewPage();
		...
		if (fx[i]->Eval(-76+ss)<=0) cout <<"some thing is wrong"<<endl;//return -1;
		else if(fx[i]->Eval(-76+ss)>0)
		  {
		    if (i==1)
		      {
			//			if (ss<152)
			  {
			    gainP[i]->SetPoint(lg2,-76+ss,Ref);// /fx[i]->Eval(-78+ss));
			    lg2++;
			  }
		      }
		    else
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for (int i=0;i<10;i++)
	...
	{
	  EFW0 = FITFUX(b3_WLdet1,b3_MWPCX1-95.5,b3_WLE1,b3_WRE1);
...
double FITFUX(double detN, double X,double adcL, double adcR)
{
  if (detN==1) finalfuncpx->SetParameters(11.3945,-0.00770806,-0.000114415,5.17868e-07,4.74886e-09);
  if (detN==2) finalfuncpx->SetParameters(10.2267,-0.00124446,-0.000109443,3.34713e-07,6.83943e-09);
  if (detN==3) finalfuncpx->SetParameters(11.3008,-0.00219675,-9.98263e-05,4.17427e-07,5.38547e-09);
  if (detN==4) finalfuncpx->SetParameters(11.0741,-0.000289716,-5.67013e-05,9.33379e-08,-1.03381e-09);
  if (detN==5) finalfuncpx->SetParameters(6.1516,-0.000736735,-0.00013859,1.43878e-08,6.5832e-09);
  if (detN==6) finalfuncpx->SetParameters(5.49184,0.000220729,-0.000113417,-1.67957e-09,4.43836e-09);
  if (detN==7) finalfuncpx->SetParameters(9.0593,-0.00152951,-4.90659e-05,5.87302e-09,2.27433e-09);
  if (detN==8) finalfuncpx->SetParameters(8.68874,-0.000117846,-4.31323e-05,-8.8026e-08,-5.53255e-10);
  if (detN==9) finalfuncpx->SetParameters(9.14,-0.00047518,-9.83665e-05,2.19587e-07,4.31338e-09);
  if (detN==10) finalfuncpx->SetParameters(11.3429,-0.00158524,-0.000118636,7.22401e-08,4.39448e-09);
  return sqrt(adcL*adcR)/finalfuncpx->Eval(X);
}
 */
