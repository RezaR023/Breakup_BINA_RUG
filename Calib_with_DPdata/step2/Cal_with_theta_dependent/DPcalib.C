...
void DPcalib::Loop()
{
  ...
  fitfuncp= new TF1("fitfuncp","pol5");
...
*/
	...
	htes[i]=new TH2D(nam2[i],";#theta_{p} [deg];E_{d} [Chan]",70,10,45,1900,0,1900);
	...
	for (int ss=0;ss<21;ss++)
	  {//lg++;//120//previous <120
	    hteta[i]->ProjectionY("test",hteta[i]->GetXaxis()->FindBin((int) 17+ss),hteta[i]->GetXaxis()->FindBin((int) 19+ss));//4+2*ss,8+2*ss);
	    bm=test->GetMaximumBin();
	    BC=test->GetBinContent(bm);
	    if (BC>10)
	      {  //<<--for BC
		test->Fit(ffga,"Q","",bm-100,bm+100);
		// bg=ffga->GetParameter(0);
		bm=Ref=ffga->GetParameter(1);
		//bn=ffga->GetParameter(2);
		htes[i]->Fill((int) 18+ss,Ref);
		//	cc->cd();
		//test->Draw("cont0");
		//cc->Update();
		//	ps->NewPage();
	...
	//cc->Update();
		...
		if (ftta->Eval(18+ss)<=0) cout <<"some thing is wrong"<<endl;//return -1;
		else if(ftta->Eval(18+ss)>0)
		  {
		    gainP[i]->SetPoint(lg2,(int)18+ss,Ref/ftta->Eval((int)18+ss));
		    lg2++;
		  }
	      } else continue;
	  }//for ss
    ...
    for (int i=0;i<10;i++)
      {
      gainP[i]->Fit(fitfuncp,"Q","",17,39);//10,105
...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  for (int i=0;i<10;i++)
	...
	{
	  EFW0 = FITFU(b3_WLdet1,b3_MWPCtheta1,b3_WLE1,b3_WRE1);
