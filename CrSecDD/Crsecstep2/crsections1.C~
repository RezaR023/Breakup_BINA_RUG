...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcrsecsfi120n2.root","recreate");
  fitfunctions();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
	    ...
	    //++if (tta1<=20) q=1;
	    //++else q=5;
	    if (abs(b3_MWPCtheta1-tta1)<1 && abs(b3_MWPCtheta2-tta2)<1 && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
	      {//asume that first particle is d and second is p//
		contrevent++;	    
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
	...
	if (ES1[k][l]>25 && ES2[k][l]<27.3 && ES2[k][l]>27) Smax1[k]=Scu[k][l];
