...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcrsecsfi180histQFSAh.root","recreate");
  fitfunctions();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(b3_ID==1 && LT>.1 && dowscale<100 && abs(b3_WLtdc1-b3_WLtdc2)<80 && b3_T1>0 && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 && b3_Pol==31 )//&& (b3_EFW2+b3_EFW1)>127.7)
      {//<<--<<--!
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
	    ...
	    //++if (tta1<=20) q=1;
	    //++else q=5;
	    if (abs(b3_MWPCtheta1-tta1)<1 && abs(b3_MWPCtheta2-tta2)<1 && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5))
	      {//asume that first particle is d and second is p//
		contrevent++;	    
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ 
	  ...
	  TF1 *ff1= new TF1("cn1n","pol3(0)+gaus(4)",-10,20);
	  TF1 *ff2= new TF1("cn2n","pol3(0)+gaus(4)",-10,20);
	  ...
	  double omega1=(2.*D2R*(dlF1*D2R)*sin(tta_1[q]*D2R));/*sr*/
	  double omega2=(2.*D2R*(dlF2*D2R)*sin(tta_2[q]*D2R));/*sr*/
...
double pp_scalning(double x ,int i)
{
  ...
  E1=0.0;
    ...
    E1=(sqrt(1+4*x*p2p[i]/p1p[i])-1.0)/(2.0*p2p[i])-.9;
...
double dd_scalning(double y ,int i)
{
  ...
  double E2;
  E2==0.0;
  E2=(sqrt(1+4*y*p2d[i]/p1d[i])-1.0)/(2.0*p2d[i])-1.1;
  return E2;
}
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
	...
	if (k==0) {Smin1[k]=220; Smax1[k]=240;}
	else if (k==1 || k==2 || k==3) {Smin1[k]=210; Smax1[k]=230;}
	else if (k==4) {Smin1[k]=200; Smax1[k]=220;}
	else {Smin1[k]=200; Smax1[k]=240;}
...
int BookingHist(){
  ...
  for (int i=0;i<Nkinema;i++){
    ...
    ET[i] = new TH2D (name22[i],"",2048,0,1024,800,-200,200);
  ...
  */
  ...
  /*  for (int i=0;i<Nkinema;i++){
	  sprintf(name25tot[i],"sbintot_phi_%d",i);
	  sprintf(name55tot[i],"sbin1Dtot_%d",i);
	  sprintf(nameEtot[i],"sbinraw1tot_%d",i);
	  sbin11tot[i]= new TH2D(name25tot[i],";#phi_{d} [deg];D [MeV]",360,0,360,60,-30,30);
	  sbin1Dtot[i]= new TH1D(name55tot[i],";D [MeV]",60,-30,30);
	  sbinraw1Dtot[i]= new TH1D(nameEtot[i],";D [MeV]",60,-30,30);
}*/
}
