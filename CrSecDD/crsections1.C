...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcrsecsfi180UsingTOF_MWTOF_Check10Per.root","recreate");
  fitfunctions();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
	    ...
	    //if ((abs(b3_MWPCtheta1-tta1)<2 && abs(b3_MWPCtheta2-tta2)<2 && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
	    //	||(abs(b3_MWPCtheta1-tta2)<2 && abs(b3_MWPCtheta2-tta1)<2 && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta2,tta1,b3_MWPCphi1,b3_MWPCphi2)))
	      {//asume that first particle is d and second is p//
		contrevent++;	    
