...
double outputMis[Nkinema][Snr]={0.};
...
TRandom *xran = new TRandom();
   ...
   TCutG *cutgmis = new TCutG("cutgmis",21);
   //   cutgmis->SetVarX("");
   //cutgmis->SetVarY("");
   cutgmis->SetTitle("Graph");
   cutgmis->SetFillColor(1);
   cutgmis->SetPoint(0,0.0392784,-4.89361);
   cutgmis->SetPoint(1,2.283628,-3.51884);
   cutgmis->SetPoint(2,8.28794,-.27849);
   cutgmis->SetPoint(3,13.1798,0.844945);
   cutgmis->SetPoint(4,21.6429,0.872243);
   cutgmis->SetPoint(5,30.939,1.14108);
   cutgmis->SetPoint(6,86.061,1.09628);
   cutgmis->SetPoint(7,89.9889,1.2307);
   cutgmis->SetPoint(8,91.1673,1.58915);
   cutgmis->SetPoint(9,96.9283,1.58915);
   cutgmis->SetPoint(10,96.1427,0.648208);
   cutgmis->SetPoint(11,92.0838,-0.606388);
   cutgmis->SetPoint(12,85.5373,-1.09926);
   cutgmis->SetPoint(13,76.7649,-1.50253);
   cutgmis->SetPoint(14,16.7985,-1.50253);
   cutgmis->SetPoint(15,12.8705,-2.35386);
   cutgmis->SetPoint(16,8.89514,-3.33961);
   cutgmis->SetPoint(17,3.62907,-6.25267);
   cutgmis->SetPoint(18,2.908673,-8.25207);
   cutgmis->SetPoint(19,0.0222584,-8.38649);
   cutgmis->SetPoint(20,0.0392784,-5.89361);
...
//double charge=1640492.0;
 double cross[Nkinema][Snr],Ercross[Nkinema][Snr],crossMis[Nkinema][Snr],ErcrossMis[Nkinema][Snr];
 double LT=0.0,dowscale=0.0;
 Double_t xran1,tdc1t,tdc2t,tdc1m,tdc2m,deltTDC,eep0,ep0,PtargMis,MMN,EnMis,eep0n,ep0n,PtargMisn,MMNn,EnMisn,X1X,X2X,X3X,Momtrf_p,Momtrfn_p;
 double Edmin=3., Edmax=90, Epmin=10, Epmax=105.;
 bool success0,success0n;
 TLorentzVector pDeuteron0,pDeuteron0n;
 TLorentzVector pProton0,pProton0n;
 TLorentzVector pNeutron0,pNeutron0n;	
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  gr_Cro->SetName("gr_Cro");
  gr_Cro->SetMarkerStyle(21);
  gr_Cro->SetMarkerColor(4);*/
  TFile *f = new TFile("vbrkupcrsecsfiWholefinal_factor2_mbarn_Ahcal_dtta1_mismass.root","recreate");
  fitfunctions();
...
if(outchQ)
		...
		if (abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		  {
			...
			if (cutgmis->IsInside(b3_EFW1,deltTOF2))
			  {
			    Ddetec0=b3_EFW1;
			    Dtarg0 = Ddetec0+funcc2->Eval(Ddetec0);
			    /*********	    
			    ...
			    eep0 = 5e-5*Ptarg0;//GetEnergyResolutionNeutron(pProton);
			    ep0=Ptarg0/1000;
			    success0 = GetNeutronEnergy(minimizer,
							b3_MWPCtheta1/r2d,
							b3_MWPCphi1/r2d,
							Dtarg0/1000,
							b3_MWPCtheta2/r2d,
			    ...
			    GetMomenta(b3_MWPCtheta2/r2d,b3_MWPCphi2/r2d,ep0,MP,pProton0);
			    //TLorentzVector pNeutron;
			    //  GetMomenta(thetaNeutron,phiNeutron,par[0],MN,pNeutron);
			    pNeutron0 = (target+beam) - (pDeuteron0+pProton0); 
			    MMN=pNeutron0.M()*1000;			
			    MissingMassN[q]->Fill(MMN);
			    EnMis=1000*(pNeutron0(3)-pNeutron0.M());
			    Momtrf_p=(beam-pProton0).M2()*1e3;
			    //////////////////////////////////////////
			    he[q]->Fill(PtargMis,Dtarg0);
			    
			    for(int j=0 ; j<Snr ; j++)
			      {
				if (cut[q][j]->IsInside(PtargMis,Dtarg0))
				  {
				    value1=projectScut(PtargMis,Dtarg0,j,q,corec);
				    sbin1DMis[q][j]->Fill(value1,dowscale/LT);
				    sbinraw1DMis[q][j]->Fill(value1);
				    hEn[q][j]->Fill(EnMis);
		...
		if (abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta2,tta1,b3_MWPCphi1,b3_MWPCphi2))
		  {
			...
			if (cutgmis->IsInside(b3_EFW2,deltTOF3))
			  {
			    Ddetec0n=b3_EFW2;
			    Dtarg0n = Ddetec0n+funcc2->Eval(Ddetec0n);
			    /*********	    
			    ...
			    /////TLorentzVector pDeuteronSmear0;
			    /////GetMomenta(b3_MWPCtheta1/r2d,b3_MWPCphi1/r2d,Dtarg0/1000,MD,pDeuteronSmear0);
			    eep0n = 5e-5*Ptarg0n;//GetEnergyResolutionNeutron(pProton);
			    ep0n=Ptarg0n/1000;
			    success0n = GetNeutronEnergy(minimizer,
							 b3_MWPCtheta2/r2d,
							 b3_MWPCphi2/r2d,
							 Dtarg0n/1000,
							 b3_MWPCtheta1/r2d,
			    ...
			    GetMomenta(b3_MWPCtheta1/r2d,b3_MWPCphi1/r2d,ep0n,MP,pProton0n);
			    //TLorentzVector pNeutron;
			    //  GetMomenta(thetaNeutron,phiNeutron,par[0],MN,pNeutron);
			    pNeutron0n = (target+beam) - (pDeuteron0n+pProton0n); 
			    MMNn=pNeutron0n.M()*1000;			
			    MissingMassN[q]->Fill(MMNn);
			    EnMisn=1000*(pNeutron0n(3)-pNeutron0n.M());
			    Momtrfn_p=(beam-pProton0n).M2()*1e3;
			    //////////////////////////////////////////
			    he[q]->Fill(PtargMisn,Dtarg0n);
			    
			    for(int j=0 ; j<Snr ; j++)
			      {
				if (cut[q][j]->IsInside(PtargMisn,Dtarg0n))
				  {
				    value1=projectScut(PtargMisn,Dtarg0n,j,q,corec);
				    sbin1DMis[q][j]->Fill(value1,dowscale/LT);
				    sbinraw1DMis[q][j]->Fill(value1);
				    hEn[q][j]->Fill(EnMisn);
				    Mtrf_p[q][j]->Fill(Momtrfn_p);
				  }
			      }
			  }
  ...
  for(int q=0 ; q<Nkinema; q++)
    { 
      if (q<64)
	{
	  if ((q>=0 && q<=5)||(q>=8 && q<=13)||(q>=16 && q<=21)||(q>=24 && q<=29)||(q>=32 && q<=37)||(q>=40 && q<=45)){dlF1=10.; dlF2=360.;}
	  if (q==6 || q==14 || q==22 || q==30 || q==38 || q==46){dlF1=10.; dlF2=155.;}
	  if (q==7 || q==15 || q==23 || q==31 || q==39 || q==47){dlF1=10.; dlF2=100.;}
	  if (q==48 || q==49 || q==50 || q==51){dlF1=133.; dlF2=10.;}
	  if (q==52 || q==53){dlF1=155.; dlF2=10.;}
	  if (q==54){dlF1=155.; dlF2=10.;}
	  if (q==55){dlF1=10.; dlF2=102.;}
	  if (q==56 || q==57 || q==58 || q==59 || q==60){dlF1=78.; dlF2=10.;}
	  if (q==61){dlF1=95.; dlF2=10.;}
	  if (q==62 || q==63){dlF1=10.; dlF2=95.;}
	}
      else if (q>=64)
	{
	  double qm=fmod((double) q,64.);
	  //	  cout << "qm=" << qm << endl;
	  if ((qm>=0 && qm<=5)||(qm>=8 && qm<=13)||(qm>=16 && qm<=21)||(qm>=24 && qm<=29)||(qm>=32 && qm<=37)||(qm>=40 && qm<=45)){dlF1=10.; dlF2=360.;}
	  if (qm==6 || qm==14 || qm==22 || qm==30 || qm==38 || qm==46){dlF1=10.; dlF2=155.;}
	  if (qm==7 || qm==15 || qm==23 || qm==31 || qm==39 || qm==47){dlF1=10.; dlF2=100.;}
	  if (qm==48 || qm==49 || qm==50 || qm==51){dlF1=133.; dlF2=10.;}
	  if (qm==52 || qm==53){dlF1=155.; dlF2=10.;}
	  if (qm==54){dlF1=155.; dlF2=10.;}
	  if (qm==55){dlF1=10.; dlF2=102.;}
	  if (qm==56 || qm==57 || qm==58 || qm==59 || qm==60){dlF1=78.; dlF2=10.;}
	  if (qm==61){dlF1=95.; dlF2=10.;}
	  if (qm==62 || qm==63){dlF1=10.; dlF2=95.;}
	}
      int jq=0;
      for(int j=0 ; j<Snr ; j++)
	{ 
	  ...
	  outputMis[q][j] =(double) sbin1DMis[q][j]->Integral(20,40);
	  ...
	  crossMis[q][j]=(AUB*outputMis[q][j]/(1e-3*tempcharg/8.35325));
	  crossMis[q][j]=1e-3*crossMis[q][j]/(.84*.90*.99*.97*.99);/////////converted to mbarn by 1e-3
	  ErcrossMis[q][j]=crossMis[q][j]*sqrt(count11)/count12;
	  ...
	  if ((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) 
	    {
	      ...
	      gr_CroMis[q]->SetPoint(jq,(Smin1[q]+(dss[q]/2)+(j*dss[q])),crossMis[q][j]);
	      gr_CroMis[q]->SetPointError(jq,0, ErcrossMis[q][j]);
	      //	{
		  outch1 << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< cross[q][j] <<'\t' << Ercross[q][j] <<'\t'<< crossMis[q][j] <<'\t' << ErcrossMis[q][j] <<'\t' << hEn[q][j]->GetMean() << '\t'  << j <<endl;
		  //		}
		//	if (1==2 && hEn[q][j]->GetMean()<.15 && MinEnval[q]<0.15 && MinEnSval[q]>0 && MinEnSval[q]>(Smin1[q]+(j*dss[q])) && MinEnSval[q]<(Smin1[q]+((j+1)*dss[q])) )
		//{
		//outchQ << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< cross[q][j] <<'\t' << Ercross[q][j] <<'\t' << hEn[q][j]->GetMean()<< '\t'  << j << '\t' << MinEnSval[q] << '\t' << MinEnval[q] <<endl;
		//	}
...
int BookingHist(){
  for (int i=0;i<Nkinema;i++){
    ...
    sprintf(namegrmis[i],"gr_crsectionmis_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i]);
    gr_CroMis[i] = new TGraphErrors();
    gr_CroMis[i]->SetName(namegrmis[i]);
    gr_CroMis[i]->SetMarkerStyle(21);
    gr_CroMis[i]->SetMarkerColor(3);
    gr_CroMis[i]->SetLineColor(3);
  }
       cc->Size(27,18);
      cc->SetFillColor(10);
  ...
  */
  ...
  for (int i=0;i<Nkinema;i++){
	...
	sprintf(name55mis[i][j],"sbin1Dmis_%d_%d",i,j);
	sprintf(nameEmis[i][j],"sbinraw1mis_%d_%d",i,j);
	sbin1DMis[i][j]= new TH1D(name55mis[i][j],";D [MeV]",60,-30,30);
	sbinraw1DMis[i][j]= new TH1D(nameEmis[i][j],";D [MeV]",60,-30,30);
      }
  }
}
