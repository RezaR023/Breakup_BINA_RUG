...
//double charge=1640492.0;
 ...
 //  TFile *f = new TFile("vbrkupcrsecsfiWholefinal_factor2_mbarn_NEWcal_mycaln_dtta1.root","recreate");
  TFile *f = new TFile("vbrkupcrsecsfiWholefinal_factor2_mbarn_Ahcal_dtta1_offset.root","recreate");
  fitfunctions();
		...
		if (abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		  {
			...
			//Pdetec=func3->Eval(Pdetec);
			//Ddetec=func4->Eval(Ddetec);
...
void openFile_newcal()
{
  ifstream parad0,parap0;
  parad0.open(paramsdn);
  if(paradn) fprintf(stdout,"file  %s is open.\n",paramsdn);
  else{
    fprintf(stdout,"1 can't open file %s \n",paramsdn);
    return 0;
  }
