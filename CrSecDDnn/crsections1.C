...
/////precalibration for proton with theta dependent///////////////////
double FITFU(double detN, double thta,double adcL, double adcR)
{
  if (detN==1){ finalfcp->SetParameters(-459.394,74.6735,-4.74034,0.150705,-0.00240084,1.53278e-05);}
  if (detN==2){ finalfcp->SetParameters(-41.0509,9.70198,-0.731642,0.0276449,-0.000523921,3.97322e-06);}
  if (detN==3){ finalfcp->SetParameters(18.3343,-1.65468,0.143389,-0.00561488,9.67815e-05,-5.56161e-07);}
  if (detN==4){ finalfcp->SetParameters(-52.5719,13.7452,-1.1857,0.0511909,-0.001108,9.60809e-06);}
  if (detN==5){ finalfcp->SetParameters(-170.833,38.0319,-3.25628,0.138792,-0.00294802,2.49668e-05);}
  if (detN==6){ finalfcp->SetParameters(-58.204,14.0411,-1.24249,0.0549955,-0.00121825,1.07889e-05);}
  if (detN==7){ finalfcp->SetParameters(-45.7616,11.3628,-0.952052,0.0402517,-0.000859121,7.38948e-06);}
  if (detN==8){ finalfcp->SetParameters(-17.9966,5.54053,-0.465101,0.0195814,-0.000413764,3.49907e-06);}
  if (detN==9){ finalfcp->SetParameters(-16.6497,4.87646,-0.37643,0.0146517,-0.000287311,2.25825e-06);}
  if (detN==10){ finalfcp->SetParameters(-177.291,29.8454,-1.90833,0.0615189,-0.000999893,6.54241e-06);}

  return sqrt(adcL*adcR)/finalfcp->Eval(thta);
}
...
TRandom *xran = new TRandom();
   ...
   /////////////new calb func for proton////
  finalfcp= new TF1("finalfcp","pol5");
...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcrsecsfiWholefinal_factor2_mbarn_NEWcal25config_dtta1n_offsetcurrent.root","recreate");
  fitfunctions();
...
if(outchQ)
 ...
 openFile_newcal();
		...
		if (abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta1,tta2,b3_MWPCphi1,b3_MWPCphi2))
		  {
		    ...
		    if (Ddetec>=EDmin && Ddetec<=EDmax && Ddetec<(EDMAX[q])) Ddetec=Ddetec/dn_scalning(Ddetec ,b3_WLdet1, b3_MWPCtheta1, b3_MWPCtheta2);
		    if (Ddetec>MIN0(EDmax,(EDMAX[q]))) Ddetec=Ddetec/dn_scalning(MIN0(EDmax,(EDMAX[q])),b3_WLdet1, b3_MWPCtheta1, b3_MWPCtheta2);
		    
		    if (Pdetec<EPmin) Pdetec=Pdetec/pn_scalning(EPmin ,b3_WLdet1, b3_MWPCtheta1, b3_MWPCtheta2);
		    if (Pdetec>=EPmin && Pdetec<=EPmax && Pdetec<(EPMAX[q])) Pdetec=Pdetec/pn_scalning(Pdetec ,b3_WLdet1, b3_MWPCtheta1, b3_MWPCtheta2);
		    if (Pdetec>MIN0(EPmax,(EPMAX[q]))) Pdetec=Pdetec/pn_scalning(MIN0(EPmax,(EPMAX[q])),b3_WLdet1, b3_MWPCtheta1, b3_MWPCtheta2);
		    //////////////////////////////////////////////////////////////////////////
		  ...
		  //////////////////////////////////////////////////////////////////
		    ...
		    //		    TDC_ratio_phi[q]->Fill(b3_MWPCphi1,deltTOF2);
		    if ( 1==1 && cutgpm1->IsInside(b3_EFW1,deltTOF2)) MissingMassN00->Fill(P_Neut,MMN);
		    ///////cut when your first particle is deuteorn///
		    if ( 1==1 && cutgpm1->IsInside(b3_EFW1,deltTOF2))// && MMN>932 && MMN<947)
		      {   
			...
			//Pdetec=FITFU(b3_WLdet2,b3_MWPCtheta2,b3_WLE2,b3_WRE2);
			//Pdetec=FITFUX(b3_WLdet2,b3_MWPCX2-95.5,b3_WLE2,b3_WRE2);
			//Ddetec=FITFUXD(b3_WLdet1,b3_MWPCX1,b3_WLE1,b3_WRE1);
			///Ddetec=b3_EFW1;//Ddetec0;
		...
		if (abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5) && phi_selection(tta2,tta1,b3_MWPCphi1,b3_MWPCphi2))
		  {
		    ...
		    if (Ddetec0n>=EDmin && Ddetec0n<=EDmax && Ddetec0n<(EDMAX[q]))  Ddetec0n=Ddetec0n/dn_scalning(Ddetec0n ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
		    if (Ddetec0n>MIN0(EDmax,(EDMAX[q])))  Ddetec0n=Ddetec0n/dn_scalning(MIN0(EDmax,(EDMAX[q])),b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
		    
		    if (Pdetec0n<EPmin)  Pdetec0n=Pdetec0n/pn_scalning(EPmin ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
		    if (Pdetec0n>=EPmin && Pdetec0n<=EPmax && Pdetec0n<(EPMAX[q])) Pdetec0n=Pdetec0n/pn_scalning(Pdetec0n ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
		    if (Pdetec0n>MIN0(EPmax,(EPMAX[q])))  Pdetec0n=Pdetec0n/pn_scalning(MIN0(EPmax,(EPMAX[q])),b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
		    /////////////////////////////////////////////////////////////////////////////
		    /******************
		  ...
		  ////////////////////////////////////////////////////////////////////////
		    ...
		    if ( 1==1 && cutgpm1->IsInside(b3_EFW2,deltTOF3)) MissingMassN00->Fill(P_Neut,MMN);
		    ///////cut when your second particle is deuteron//////
		    if ( 1==1 && cutgpm1->IsInside(b3_EFW2,deltTOF3))// && MMN>932 && MMN<947)
		      {   
			he0[q]->Fill(b3_EFW2,b3_EFW1);
  ...
  tempcharg=8.35*1e-4*kHzPol*(II+.28);
  for(int q=0 ; q<Nkinema; q++)
    { 
      if (q<64)
	{
	  ...
	  if ((Smin1[q]+(dss[q]/2)+(j*dss[q]))>Smin2[q] && (Smin1[q]+(dss[q]/2)+(j*dss[q]))<Smax2[q]) 
	    {
	      ...
	      //	{
		  outch1 << tta_1[q] << '\t' << tta_2[q] <<'\t' << Smin1[q]+(dss[q]/2)+(j*dss[q]) <<'\t'<< cross[q][j] <<'\t' << Ercross[q][j] /*<<'\t' << hEn[q][j]->GetMean()*/<< '\t'  << j <<endl;
		  //		}
