...
TFile *f1=new TFile("vhistAllconfigvastnn.root");
...
{
  using namespace std;
  TFile *f = new TFile("vbreakaftercalib_pol4pol4n_check.root","recreate");
  ...
  TPostScript *ps = new TPostScript(psfile1,111);
  cc=new TCanvas("cc","");
  BookingHist();
  openfiles();
  TH1D *test = new TH1D("test","",1900,0,1900);
  TH1D *test1 = new TH1D("test1","",1900,0,1900);
  ...
  if(out1)
   ...
   for (int i=0;i<Nkinema;i++){
     for (int j=1;j<11;j++){lg=0;Ref=0,0;
       ...
       for (int ss=3;ss<120;ss=ss+1){lg++;//120//100
	 /*ps->NewPage();
	   cc->cd();
	   E1E2ia[i][j]->Draw();
	   cc->Update();*/
	 if (ss<60)
	   {
	     ...
	     //E1E2ia[i][j]->ProjectionY("test",120-(ss+1),120-(ss-1));
	     bm=test->GetMaximumBin();
	     double BC=test->GetBinContent(bm);
	     if (BC>5){  //<<--for BC
	       //ff->SetParameters(BC,bm,3);
		 ...
		 //if(scale21>.8 && scale21<1.2) 
		   {lgd++; gainD[i][j]->SetPoint(lgd,Ref,scale21);  /*gainD[i][j]->SetPointError(lg,0,0);*/}
		   //if(scale22>.8 && scale22<1.11) 
		   {lgp++; gainP[i][j]->SetPoint(lgd,sss,scale22);  /*gainP[i][j]->SetPointError(lg,0,0);*/}
		 //lg++;
	 ...
	 else if (ss>=60)
	   {
		 ...
		 if (A21!=0) scale21=Ref/A21;// scaling for D
		 if (A22!=0) scale22=sss/A22;// scaling for P
		 //if(scale21>.8 && scale21<2) 
		   {lgd++; gainD[i][j]->SetPoint(lgd,Ref,scale21);  /*gainD[i][j]->SetPointError(lg,0,0);*/}
		   //if(scale22>.8 && scale22<1.11) 
		   {lgp++; gainP[i][j]->SetPoint(lgp,sss,scale22);  /*gainP[i][j]->SetPointError(lg,0,0);*/}
		 //lg++;
	       }} //<<--for BC
	   }
	     minf=100000000;
       ...
       ///cc->cd();
       // henergy1[i][j]->SetMarkerStyle(7);
       //henergy1[i][j]->Draw("samep");
       //cc->Update();      

      pard0[i][j]=0; pard1[i][j]=0; pard2[i][j]=0; pard3[i][j]=0; pard4[i][j]=0; pard5[i][j]=0;
      parp0[i][j]=0; parp1[i][j]=0; parp2[i][j]=0; parp3[i][j]=0; parp4[i][j]=0; parp5[i][j]=0;

      fitfuncp1= new TF1("fitfuncp1","pol4");
      fitfuncd1= new TF1("fitfuncd1","pol4");
      fitfuncd1->SetParameters(1.,0.,0.,0.,0.);
      gainD[i][j]->Fit(fitfuncd1,"Q","",3,100);

      pard0[i][j]=fitfuncd1->GetParameter(0);
      pard1[i][j]=fitfuncd1->GetParameter(1);
      pard2[i][j]=fitfuncd1->GetParameter(2);
      pard3[i][j]=fitfuncd1->GetParameter(3);
      pard4[i][j]=fitfuncd1->GetParameter(4);
      pard5[i][j]=fitfuncd1->GetParameter(5);
      //  pard6[i][j]=fitfuncd1->GetParameter(6);
      delete fitfuncd1;
      fitfuncp1->SetParameters(1.,0.,0.,0.,0.);
      gainP[i][j]->Fit(fitfuncp1,"Q","",5,105);
      parp0[i][j]=fitfuncp1->GetParameter(0);
      parp1[i][j]=fitfuncp1->GetParameter(1);
      parp2[i][j]=fitfuncp1->GetParameter(2);
      parp3[i][j]=fitfuncp1->GetParameter(3);
      parp4[i][j]=fitfuncp1->GetParameter(4);
      parp5[i][j]=fitfuncp1->GetParameter(5);
      // parp6[i][j]=fitfuncp1->GetParameter(6);
      delete fitfuncp1;
      
           if (j==1)  out1 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==2)  out2 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==3)  out3 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==4)  out4 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==5)  out5 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==6)  out6 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==7)  out7 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==8)  out8 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
      else if (j==9)  out9 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';
     else if (j==10) out10 <<pard0[i][j]<<'\t'<<'\t'<<pard1[i][j]<<'\t'<<'\t'<<pard2[i][j]<<'\t'<<'\t'<<pard3[i][j]<<'\t'<<'\t'<<pard4[i][j]<<'\t'<<'\t'<<pard5[i][j]<<'\n';



      finalfuncp[i][j]= new TF1("finalfuncp","pol4",5,105);//pol6
      finalfuncp[i][j]->SetParameters(parp0[i][j],parp1[i][j],parp2[i][j],parp3[i][j],parp4[i][j]);//,par0[i][j]);//,par1[i][j]);
      finalfuncd[i][j]= new TF1("finalfuncd","pol4"/*"(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])"*/,3,100);
      finalfuncd[i][j]->SetParameters(pard0[i][j],pard1[i][j],pard2[i][j],pard3[i][j],pard4[i][j]);//,par5[i][j]);
    }
    }
    //ps->Close();
...
printf("\r<I> Progress: %.0f%%",100*(float) jentry/((float) nentries));
	...
	// dTOF=PID_func();
			...
			Pdetec = Pdetec/finalfuncp[q][w]->Eval(Pdetec);//p_scalning(b3_EFW2,w,tta2);
			Ddetec=Ddetec/finalfuncd[q][w]->Eval(Ddetec);//d_scalning(b3_EFW1,w,tta1);
			//if (b3_EFW1<Ymin[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymin[q][w]);//d_scalning(b3_EFW1,w,tta1);
			//if (b3_EFW1>=Ymin[q][w] && b3_EFW1<=Ymax[q][w]) b3_EFW1/finalfuncd[q][w]->Eval(b3_EFW1);//d_scalning(b3_EFW1,w,tta1);
...
int BookingHist(){
  ...
  for (int i=0;i<Nkinema;i++){
    for (int j=1;j<11;j++){
      henergy1[i][j]=new TH2D(nam8[i][j],";E_{p} [MeV];E_{d} [MeV]",150*2,0,150,150*2,0,150);
    }
  }
  for (int i=0;i<Nkinema;i++){
    for (int j=1;j<11;j++){
