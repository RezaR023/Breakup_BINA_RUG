...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vbrkupcalibfiWholefinal_factor2nnn.root","recreate");
  fitfunctions();
...
if(outchQ)
 ...
 openFile_newcal();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
		...
		if ((abs(b3_MWPCtheta1-tta1)<dtta && abs(b3_MWPCtheta2-tta2)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5)  && b3_WLdet1==w )
		    ||(abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5)  && b3_WLdet2==w))
		  {
		    contrevent++;
		    ...
		    if (1==1 && abs(b3_MWPCtheta1-tta2)<dtta && abs(b3_MWPCtheta2-tta1)<dtta && (abs(Delphi-pphi12)<5 || abs(Delphi+pphi12)<5)  && b3_WLdet2==w)
		      {
			/******************
			...
			if ( 1==1 && cutgpm1->IsInside(b3_EFW2,deltTOF3))
			  {   
			    ...
			    ***/
			    ...
			    if (Ddetec<EDmin) Ddetec=Ddetec/dn_scalning(EDmin ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    if (Ddetec>=EDmin && Ddetec<=EDmax && Ddetec<(EDMAX[q]))  Ddetec=Ddetec/dn_scalning(Ddetec ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    if (Ddetec>MIN0(EDmax,(EDMAX[q])))  Ddetec=Ddetec/dn_scalning(MIN0(EDmax,(EDMAX[q])),b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    
			    if (Pdetec<EPmin)  Pdetec=Pdetec/pn_scalning(EPmin ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    if (Pdetec>=EPmin && Pdetec<=EPmax && Pdetec<(EPMAX[q])) Pdetec=Pdetec/pn_scalning(Pdetec ,b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    if (Pdetec>MIN0(EPmax,(EPMAX[q])))  Pdetec=Pdetec/pn_scalning(MIN0(EPmax,(EPMAX[q])),b3_WLdet2, b3_MWPCtheta2, b3_MWPCtheta1);
			    E1E2idetec[q][w]->Fill(Pdetec,Ddetec);
			    //Ptarg = Pdetec+funcc1->Eval(Pdetec);
			    //Dtarg = Ddetec+funcc2->Eval(Ddetec);
			    /***/
  ...
  //xs  cout << "tempcharg=" << tempcharg <<'\t'<< "II= " << II <<endl;
  f->cd();
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
	...
	Smin1[k]=130;//Scu[k][l];
	if (ES1[k][l]>25 && ES2[k][l]<25.6 && ES2[k][l]>25) {Smax2[k]=Scu[k][l];}
	...
	//p1s[k]->SetPoint(l,l,l);   p1s[k]->SetPointError(l,0,0);
      ...
      Sinput.close();
      contrll[k]=ll;
      //	cout << "***********************EDMAX= " << EDMAX[k] << '\t' << EPMAX[k] <<endl;
...
{// i is the detector number for deuteron
  ...
  if (thetd>=27.5  && thetd<32.5)
	...
	{j=20;
	  finalfuncpf->SetParameters(parp0[i][j],parp1[i][j],parp2[i][j],parp3[i][j],parp4[i][j],parp5[i][j]);
	  //if (i==8)cout  <<"i======== " << i << '\t' << j << '\t' <<  parp0[i][j]<< '\t' << parp1[i][j] << '\t' << parp2[i][j] << '\t' << parp3[i][j]<< '\t' << parp4[i][j] << '\t' <<parp5[i][j]<< endl;
	}
    } ///
  if (thetd>=32.5)
    {
...
{// i is the detector number for deuteron
  ...
  if (thetd>=27.5  && thetd<32.5)
	...
	{j=20;
	  finalfuncdf->SetParameters(pard0[i][j],pard1[i][j],pard2[i][j],pard3[i][j],pard4[i][j],pard5[i][j]);
	  //if (i==8)cout  <<"i======== " << i << '\t' << j << '\t' <<  pard0[i][j]<< '\t' << pard1[i][j] << '\t' << pard2[i][j] << '\t' << pard3[i][j]<< '\t' << pard4[i][j] << '\t' <<pard5[i][j]<< endl;
	}
    } ///
  if (thetd>=32.5)
    {
