...
//double charge=1640492.0;
 ...
 double Ayy[Nkinema][Snr]={0.}, EAyy[Nkinema][Snr]={0.},Axx[Nkinema][Snr]={0.},EAxx[Nkinema][Snr]={0.};
 double Ay[Nkinema][Snr]={0.},EAy[Nkinema][Snr]={0.},Ax[Nkinema][Snr]={0.},EAx[Nkinema][Snr]={0.};
 double Azz[Nkinema][Snr]={0.}, EAzz[Nkinema][Snr]={0.},Axy[Nkinema][Snr]={0.},EAxy[Nkinema][Snr]={0.};
  TFile *f = new TFile("vbrkupAnaPow_DP_1A.root","recreate");
  fitfunctions();
...
****/
   ...
   TF1 *function1m=new TF1("function1m","[0]*sqrt(3)*cos((x+180)*0.01745)",0,360);
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ 
	  ...
	  Ayy[q][j] =  -1*sqrt(3)* T22[q][j]  - (sqrt(2)/2)*T20[q][j];
	  EAyy[q][j] =  sqrt((-1*sqrt(3)*ET22[q][j])*(-1*sqrt(3)*ET22[q][j]) +  ((sqrt(2)/2)*ET20[q][j]) *  ((sqrt(2)/2)*ET20[q][j]));
	  Ay[q][j] = (2./sqrt(3))*iT11[q][j];
	  EAy[q][j] = (2./sqrt(3))*EiT11[q][j];
	  Ax[q][j] = -(2./sqrt(3))*ImiT11[q][j];
	  EAx[q][j] = (2./sqrt(3))*EImiT11[q][j];

	  Azz[q][j] = sqrt(2)*T20[q][j]; 
	  EAzz[q][j] = sqrt(2)*ET20[q][j];
	  Axx[q][j] =  1*sqrt(3)* T22[q][j]  - (sqrt(2)/2)*T20[q][j];
	  //EAxx[q][j] =  sqrt((-1*sqrt(3)*ET22[q][j])*(-1*sqrt(3)*ET22[q][j]) +  ((sqrt(2)/2)*ET20[q][j]) *  ((sqrt(2)/2)*ET20[q][j]));
	  //Axx[q][j]=-Ayy[q][j]-Azz[q][j];
	  EAxx[q][j]=EAyy[q][j]+EAzz[q][j];
	  Axy[q][j] = sqrt(3)*ImT22[q][j];
	  EAxy[q][j] = sqrt(3)*EImT22[q][j];


	  gr_iT11[q]->SetPoint(j,(Smin1[q]+(dss[q]/2)+(j*dss[q])),Ay[q][j]);
	  gr_iT11[q]->SetPointError(j,0, EAy[q][j]);
	  
	  gr_ImiT11[q]->SetPoint(j,(Smin1[q]+(dss[q]/2)+(j*dss[q])),Ax[q][j]);
	  gr_ImiT11[q]->SetPointError(j,0, EAx[q][j]);
	  
	  gr_T20[q]->SetPoint(j,(Smin1[q]+(dss[q]/2)+(j*dss[q])),Axx[q][j]);
	  gr_T20[q]->SetPointError(j,0, EAxx[q][j]);
	  
	  gr_T22[q]->SetPoint(j,(Smin1[q]+(dss[q]/2)+(j*dss[q])),Ayy[q][j]);
	  gr_T22[q]->SetPointError(j,0, EAyy[q][j]);

	  gr_ImT22[q]->SetPoint(j,(Smin1[q]+(dss[q]/2)+(j*dss[q])),Axy[q][j]);
	  gr_ImT22[q]->SetPointError(j,0, EAxy[q][j]);
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
    ...
    do { l++;
      ...
      //if (ES2[k][l]>26 && ES1[k][l]>20.8 && ES1[k][l]<21)
      Smin1[k]=40;//130.;//Scu[k][l];
      //if (ES1[k][l]>25 && ES2[k][l]<27.2 && ES2[k][l]>27) 
      Smax1[k]=190;//280.;//Scu[k][l];
