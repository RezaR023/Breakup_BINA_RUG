...
//double charge=1640492.0;
 ...
 /* TGraphErrors  *gr_Cro = new TGraphErrors();
  ...
  TFile *f = new TFile("vDPbrkupcrsecsfi180testDelTOFn.root","recreate");
  fitfunctions();
	...
	for(int q=0 ; q<Nkinema; q++)
	  {
		...
		Ddetec0 = b3_EFW1*.92;///d_scalning(b3_EFW1,b3_WLdet1,tta1); 
		Pdetec0 = b3_EFW2*.92;///p_scalning(b3_EFW2,b3_WLdet1,tta2);
		if (q<3){
		...
		TOF1dd=R1/(C0*TMath::Sqrt(1-TMath::Power((mp/(Dtarg0+mp)),2)));
		...
		///////if (deltTOF2<1.5)
		  ...
		  if (b3_EFW2>Xmax0[b3_WLdet1][qp]) Pdetec = b3_EFW2/p_scalning(Xmax0[b3_WLdet1][qp],b3_WLdet1,tta2);*/
		    if (b3_EFW1<3) Ddetec = b3_EFW1*.92;//  /p_scalning(3,b3_WLdet1,tta1);
		    if (b3_EFW1>=3 && b3_EFW1<=90) Ddetec = b3_EFW1*.92;//  /p_scalning(b3_EFW1,b3_WLdet1,tta1);
		    if (b3_EFW1>90) Ddetec = b3_EFW1*.92;//  /p_scalning(90,b3_WLdet1,tta1);
		  
		    if (b3_EFW2<10) Pdetec = b3_EFW2*.92;// /p_scalning(10,b3_WLdet1,tta2);
		    if (b3_EFW2>=10 && b3_EFW2<=105) Pdetec = b3_EFW2*.92;//  /p_scalning(b3_EFW2,b3_WLdet1,tta2);
		    if (b3_EFW2>105) Pdetec = b3_EFW2*.92;// /p_scalning(105,b3_WLdet1,tta2);
  ...
  for(int q=0 ; q<Nkinema; q++)
	...
	{ 
	  ...
	  Double_t par[7]={4,0,0,0,10,0,1}; 
	  ff1->SetParameters(par); 
	  sbin1D[q][j]->Fit(ff1,"Q","",-14,15);
	  ff2->SetParameters(par); 
	  sbinraw1D[q][j]->Fit(ff2,"Q","",-14,15);
	  Double_t param1[7];
	  ...
	  double   dx = 13.75;//(3.35 + 0.65)*0.1*0.169*1000; /* 0.65 ballging --target thickness mg/cm2 Density off D =0.169 (g/cm3)*/
	  double   n_atom = 2.; /* number of atoms per compond molecule*/
	  double  m_mol = 12.0+1.00794+1.00794;//4.028;    /* target weight in amu*/
	  double Fac=.5;
	  if (q==2) Fac==1.;
	  double  a = n_atom/m_mol;
	  double omega1=(2*2.*D2R*(10.*D2R)*sin(tta_1[q]*D2R));/*sr*/
	  double omega2=Fac*(2*2.*D2R*(360.*D2R)*sin(tta_2[q]*D2R));/*sr*////////.5* is for DP 180 Deg.
	  ...
	  cross[q][j]=cross[q][j]/(.84*.84*.98*.98*.99);//(.90*.90*.97*.97*.99);
...
int openfiles()
{
  ...
  for(int k=0 ; k<Nkinema ; k++)
    ...
    do { l++;
      ...
      if (ES2[k][l]>26 && ES1[k][l]>20.8 && ES1[k][l]<21) Smin1[k]=Scu[k][l];
      if (ES1[k][l]>25 && ES2[k][l]<21.5 && ES2[k][l]>21.2) Smax1[k]=Scu[k][l];
	...
	p1p2[k]->SetPoint(l,ES2[k][l],ES1[k][l]);  p1p2[k]->SetPointError(l,0,0);}
      if (k<3){
	...
	p1p2dep[k]->SetPoint(ll,ES2dep[k][l],ES1dep[k][l]);  p1p2dep[k]->SetPointError(ll,0,0);
