...
void crsections1::Loop()
{
  ...
  TFile *f = new TFile("vbreakcrsecs1.root","recreate");
  fitfunctions();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double dowscale=b3_T1sDT/b3_T1sDTS;
	...
	if (abs(b3_MWPCtheta1-28)<2 && abs(b3_MWPCtheta2-28)<2 && abs(abs(b3_MWPCphi1-b3_MWPCphi2)-180)<5)//  && b3_WLdet1==w && b3_WLdet2==11-w)
	  {//asume that first particle is d and second is p//
	    ...
	    if (deltTOF2>0 && deltTOF2<13){   
	      ...
	      Ddetec = b3_EFW1/d_scalning(b3_EFW1,b3_WLdet1,28);//finalfuncd[q][w]->Eval(b3_EFW1);
	      
	      Pdetec = b3_EFW2/p_scalning(b3_EFW2,b3_WLdet1,28);//finalfuncp[q][w]->Eval(b3_EFW2);
		...
		Dtarg = Ddetec+func2->Eval(Ddetec);*/
		 ...
		 if (q>=3){*/
	      ...
	      for(int j=0 ; j<Snr ; j++)
		{
		  if (cut[4][j]->IsInside(Ptarg,Dtarg))
		    {
		      value1=projectScut(Ptarg,Dtarg,j,4,corec);
  ...
  for(int j=0 ; j<Snr ; j++)
    {
      ...
      double omega1=(2.*D2R*(5.*D2R)*sin(28*D2R));/*sr*/
      double omega2=(2.*D2R*(360.*D2R)*sin(28*D2R));/*sr*/
