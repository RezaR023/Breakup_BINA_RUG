...
int pdbreakup::Analysis(unsigned int evSta ,unsigned int evSto,int Polar)
{
  double eltaEp=0.0;
  double deltaEd=0.0;
  double TotBeamC=0.0;
  ...
  /* loop over data */
  double Delphi;
  for (Long64_t jentry=evSta; jentry<evSto;jentry++)
    {///<<<---<<<---<<<---!
      //printf(" %i %i \n",evSta,evSto); 
      ...
      // printf(" %i \n",jentry); 
      if (ientry < 0) {analysis =1;break;}
      nb = fChain->GetEntry(jentry); //read all branches   
      nbytes += nb;
...
for(int i=1 ; i<11 ; i++)
   ...
   // sprintf(ch1,"test%d.root",pol); 
   froot=new TFile(ch2,"RECREATE");
   TTree *tree=0;
   // out=fopen(output1,"w");
   // out2=fopen(output2,"w");
   pdbreakup *t=new pdbreakup(tree); 
   int i=0;int indic=1;
   while(indic==1)
     {
   //fprintf(stdout,"sum=%i %20i \n",i,t->fChain->GetEntriesFast());
       fflush(stdout);
       indic=!(t->Analysis(i,NEvents+i,pol));        /*  ???? */
       indic=0; /* command in to run just for one bounch */
       i+=NEvents; 
       // printf("%f \n",NEvents);
     }
...
/**********correction to energy to bring it to S-curve ****/
/*
 double dismin1 = (tempd1-Emid2[i][0])*(tempd1-Emid2[i][0]) + 
		    (tempp1-Emid1[i][0])*(tempp1-Emid1[i][0]);
		  int selecbin1=0;
		  for(int jj=0 ; jj<Snr ; jj++)
		    {
		      double newdismin = (tempd1-Emid2[i][jj])*(tempd1-Emid2[i][jj]) +  
			(tempp1-Emid1[i][jj])*(tempp1-Emid1[i][jj]);
		      if (dismin1>newdismin)
			{
			  dismin1 = newdismin;
			  selecbin1 = jj;
			}
  ...
  double dismin2 = (tempd2-Emid2[i][0])*(tempd2-Emid2[i][0]) + 
 // (tempp2-Emid1[i][0])*(tempp2-Emid1[i][0]);
  int selecbin2=0;
  for(int jj=0 ; jj<Snr ; jj++)
  {
  double newdismin = (tempd2-Emid2[i][jj])*(tempd2-Emid2[i][jj]) +  
  (tempp2-Emid1[i][jj])*(tempp2-Emid1[i][jj]);
  if (dismin2 > newdismin)
  {
  dismin2 = newdismin;
  selecbin2 = jj;
  }
  ...
  double cor1[3],cor2[3];
  projectScut( tempp1, tempd1,selecbin1,i,cor1);		       
  projectScut( tempp2, tempd2,selecbin2,i,cor2);	
  tempp1 += cor1[0];
  tempd1 += cor1[1];
  tempp2 += cor2[0];
  tempd2 += cor2[1]; */
/**********correction to energy to bring it to S-curve End ****/ 
