...
TFile *f1=new TFile("vhistAllconfigvastf.root");
...
{
  using namespace std;
  TFile *f = new TFile("vbreakaftercalib22.root","recreate");
  ...
  //=============   start for scaling at detector ======/
   /*for (int i=0;i<Nkinema;i++){
     for (int j=1;j<11;j++){
       Xmin[i][j]=Ymin[i][j]=0.0;
       Xmax[i][j]=Ymax[i][j]=1000000;
	 }
   }*/
    for (int i=0;i<Nkinema;i++){
      for (int j=1;j<11;j++){lg1=0;lg2=0;Ref=0,0;   sss=150;
	...
	for (int ss=3;ss<100;ss=ss+1){//lg++;//120//previous <120
	  // double bm0=E1E2ia[i][j]->GetMaximumBin();
	  // double BC0=E1E2ia[i][j]->GetBinContent(bm0);
	  // if (BC0==0) continue;//break;
	E1E2ia[i][j]->ProjectionX("test",ss-1,ss+1);
	bm=test->GetMaximumBin();
	double BC=test->GetBinContent(bm);
	if (BC>10){  //<<--for BC
	test->Fit(ff,"Q","",bm-15,bm+15);
	bg=ff->GetParameter(0);
	bm=Ref=ff->GetParameter(1);
	bn=ff->GetParameter(2);
	if (Ref>10 && Ref<110 && Ref<sss){//120//100 precious(Ref>0 && Ref<120)
	sss=Ref;
	if (i<3) Ref=ss-1;//-1 because of bin content between 0-1 belong to 1!  
	else Ref=ss;
	//sss=ss;
	maxf=MAX0(sss,maxf);
	if (Ref==3 && sss!=0) {Xmax[i][j]=sss; Ymin[i][j]=Ref;}
	else if (maxf==sss) {Xmax[i][j]=sss; Ymin[i][j]=Ref;}

	if (Ref) {Xmin[i][j]=sss; Ymax[i][j]=Ref;}
	...
	if (A22!=0) scale22=sss/A22;// scaling for P
	  if(scale21>.5 && scale21<2 && Ref<84) {lg1++; gainD[i][j]->SetPoint(lg1,Ref,scale21);/*  gainD[i][j]->SetPointError(lg1,0,0);*/}

	if(scale22>.6 && scale22<1.11 && sss>20.5 && sss<109) {lg2++; gainP[i][j]->SetPoint(lg2,sss,scale22);  /*gainP[i][j]->SetPointError(lg2,0,0);*/}//previous scale22<1.11
	//gaindd[i][j]->Fill(Ref,scale21);
	//gainpp[i][j]->Fill(sss,scale22);
	}
	else continue;}else continue; //<<--for BC
	minf=100000000;
      }
      maxf=0.;
      //gaindd[i][j]->ProfileX("hprof");
      	   ps->NewPage();
	   cc->cd();
	   E1E2ia[i][j]->Draw("colz");
	   henergy1[i][j]->Draw("samep");
	   p1p2dep[i]->Draw("same");
	...
	parr00[i][j]=fffp->GetParameter(4);*/
      gainD[i][j]->Fit(fitfuncd1,"Q","",3,90);//3,90
      gainP[i][j]->Fit(fitfuncp1,"Q","",10,105);//10,105
      ...
      cc->Update();
	   ...
	   if (j==1)  out1 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==2)  out2 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==3)  out3 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==4)  out4 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==5)  out5 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==6)  out6 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==7)  out7 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==8)  out8 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==9)  out9 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'/*<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";
	   if (j==10) out10 /*<<Xmin[i][j]<<"\t"<<"\t"<<Xmax[i][j]<<"\t"<<"\t"<<Ymin[i][j] <<"\t"<<"\t"<<Ymax[i][j]*/<<parr0[i][j]<<"\t"<<"\t"<<parr1[i][j]<<"\t"<<"\t"<<parr2[i][j]<<'\'t'<<"\t"<<par3[i][j]<<"\t"<<"\t"<<par4[i][j]<<"\t"<<"\t"<<par5[i][j]<<'\t'<<"\t"<<par6[i][j]<</*'\t'<<parr3[i][j]<<"\t"<< "\t"<<parr4[i][j]<<"\t"<<"\t"<<parr5[i][j]<<'\t'<< "\t"<<parr33[i][j]<<"\t"<< "\t"<<parr00[i][j]<<"\t"<< "\t"<<par0[i][j]<<"\t"<< "\t"<<par1[i][j]<<*/"\n";

      finalfuncd[i][j]= new TF1("finalfuncd","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",3,80);
	  finalfuncd[i][j]->SetParameters(parr0[i][j],parr1[i][j],parr2[i][j],par3[i][j],par4[i][j],par5[i][j]);
	  /*+++++++++++++++++++finalfuncp[i][j]= new TF1("finalfuncp","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",10,105);
	  finalfuncp[i][j]->SetParameters(parr3[i][j],parr4[i][j],parr5[i][j],parr33[i][j],parr00[i][j],par0[i][j]);*/
	  ...
	  finalfuncp[i][j]= new TF1("finalfuncp","pol5",20,105);//pol6
	  finalfuncp[i][j]->SetParameters(parr3[i][j],parr4[i][j],parr5[i][j],parr33[i][j],parr00[i][j],par0[i][j]);//,par1[i][j]);
    }
    }
    //ps->Close();
  //+=+=+=+=+=+=+=+=+=+ start reading events +=+=+=+=+=+=+=+=+//
    if (fChain == 0) return;
    Long64_t nentries =1e6;// fChain->GetEntriesFast();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(b3_MWPCphi1>0. && b3_MWPCphi2>0. && b3_MWPCphi1<360. && b3_MWPCphi2<360.)	 Delphi= abs(b3_MWPCphi1-b3_MWPCphi2);
	  ...
	  if(b3_T1>0 && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 )
		...
		if (abs(b3_MWPCtheta1-tta1)<3 && abs(b3_MWPCtheta2-tta2)<3 && abs(abs(b3_MWPCphi1-b3_MWPCphi2)-pphi12)<20  && b3_WLdet1==w && b3_WLdet2==11-w)
	...
	// dTOF=PID_func();
		    if (deltTOF2>0 && deltTOF2<13){   
		      ...
		      //++if (b3_EFW1<Ymin[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymin[q][w]);//d_scalning(b3_EFW1,w,tta1);
		      //++if (b3_EFW1>=Ymin[q][w] && b3_EFW1<=Ymax[q][w]) 
		      //Ddetec = b3_EFW1;// /finalfuncd[q][w]->Eval(b3_EFW1);//d_scalning(b3_EFW1,w,tta1);
			//++if (b3_EFW1>Ymax[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymax[q][w]);//d_scalning(b3_EFW1,w,tta1);
		      
		      // if (b3_EFW2<Xmin[q][w]) Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(Xmin[q][w]);//p_scalning(b3_EFW2,w,tta2);
		      //if (b3_EFW2>=Xmin[q][w] && b3_EFW2<=Xmax[q][w]) Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(b3_EFW2);//p_scalning(b3_EFW2,w,tta2);
		      //if (b3_EFW2>Xmax[q][w]) Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(Xmax[q][w]);//p_scalning(b3_EFW2,w,tta2);
		      Ymin[q][w]=3;
		      Ymax[q][w]=80;
		      Xmin[q][w]=20;
		      Xmax[q][w]=105;
		      if (b3_EFW1<Ymin[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymin[q][w]);//d_scalning(b3_EFW1,w,tta1);
		      if (b3_EFW1>=Ymin[q][w] && b3_EFW1<=Ymax[q][w])    Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(b3_EFW1);//d_scalning(b3_EFW1,w,tta1);
		      if (b3_EFW1>Ymax[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymax[q][w]);//d_scalning(b3_EFW1,w,tta1);
		      ...
		      //Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(b3_EFW2);
...
Double_t   MAX0(Double_t a,Double_t b){
  if (a>b) return a;
  if (b>a) return b;
  if( a==b) return a; 
  else return -1;
...
int BookingHist(){
  for (int i=0;i<Nkinema;i++){
    for (int j=1;j<11;j++){
      sprintf(namegD[i][j],"gr_gainD_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],j);
      gainD[i][j] = new TGraph();
      gainD[i][j]->SetName(namegD[i][j]);
      sprintf(namegP[i][j],"gr_gainP_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],j);
      //gainP[i][j] = new TGraphErrors();
      gainP[i][j] = new TGraph();
      gainP[i][j]->SetName(namegP[i][j]);
    }
  }
       cc->Size(27,18);
      cc->SetFillColor(10);
...
int fitfunctions(){
  ...
  fitfuncp1= new TF1("fitfuncp1","pol5");//,.1,120);//pol6
  ...
  /*+++++++++++++fitfuncp1= new TF1("fitfuncp1","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",10,105);
  fitfuncp1->SetParameters(6.99657e-04,1e-5,6.9e-4,1e-5,1.00901e+00,.9);*/
  //+++++++++++++++fitfuncd1= new TF1("fitfuncd1","pol6");//,6,90);
  fitfuncd1= new TF1("fitfuncd1","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",3,90);
  fitfuncd1->SetParameters(3.65988e-03,1e-5,3.6e-3,1e-5,7.19197e-01,1.0);
