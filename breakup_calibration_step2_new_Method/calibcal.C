  ...
  //=============   start for scaling at detector ======/
   /*for (int i=0;i<Nkinema;i++){
	...
	for (int l=1; l<contrll[i]; l++){
	  ...
	  ///dist= TMath::Abs(ES1depr[i][l]-sss);
	  minf=MIN0(dist,minf);
	...
	if(scale21>.5 && scale21<3) {gainD[i][j]->SetPoint(lg,Ref,scale21);  gainD[i][j]->SetPointError(lg,0,0);}
	if(scale22>.6 && scale22<1.11) {gainP[i][j]->SetPoint(lg,sss,scale22);  /*gainP[i][j]->SetPointError(lg,0,0);*/}
	...
	parr00[i][j]=fffp->GetParameter(4);*/
      gainD[i][j]->Fit(fitfuncd1,"Q","",3,90);//6,90//3
      gainP[i][j]->Fit(fitfuncp1,"Q","",10,105);//.1,120//10,105
      ...
      finalfuncd[i][j]= new TF1("finalfuncd","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",3,90);
	  ...
	  /***+++++++++++++++finalfuncp[i][j]= new TF1("finalfuncp","pol5",10,105);//pol6
	  finalfuncp[i][j]->SetParameters(parr3[i][j],parr4[i][j],parr5[i][j],parr33[i][j],parr00[i][j],par0[i][j]);//,par1[i][j]);*/
    }
    }
    //ps->Close();
