...
void calibcal::Loop()
{
  ...
  //=============   start for scaling at detector ======//
	...
	sss=Ref;
	Ref=ss;
	//cout << "maxf=" << maxf << '\t' << sss << '\t' << i << '\t' << j << endl;
	maxf=MAX0(sss,maxf);
	if (maxf==sss) {Xmax[i][j]=sss; Ymin[i][j]=Ref;}
	...
	parr00[i][j]=fffp->GetParameter(4);*/
      gainD[i][j]->Fit(fitfuncd1,"NQ","",6,90);
      gainP[i][j]->Fit(fitfuncp1,"NQ","",.1,120);
	  ...
	  finalfuncp[i][j]->SetParameters(parr3[i][j],parr4[i][j],parr5[i][j],parr33[i][j],parr00[i][j],par0[i][j],par1[i][j]);
	     ...
	     for (int i=0;i<Nkinema;i++){
	       for (int j=1;j<11;j++){cout << "Xmin="<< Xmin[i][j] << '\t' << Xmax[i][j] << '\t' << Ymax[i][j] << endl;}}
    //ps->Close();
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	// dTOF=PID_func();
		    if (deltTOF2>0 && deltTOF2<13){   
		      ...
		      Ddetec = b3_EFW1;
		      if (b3_EFW1<=Ymax[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(b3_EFW1);//d_scalning(b3_EFW1,w,tta1);
		      if (b3_EFW1>Ymax[q][w]) Ddetec = b3_EFW1/finalfuncd[q][w]->Eval(Ymax[q][w]);//d_scalning(b3_EFW1,w,tta1);
		      Pdetec = b3_EFW2;
		      if (b3_EFW2<=Xmax[q][w]) Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(b3_EFW2);//p_scalning(b3_EFW2,w,tta2);
		      if (b3_EFW2>Xmax[q][w])  Pdetec = b3_EFW2/finalfuncp[q][w]->Eval(Xmax[q][w]);//d_scalning(b3_EFW1,w,tta1);
