...
{
  using namespace std;
  TFile *f = new TFile("vbreakaftercalibtdcCheck.root","recreate");
 ...
 TF1 *fcorec2=new TF1("fcorec2","-1.35 + (0.45*sin((3.14/180.)*x))**2",0,360);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	Double_t deltTDC=(tdc1t-tdc2t)+TDC_shift(w)/4;
	X1 =(TOF2p-TOF1p);
	X2 =(TOF2p-TOF1d);
	X3 =(TOF2d-TOF1p);
	Double_t X1X =(TOF2pp-TOF1pp);
	Double_t X2X =(TOF2pp-TOF1dd);
	Double_t X3X =(TOF1dd-TOF2pp);//(TOF2dd-TOF1pp);
	deltTOF1 = (X1X-deltTDC);
	deltTOF2 = (X2X-deltTDC);
	deltTOF3 =deltTDC/X3X;//(X3X-deltTDC);
	deltTOF3 += -1 -fcorec1->Eval(b3_MWPCphi1);
	h23[q]->Fill(deltTOF3);
	h00[q]->Fill(b3_MWPCphi1,deltTOF3);
	ET->Fill(E1,deltTOF2);
...
int BookingHist(){
  ...
  for (int i=0;i<Nkinema;i++){
      sprintf(car1[i],"h00_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i]);
      h00[i]=new TH2D(car1[i],";E_{p} [MeV];E_{d} [MeV]",360*2,0,360,200*4,-100,100);
      sprintf(car2[i],"h23_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i]);
      h23[i]=new TH1D(car2[i],";E_{d} [MeV]",200*4,-100,100);
    }
}
...
/* 25-25-180 */
  ...
  // double shift[11]={0.0, 3.0, 10.0, 5.96, 11.3, 7.04,  14.17, 4.05, 10.71, 4.02, 10.5};
 return  shift[i];
