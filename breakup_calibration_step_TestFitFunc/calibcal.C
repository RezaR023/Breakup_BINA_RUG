		...
		{  //<<--for BC
		  ...
		  if (Ref>0 && Ref<120)
		    {//120//100
		      ...
		      //test->Draw("cont0");
		      //cc->Update();
		      //cout << "contrll[i]=" << contrll[i]<< '\t' << i<< endl;
		      if (i==0)contrll[i]=1182;
		      else if (i==1)contrll[i]=1157;
		      else if (i==2)contrll[i]=1137;
		      else if (i==3)contrll[i]=1067;
		      else if (i==4)contrll[i]=1017;
		      else if (i==5)contrll[i]=978;
		      else if (i==6)contrll[i]=911;
		      else if (i==7) contrll[i]=864;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(b3_MWPCphi1>0. && b3_MWPCphi2>0. && b3_MWPCphi1<360. && b3_MWPCphi2<360.)	 Delphi= abs(b3_MWPCphi1-b3_MWPCphi2);
	  ...
	  if(b3_T1>0 && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 )
		...
		if (abs(b3_MWPCtheta1-tta1)<1 && abs(b3_MWPCtheta2-tta2)<1 && abs(abs(b3_MWPCphi1-b3_MWPCphi2)-pphi12)<20  && b3_WLdet1==w )
...
/*++==  }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	gainD[i][j]->Draw("samep");
	//	fitfuncd1[j][i]->Draw("same");
	cc->Update();
	ps->NewPage();
	cc->cd();
	fitfuncp1[j][i]->Draw();
	gainP[i][j]->SetMarkerStyle(7);
  	gainP[i][j]->Draw("samep");
	//		fitfuncp1[j][i]->Draw("same");
  	cc->Update();
      }
      }
     // for (int i=0;i<Nkinema;i++){
  ...
  for(int k=0 ; k<Nkinema ; k++)
{
  ...
  char fitname1[20][20][400],fitname2[20][20][400];
  for(int kk=1 ; kk<11 ; kk++){
    ...
    do { l1++;
      ...
      sprintf(fitname1[kk][l1],"fitfuncP_det_conf_%d_%d",kk,l1);
      fitfuncp1[kk][l1]= new TF1("fitname1[kk][l1]","pol5",10,105);
      fitfuncp1[kk][l1]->SetParameters(parr0[kk][l1],parr1[kk][l1],parr2[kk][l1],parr3[kk][l1],parr4[kk][l1],parr5[kk][l1]);
    ...
    do { l1++;
      parainputd >> par0[kk][l1]>>par1[kk][l1]>>par2[kk][l1]>>par3[kk][l1]>>par4[kk][l1]>>par5[kk][l1]>>par6[kk][l1];
      // parainputp >> parr0[kk][l1]>>parr1[kk][l1]>>parr2[kk][l1]>>parr3[kk][l1]>>parr4[kk][l1]>>parr5[kk][l1]>>parr6[kk][l1];
      sprintf(fitname2[kk][l1],"fitfuncD_det_conf_%d_%d",kk,l1);
      fitfuncd1[kk][l1]= new TF1("fitname2[kk][l1]","(2.0*[0]*x+[1]*x*x)/(sqrt(1+4*x*[2]+[3]*x*x/[4])-[5])",3,90);
      fitfuncd1[kk][l1]->SetParameters(par0[kk][l1],par1[kk][l1],par2[kk][l1],par3[kk][l1],par4[kk][l1],par5[kk][l1]);
