#define calibcal1_cxx
#include "calibmanip1.h"
...
{
  using namespace std;
  TFile *f = new TFile("vhistAllconfigvast.root","recreate");
  ...
  func1->SetParameters((18.72+.5),-0.8077,0.0194,-0.000254,1.723e-6,-4.736e-9);
  func2->SetParameters((25.69+.5),-0.7636,0.01181,-8.67e-5,2.307e-7);
  ...
  TH2F *hg2 = new TH2F("hg2","",150,0,150,150,0,150);
  ...
  do{k++;
    ...
    sc1->SetPoint(k,s2[k],s1[k]);  sc1->SetPointError(k,0,0);
...
//    htest2->Fill(s4[k],s3[k]);
  ...
  do{g++;
	...
	sc2->SetPoint(g,EPdep,EDdep);  sc2->SetPointError(g,0,0);
      }
  }while(!sputt.eof());
  double contrg=g;
  double contrgg=gg;
...
double  contr2=l;
  ...
  //TPostScript *ps1 = new TPostScript(psfile2,111);
  char ch1[11][500];
  char ch2[11][500];
  char cha[11][500];
  for (int i=1 ; i<11 ; i++)
    {
      sprintf(ch1[i],"EdEp_%d",i);
      sprintf(cha[i],"c1_%d",i);
      //EdEp[i] = new TH2D (ch1[i],"",600,0,1200,600,0,1200);
      //++EdEp[i]=(TH2D*)f1->Get(ch1[i]);
      // EdEp[i]->SetXTitll("E_{p} [channel]");
      // EdEp[i]->SetYTitll("E_{d} [channel]");
      //++CenterTitle(EdEp[i]);
      c1[i]=new TCanvas(cha[i],""); c1[i]->Size(27,18);
      c1[i]->SetFillColor(10);
      c1[i]->SetBottomMargin(.15);
      c1[i]->SetLeftMargin(.15);
  ...
  for (int i=0;i<Nkinema;i++){
    ...
    for (int j=1;j<11;j++){
      sprintf(nam2[i][j],"E1E2_%d_%d_%d_%d",(int)tta_1[i],(int)tta_2[i],(int)phi12[i],j);
    }
  }
  ...
  for (int i=0;i<Nkinema;i++){
    ...
    for (int j=1;j<11;j++){
      E1E2i[i][j]=new TH2D(nam2[i][j],";E_{p} [MeV];E_{d} [MeV]",120,0,120,120,0,120);
    }
  }
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	Delphi= abs(b3_MWPCphi1-b3_MWPCphi2);
      if(Delphi>180.) Delphi=360. - Delphi;
      //if(/*b3_Pol==Polar  && LT>0.1 && b3_DowScalT1<100*/)
	{ //<---!
	  if(b3_T1>0 /*&& abs(b3_WLtdc1-b3_WLtdc2)<80*/  && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 )
	    {//<<--<<--!
	      for(int q=0 ; q<Nkinema; q++)
	      {
		for(int w=1 ; w<11; w++){
		tta1=tta_1[q]; tta2=tta_2[q]; pphi12=phi12[q];
		//if (abs(b3_MWPCtheta1-tta1)<2 && abs(b3_MWPCtheta2-tta2)<2 && abs(abs(b3_MWPCphi1-b3_MWPCphi2)-pphi12)<10  && b3_WLdet1==2 && b3_WLdet2==9){posi1->Fill(b3_MWPCX1,b3_MWPCY1);}
	...
	//if (cutg->IsInside(E1,deltTOF2)){
	  if (deltTOF2>0 && deltTOF2<13){  he1->Fill(b3_EFW2,b3_EFW1); 
	    ...
	    E1E2i[q][w]->Fill(b3_EFW2,b3_EFW1); 
	    ...
	    Double_t b3_EFW1f=d_scalning(b3_EFW1,2);//b3_EFW1/(1.52948-0.0256455*b3_EFW1+0.000400344*pow(b3_EFW1,2)-2.02715e-06*pow(b3_EFW1,3));
...
for (int j=1; j<contrgg; j++){
 ...
 for(int i=1 ; i<5 ; i++){
   ...
   if(i==1) sc1->Draw();//"SC");
   if(i==2) sc2->Draw();//"SC");
   if(i==3) hg1->Draw("colz");
   if(i==4) hg2->Draw("cont0");
    c1[i]->Update();
 }
ps->Close();
...
int openfiles(){
  ...
  for(int k=0 ; k<Nkinema ; k++){
	...
	ES2dep[k][l]=func4->Eval(ES2[k][l]);
      ...
      p1p2dep[k]->SetPoint(l,ES1dep[k][l],ES2dep[k][l]);  p1p2dep[k]->SetPointError(l,0,0);
