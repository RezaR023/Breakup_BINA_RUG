...
#include <TStyle.h>
#include <TCanvas.h>
double Smin1[Nkinema],Smin2[Nkinema],Smax1[Nkinema],Smax2[Nkinema];
double tempcharg=0;
double kHzPol=0.0;
double TotBeamC=0.0;
double TotTime=0.0;
...
funcc2->SetParameters(25.69,-0.7636,0.01181,-8.67e-5,2.307e-7);
  ...
  TFile *f = new TFile("vcrstest.root","recreate");
  fitfunctions();
...
//  //TH2F *h00 = new TH2F("h00","",360*2,0,360,200*4,-100,100);
  TH2F *h000 = new TH2F("h000","",360*4,0,360,200*4,-100,100);
  ...
  TF1 *f31= new TF1("f31","gaus",-20,20);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //double LT=(b3_10kHZDT * b3_AFera)/(b3_10kHZ * b3_TFera);
    double LT=.4;//(b3_10kHZ * b3_AFera)/(b3_10kHZDT * b3_TFera);
    double dowscale=1;//b3_T1sDT/b3_T1sDTS;
    ...
    if(LT>.1 && dowscale<100 && b3_T1>0 && (b3_WLdet1!=b3_WLdet2) && (b3_WRdet1!=b3_WRdet2) &&  b3_EFW2>0 && b3_EFW1>0 )
      {//<<--<<--!
	...
	//int w=10;
	tta1=28; tta2=28;
	int qq=0;//number of config
	if (tta1<=20) q=1;
	  else q=5;
	if (abs(b3_MWPCtheta1-tta1)<1 && abs(b3_MWPCtheta2-tta2)<1 && abs(abs(b3_MWPCphi1-b3_MWPCphi2)-180)<5 )//&& (/*b3_WLdet1==1 || b3_WLdet1==2 ||/* b3_WLdet1==3 || b3_WLdet1==4 || b3_WLdet1==5 ||*/ b3_WLdet1==6))// && b3_WLdet2==10-w)
	  {//asume that first particle is d and second is p//
	    ...
	    if(b3_WLdet1==5){b3_WLtdc1=b3_WLtdc1; b3_WRtdc1=b3_WRtdc1+3;}
	    if(b3_WLdet2==5){b3_WLtdc2=b3_WLtdc2; b3_WRtdc2=b3_WRtdc2+3;}
	    if(b3_WLdet1==6){b3_WLtdc1=b3_WLtdc1+2.5-.8; b3_WRtdc1=b3_WRtdc1+1.5-.8;}
	    ...
	    deltTOF2 -= TDC_shift((int)b3_WLdet1,(int)b3_WLdet2,180);
	    //h21->Fill(deltTOF1);
	    h22->Fill(deltTOF2);
	    //cout << deltTOF2  <<endl;
	    //h23->Fill(deltTOF3);
	    ...
	    if (deltTOF2<1.2){   
	      ...
	      Ddetec = b3_EFW1/d_scalning(b3_EFW1,b3_WLdet1,tta1);//finalfuncd[q][w]->Eval(b3_EFW1);
	      
	      Pdetec = b3_EFW2/p_scalning(b3_EFW2,b3_WLdet1,tta2);//finalfuncp[q][w]->Eval(b3_EFW2);
		he->Fill(Pdetec,Ddetec);
		...
		he1->Fill(Ptarg,Dtarg);
		  ...
		  for(int j=0 ; j<Snr ; j++)
		    {
		      if (cut[qq][j]->IsInside(Ptarg,Dtarg))
			{
			  value1=projectScut(Ptarg,Dtarg,j,qq,corec);
			  sbin11[j]->Fill(b3_MWPCphi1,value1,dowscale/LT);
			  sbin1D[j]->Fill(value1,dowscale/LT);
	...
	for(int j=0 ; j<Snr ; j++)
	  {
	    ...
	    double sig1=sbin1D[j]->GetXaxis()->FindBin(bm-3*bn);
	    double sig2=sbin1D[j]->GetXaxis()->FindBin(bm+3*bn);
	    cout << sig1 << '\t' << sig2 << endl;
	    double countgaus =(double) sbin1D[j]->Integral(sig1,sig2);
	    ...
	    double deno = a*dx*omega1*omega2*deltaS;
	    double  AUB= (ct)/(deno);
	    cross[j]=(AUB*countgaus/(1e-3*/*charge*/tempcharg/8.35325));/* .5 is due to the bining of the histograms h1 & h2*/
	    cross[j]=cross[j]/(.84*.90*.99*.97);
  ...
  for(int k=0 ; k<Nkinema ; k++)
{
    ...
    do { l++;
      ...
      /*    if (ES2[k][l]>25 && ES1[k][l]>19.8 && ES1[k][l]<20) Smin1[k]=Scu[k][l];
      if (ES1[k][l]>25 && ES2[k][l]<25.1 && ES2[k][l]>24.9) Smax1[k]=Scu[k][l];
  ...
  //for (int k=0; k<Nkinema;k++)cout << Smin1[k] << '\t' << Smax1[k] << '\t' << Smin2[k] << '\t' << Smax2[k] << endl;
